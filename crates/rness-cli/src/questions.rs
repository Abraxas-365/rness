...
use rness_engine::questions::{Answer, Answers, QuestionUiConfig, Questions, Request};
...
mod tests {
                ...
                registry
                    .dispatch(
                        ...
                        &[ToolCall {
                            call: "c".into(),
                            name: "AskUser".into(),
                            args: serde_json::json!({"questions":[{"id":"q","question":"Notes?"}]}),
                        }],
                        1,
    ...
    async fn markdown_review_scroll_resize_and_explicit_choices() {
        ...
        registry.register(Arc::new(rness_engine::questions::AskUser(
            questions.clone(),
        )));
        let markdown = format!(
            "# Review title\n\n## Changes\n\n{}\n```rust\nlet ready = true;\n```\n\nEND-OF-PLAN",
            "- **Important** 日本語 café details\n".repeat(40)
        );
        ...
        tokio::time::timeout(std::time::Duration::from_secs(2), async {
            while questions.pending().is_empty() {
        ...
        for (width, height) in [(80, 24), (40, 12), (120, 35)] {
            ...
            assert_eq!(
                overlay.scroll,
                (1 + overlay.page_rows).min(overlay.max_scroll + 1)
            );
    ...
    async fn visual_selection_custom_text_and_render() {
        ...
        for (width, height) in [(80, 20), (40, 12), (120, 30)] {
            ...
            let snapshot = (0..height)
                ...
                .collect::<Vec<_>>()
                .join("\n");
            assert!(snapshot.contains("Choose a database"));
            assert!(snapshot.contains("PostgreSQL"));
            if height == 12 {
                assert!(snapshot.contains("SQLite"));
                assert!(snapshot.contains("Other / write an answer"));
            }
            if std::env::var_os("RNESS_QUESTION_SNAPSHOT").is_some() {
                println!("{snapshot}\n");
            }
        }
        ...
        apps.set_apps(vec![rness_tui::modules::ext_apps::AppInfo {
            name: "sessions".into(),
            slot: "overlay".into(),
            title: "Sessions".into(),
        ...
        rness_tui::modules::ext_apps::handle_global_key(
            &apps,
        ...
        if std::env::var_os("RNESS_QUESTION_INTERACTIVE").is_some() {
            ...
            while overlay.wants(&ctx) {
                terminal
                    .draw(|frame| {
                        let area = frame.area();
                        overlay.render(&ctx, area, frame.buffer_mut());
                    })
                    .unwrap();
                if let crossterm::event::Event::Key(key) = crossterm::event::read().unwrap() {
                    overlay.on_key(&ctx, key);
                }
            }
            ...
            for code in [
                ...
                overlay.on_key(&ctx, KeyEvent::new(code, KeyModifiers::NONE));
            }
        }
        let result = tokio::time::timeout(std::time::Duration::from_secs(2), task)
            .await
            ...
            .unwrap();
        assert!(!result[0].is_error);
        assert!(result[0].output.contains("SQLite"));
        assert!(result[0].output.contains("ok"));
    }
}
...
fn markdown_alias(ui: &QuestionUiConfig, action: &str, key: KeyEvent) -> bool {
    ...
    ui.keys.get(action).is_some_and(|binding| binding == action)
        && !ui
            .keys
            .values()
            .any(|binding| Chord::parse(binding) == Chord::parse(alias))
...
fn editor_key<'a>(ui: &'a QuestionUiConfig, action: &str) -> &'a str {
    ...
    ui.keys
        .get(action)
        .filter(|key| {
            !Chord::parse(key).is_some_and(|chord| {
                chord.code == KeyCode::Backspace
...
fn question_style(ctx: &Ctx<'_>, ui: &QuestionUiConfig, name: &str, mut fallback: Style) -> Style {
    ...
    if name != "panel" {
        if let Some(value) = ui.styles.get("panel") {
            if let Ok(panel) = ctx.theme.resolve_style(value, ctx.theme.overlay) {
                fallback.bg = panel.bg;
            }
        }
    }
    ui.styles
        .get(name)
        .and_then(|value| ctx.theme.resolve_style(value, fallback).ok())
        .unwrap_or(fallback)
}
...
fn padded_area(area: Rect, ui: &QuestionUiConfig) -> Rect {
    ...
    Rect::new(
        ...
        area.width
            .saturating_sub(left)
            .saturating_sub(ui.padding.right),
        area.height
            .saturating_sub(top)
            .saturating_sub(ui.padding.bottom),
    )
}
...
fn question_help(
    ...
    let cancel = if ui.keys.get("cancel").is_some_and(|s| s == "ctrl+c") {
        "ctrl+c:cancel".into()
    } else {
        join(vec![hint("cancel", "cancel"), "ctrl+c:cancel".into()])
    };
    if editing {
        return format!(
            "{}\n{}",
            join(vec![
                format!("{}:done", editor_key(ui, "submit")),
                format!("{}:back", editor_key(ui, "dismiss"))
            ]),
            cancel
        );
    }
    ...
    if detail {
        return format!(
            "{}\n{}",
            join(vec![hint("dismiss", "back"), cancel]),
            join(vec![hint("page_up", "scroll"), hint("details", "scroll")])
        );
    }
    if width < 40 {
        format!(
            ...
            join(vec![hint("dismiss", "close"), hint("details", "details")])
        )
    } else {
        format!(
            "{}\n{}",
    ...
    fn sync(&mut self, request: &Request) {
        ...
        if self.key.as_ref() != Some(&key) {
            ...
            self.drafts.retain(|key, _| {
                pending
                    .iter()
                    .any(|r| r.session == key.0 && r.call == key.1)
            });
            ...
            self.editing = false;
            self.error.clear();
            self.scroll = if request.questions[0].markdown.is_some() {
                1
            } else {
                0
            };
...
impl Component for QuestionOverlay {
    ...
    fn on_paste(&mut self, ctx: &Ctx<'_>, text: &str) -> KeyOutcome {
        let Some(request) = self
            ...
            .pending()
            .into_iter()
            .find(|p| p.session == ctx.model.session)
        else {
            return KeyOutcome::pass();
        };
        self.sync(&request);
        if self.editing {
            if let Some(answer) = self.answers.get_mut(self.page) {
                answer.custom.get_or_insert_default().push_str(text);
            }
        }
        KeyOutcome::consumed()
    }
    ...
    fn priority(&self) -> Option<i32> {
        Some(self.questions.overlay_config().priority)
    }
    fn wants(&self, ctx: &Ctx<'_>) -> bool {
        !self.apps.as_ref().is_some_and(|apps| apps.active_overlay())
            && self
                .questions
                .pending()
                .iter()
                .any(|p| p.session == ctx.model.session)
    }
    fn height(&self, ctx: &Ctx<'_>, _: u16) -> Option<u16> {
        Some(
            self.questions
                ...
                .iter()
                .find(|p| p.session == ctx.model.session)
                .and_then(|p| p.questions.first())
                .and_then(|q| q.plan_review.as_ref())
                .map_or(self.questions.overlay_config().height, |config| {
                    config.height
                }),
        )
    }
    fn render(&mut self, ctx: &Ctx<'_>, area: Rect, buf: &mut Buffer) {
        ...
        if area.width < 30 || area.height < 10 {
            let help = if ui.show_help {
                format!("\n{}", question_help(ui, false, true, false, area.width))
            } else {
                String::new()
            };
            Paragraph::new(format!("Resize to 30x10{help}"))
        ...
        if inner.width < 10 || inner.height < 6 {
            let notice = if ui.show_help {
                format!(
                    "Resize or reduce padding\n{}",
                    question_help(ui, self.editing, false, false, area.width)
                )
            } else {
                "Resize or reduce padding".into()
            };
        ...
        if self.scroll > 0 && q.markdown.is_some() {
            let chunks = Layout::vertical([
                Constraint::Min(1),
                Constraint::Length(if ui.show_help { 2 } else { 0 }),
            ])
            .split(inner);
            let lines = rness_tui::core::render::render_markdown(
                q.markdown.as_deref().unwrap(),
                chunks[0].width,
                ctx.theme,
            );
            ...
            Paragraph::new(lines)
                ...
                .render(chunks[0], buf);
            Paragraph::new(question_help(ui, false, false, true, inner.width))
                .style(style("help", ctx.theme.dim))
                .render(chunks[1], buf);
            return;
        }
        let compact = inner.height < 14;
        let footer_rows = if ui.show_help || !self.error.is_empty() {
            2
        } else {
            0
        };
        ...
        .split(inner);
        Paragraph::new(q.question.as_str())
            .style(style("question", ctx.theme.heading))
            .wrap(Wrap { trim: false })
            .render(chunks[0], buf);
        let detail = self.scroll > 0;
        if detail {
            let text = if let Some(option) = q.options.get(self.cursor) {
                if ui.descriptions == "never" {
            ...
            let lines: Vec<Line> = text
                .lines()
                .flat_map(|line| {
                    textwrap::wrap(line, chunks[1].width.max(1) as usize)
                        .into_iter()
                        .map(|s| Line::raw(s.into_owned()))
                        .collect::<Vec<_>>()
                })
                .collect();
            let offset = (self.scroll - 1).min(
                lines
                    .len()
            ...
            Paragraph::new(lines)
                .scroll((offset, 0))
                .style(style("description", ctx.theme.dim))
                .render(chunks[1], buf);
        } else {
            let cursor_width = Line::raw(ui.symbols.cursor.as_str()).width();
            let mut items: Vec<_> = q
                .options
                .iter()
                .map(|o| {
                    let mark = if answer.selected.contains(&o.label) {
                        &ui.symbols.selected
                    } else {
                        &ui.symbols.unselected
                    };
                    let width = (chunks[1].width as usize)
                        .saturating_sub(cursor_width)
                        .max(1);
                    let mut lines: Vec<Line> =
                        textwrap::wrap(&format!("{mark} {}", o.label), width)
                            .into_iter()
                            .map(|s| Line::raw(s.into_owned()))
                            .collect();
                    if (ui.descriptions == "always" || (ui.descriptions != "never" && !compact))
                        && !o.description.is_empty()
                    {
                        lines.extend(textwrap::wrap(&o.description, width).into_iter().map(|s| {
                            Line::styled(s.into_owned(), style("description", ctx.theme.dim))
                        }));
                    }
                    let available = chunks[1].height as usize;
                    if lines.len() > available && available > 0 {
                        lines.truncate(available);
                        let hint = if ui.show_help {
                            ui.keys
                                .get("details")
                                .map(|key| format!("… {key}: full details"))
                                .unwrap_or_else(|| "…".into())
                        } else {
                            "…".into()
                        };
                    ...
                    lines.extend(
                        (0..ui
                            .option_spacing
                            .min(chunks[1].height.saturating_sub(lines.len() as u16)))
                            .map(|_| Line::raw("")),
                    );
                    ListItem::new(lines)
                })
            ...
            StatefulWidget::render(
                List::new(items)
                    .style(style("option", ctx.theme.overlay))
                    .highlight_symbol(ui.symbols.cursor.as_str())
                    .highlight_style(style("selected", ctx.theme.statusline_accent)),
                chunks[1],
                buf,
                &mut state,
            );
        ...
        let text = format!(
            ...
            answer.custom.as_deref().unwrap_or("")
        );
        let lines: Vec<Line> = textwrap::wrap(&text, chunks[2].width.max(1) as usize)
            .into_iter()
            .map(|s| Line::raw(s.into_owned()))
            .collect();
        let offset = if self.editing {
            lines
                .len()
        ...
        Paragraph::new(lines)
            .scroll((offset, 0))
            .style(style("input", ctx.theme.editor_prompt))
            .render(chunks[2], buf);
        let footer = if self.error.is_empty() {
            question_help(ui, self.editing, detail, false, inner.width)
        } else {
            self.error.clone()
        };
        Paragraph::new(footer)
            .style(if self.error.is_empty() {
                style("help", ctx.theme.dim)
            } else {
                style("error", ctx.theme.error)
            })
            .render(chunks[3], buf);
    }
    fn on_key(&mut self, ctx: &Ctx<'_>, key: KeyEvent) -> KeyOutcome {
        ...
        if key.code == KeyCode::Char('c') && key.modifiers.contains(KeyModifiers::CONTROL) {
            return KeyOutcome::act(vec![rness_tui::app::Action::Cancel]);
        }
        if let Some(config) = &request.questions[self.page].plan_review {
        ...
        if self.editing {
            if ["dismiss", "submit"]
                .iter()
                .any(|action| chord_matches(editor_key(&config.ui, action), key))
            {
                self.editing = false;
            }
            return KeyOutcome::consumed();
        ...
        if self.scroll > 0 && request.questions[self.page].markdown.is_some() {
            if matches("down") || markdown_alias(&config.ui, "down", key) {
                self.scroll = self.scroll.saturating_add(1).min(self.max_scroll + 1);
            } else if matches("up") || markdown_alias(&config.ui, "up", key) {
                self.scroll = self.scroll.saturating_sub(1).max(1);
            } else if matches("details") {
                self.scroll = self
                    .scroll
            ...
            } else if matches("submit") || matches("next") {
                self.scroll = 0;
            } else if matches("dismiss") {
        ...
        if matches("dismiss") && self.scroll > 0 {
            self.scroll = 0;
            return KeyOutcome::consumed();
        }
        self.scroll = 0;
        ...
        if matches("dismiss") {
            ...
            self.cursor = (self.cursor + 1).min(q.options.len());
        } else if matches("next") {
            self.page = (self.page + 1) % request.questions.len();
            self.cursor = 0;
        } else if matches("previous") {
            self.page = (self.page + request.questions.len() - 1) % request.questions.len();
            self.cursor = 0;
                    ...
                    .map_or(true, |s| s.trim().is_empty())
        {
            ...
            if answer.selected.contains(label) {
                answer.selected.retain(|s| s != label);
            } else {
                if !q.multi_select {
            ...
            if answer.selected.is_empty()
                && answer.custom.as_ref().map_or(true, |s| s.trim().is_empty())
                && !q.options.is_empty()
            {
                answer.selected.push(q.options[self.cursor].label.clone());
            }
            if self.page + 1 < request.questions.len() {
                self.page += 1;
                self.cursor = 0;
            } else if let Err(error) = self.questions.resolve(
                &request.session,
                &request.call,
                Answers {
...
mod configurable_tests {
    ...
    fn help_uses_active_bindings_and_editor_fallback() {
        ...
        for (action, key) in [
            ...
            ui.keys.insert(action.into(), key.into());
        }
        ...
        for hint in [
            ...
            assert!(help.contains(hint), "{help}");
        }
        for stale in ["enter", "esc", "pagedown"] {
            assert!(!help.contains(stale));
        }
        let help = question_help(&ui, false, false, true, 120);
        for hint in ["f2:choices", "home:first", "end:last"] {
            assert!(help.contains(hint));
        }
        ui.keys.insert("submit".into(), "x".into());
        ui.keys.insert("dismiss".into(), "q".into());
        let help = question_help(&ui, true, false, false, 120);
        assert!(
            help.contains("enter:done") && help.contains("esc:back") && !help.contains("x:done")
        );
        ui.show_help = false;
        for (editing, detail, markdown) in [
            (false, false, false),
            (true, false, false),
            (false, true, false),
            (false, false, true),
        ] {
            assert!(question_help(&ui, editing, detail, markdown, 120).is_empty());
        }
    }
    #[tokio::test]
    async fn configured_layout_styles_symbols_and_descriptions() {
        ...
        config.ui.padding.left = 2;
        config.ui.padding.top = 1;
        config.ui.border = "double".into();
        config.ui.option_spacing = 1;
        config.ui.descriptions = "always".into();
        config.ui.show_help = false;
        config.ui.symbols.cursor = "→ ".into();
        config.ui.symbols.unselected = "( )".into();
        config.ui.symbols.selected = "(*)".into();
        config.ui.symbols.custom = "++".into();
        config.ui.labels.other = "Write here".into();
        config.ui.labels.custom = "Response: ".into();
        for (name, color) in [
            ("panel", "blue"),
            ("border", "red"),
            ("title", "green"),
            ...
            config
                .ui
                .styles
                .insert(name.into(), serde_json::json!({"fg":color}));
        }
        ...
        assert!(
            view.contains("→ ( ) Alpha")
                && view.contains("++ Write here")
                && view.contains("Response: "),
            "{view}"
        );
        ...
        config
            .ui
            .styles
            .insert("question".into(), serde_json::json!("missing"));
        assert_eq!(
            question_style(&ctx, &config.ui, "question", theme.heading),
            theme.overlay.patch(theme.heading)
        );
        config
            .ui
            .styles
            .insert("question".into(), serde_json::json!("added"));
        assert_eq!(
            question_style(&ctx, &config.ui, "question", theme.heading),
            theme.overlay.patch(theme.heading).patch(theme.added)
        );
        let changed = rness_tui::theme::Theme {
        ...
        config
            .ui
            .styles
            .insert("panel".into(), serde_json::json!({"bg":"green"}));
        assert_eq!(
            question_style(&ctx, &config.ui, "option", theme.overlay).bg,
            Some(Color::Green)
        );
    }
    #[tokio::test]
    async fn tiny_layouts_clamp_padding_and_hidden_help_keeps_errors() {
        ...
        let mut config = overlay.questions.overlay_config();
        config.ui.width = Some(120);
        config.ui.show_help = false;
        config.ui.padding.left = u16::MAX;
        config.ui.padding.top = u16::MAX;
        config.ui.padding.right = u16::MAX;
        config.ui.padding.bottom = u16::MAX;
        overlay.questions.set_overlay_config(config.clone());
        for (width, height) in [(0, 0), (1, 1), (20, 8), (30, 10), (40, 12)] {
            ...
            if width >= 30 {
                assert!(view.contains("Resize or reduce padding"));
            }
        }
        ...
        config
            .ui
            .styles
            .insert("error".into(), serde_json::json!({"fg":"red"}));
    ...
    async fn remapped_keys_replace_defaults_and_preserve_custom_input() {
        ...
        for (action, key) in [
            ...
            config.ui.keys.insert(action.into(), key.into());
        }
        overlay.questions.set_overlay_config(config.clone());
        let press = |overlay: &mut QuestionOverlay, code| {
            overlay.on_key(&ctx, KeyEvent::new(code, KeyModifiers::NONE))
        };
        for code in [
            KeyCode::Down,
        ...
        press(&mut overlay, KeyCode::Char('s'));
        assert_eq!(overlay.answers[0].selected, ["Beta"]);
        press(&mut overlay, KeyCode::Char('n'));
        assert_eq!(overlay.page, 1);
        press(&mut overlay, KeyCode::Char('p'));
        assert_eq!(overlay.page, 0);
        press(&mut overlay, KeyCode::Char('d'));
        assert_eq!(overlay.scroll, 1);
        press(&mut overlay, KeyCode::Char('q'));
        assert_eq!(overlay.scroll, 0);
        overlay.cursor = 2;
        press(&mut overlay, KeyCode::Char('x'));
        assert!(overlay.editing);
        for c in "kj sxqnpd日本語".chars() {
            press(&mut overlay, KeyCode::Char(c));
        }
        assert_eq!(
            overlay.answers[0].custom.as_deref(),
            Some("kj sxqnpd日本語")
        );
        assert!(overlay.editing);
        press(&mut overlay, KeyCode::Backspace);
        assert!(overlay.answers[0]
            .custom
            .as_ref()
            .unwrap()
            .ends_with("日本"));
        assert!(matches!(
            press(&mut overlay, KeyCode::F(6)).actions.as_slice(),
            [rness_tui::app::Action::Cancel]
        ));
        assert!(matches!(
            overlay
                .on_key(
        ...
        assert!(
            !overlay.editing,
            "printable submit has safe editor fallback"
        );
        overlay.editing = true;
        config.ui.keys.insert("submit".into(), "ctrl+e".into());
        overlay.questions.set_overlay_config(config);
        press(&mut overlay, KeyCode::Enter);
        assert!(overlay.editing, "nonprintable remap replaces old submit");
        overlay.on_key(
            &ctx,
