...
mod tests {
    use super::*;
    use rness_engine::{plan::{ExitPlan, PlanConfig}, session::branch::SessionStore, tools::{ToolRegistry, ToolCall}};
    use rness_protocol::events::{SessionEvent, PlanReview};
    ...
    async fn plan_editor_roundtrip_and_configurable_review() {
        for auto_approve in [true, false] {
            ...
            tools.register(Arc::new(ExitPlan { config, store, questions: questions.clone(), alive: CancellationToken::new() }));
            let s = session.clone();
            ...
            assert!(matches!(overlay.on_key(&ctx, KeyEvent::new(KeyCode::F(6), KeyModifiers::NONE)).actions.as_slice(), [Action::Cancel]));
            overlay.on_key(&ctx, KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
            assert!(overlay.on_key(&ctx, KeyEvent::new(KeyCode::Char('e'), KeyModifiers::NONE)).actions.is_empty());
            let outcome = overlay.on_key(&ctx, KeyEvent::new(KeyCode::Char('e'), KeyModifiers::CONTROL));
            let Action::Custom(name, mut payload) = outcome.actions.into_iter().next().unwrap() else { panic!("editor action") };
            ...
            let mut stale = payload.clone(); stale["call"] = serde_json::json!("old-review");
            overlay.on_action(&ctx, "questions:plan-edited", &stale);
            assert!(!questions.pending().is_empty());
            overlay.on_action(&ctx, "questions:plan-edited", &payload);
            if !auto_approve {
                assert!(!questions.pending().is_empty());
                assert_eq!(overlay.answers[0].edited_markdown.as_deref(), payload["text"].as_str());
            ...
            assert!(results[0].output.contains("# Edited plan\nUse the user's version"));
            overlay.on_action(&ctx, "questions:plan-edited", &payload);
            assert!(questions.pending().is_empty());
        }
    }
...
impl QuestionOverlay {
    pub(super) fn render_plan(&mut self, ctx: &Ctx<'_>, area: Rect, buf: &mut Buffer, request: &Request, config: &PlanReviewConfig) {
        let ui = config.ui();
        ...
        let block = Block::default().borders(if config.border == "none" { Borders::NONE } else { Borders::ALL })
            .border_type(border).title(Line::styled(format!(" {} ", config.title), style("title", ctx.theme.heading)))
        ...
        if inner.height < 6 || inner.width < 24 {
            Paragraph::new("Resize to review plan").style(style("error", ctx.theme.error)).render(inner, buf);
            return;
        }
        let hint = |action: &str, label: &str| format!("{}: {}", config.keys[action], label);
        let mut actions = vec![hint("approve", &config.labels.approve), hint("feedback", &config.labels.feedback)];
        if config.edit_enabled { actions.push(hint("edit", if config.approve_after_edit { &config.labels.edit_and_approve } else { &config.labels.edit })); }
        ...
        let action_lines = if wrapped.len() > 3 {
            actions.into_iter().map(|text| {
                let mut text = text.chars().take(inner.width.saturating_sub(1) as usize).collect::<String>();
                text.push('…'); Line::raw(text)
            }).collect::<Vec<_>>()
        } else { wrapped.into_iter().map(|s| Line::raw(s.into_owned())).collect::<Vec<_>>() };
        let footer = if self.editing {
            format!("{} ({}: send, {}: back): {}", config.labels.feedback_prompt, config.keys["feedback_submit"], config.keys["feedback_back"], self.answers[self.page].custom.as_deref().unwrap_or(""))
        } else if !self.error.is_empty() { self.error.clone() }
        else if config.show_help {
            format!("{}/{}: scroll  {}/{}: page  {}: close", config.keys["up"], config.keys["down"], config.keys["page_up"], config.keys["page_down"], config.keys["dismiss"])
        } else { String::new() };
        let footer_lines = textwrap::wrap(&footer, inner.width as usize).into_iter().map(|s| Line::raw(s.into_owned())).collect::<Vec<_>>();
        let footer_height = footer_lines.len().min(2) as u16;
        let chunks = Layout::vertical([Constraint::Min(1), Constraint::Length(action_lines.len().min(3) as u16), Constraint::Length(footer_height)]).split(inner);
        let answer = &self.answers[self.page];
        let markdown = answer.edited_markdown.as_deref().or(request.questions[self.page].markdown.as_deref()).unwrap_or("");
        ...
        Paragraph::new(action_lines).style(style("option", ctx.theme.heading)).render(chunks[1], buf);
        let offset = footer_lines.len().saturating_sub(footer_height as usize) as u16;
        Paragraph::new(footer_lines).scroll((offset, 0)).style(style(if self.error.is_empty() { "help" } else { "error" }, if self.error.is_empty() { ctx.theme.dim } else { ctx.theme.error })).render(chunks[2], buf);
    }
    ...
    fn submit_plan(&mut self, request: &Request, approve: bool) {
        ...
        if approve { answer.custom = None; } else { answer.edited_markdown = None; }
        if let Err(error) = self.questions.resolve(&request.session, &request.call, Answers { answers: vec![answer] }) { self.error = error; }
    }

    pub(super) fn plan_key(&mut self, key: KeyEvent, request: &Request, config: &PlanReviewConfig) -> KeyOutcome {
        let matches = |action: &str| config.keys.get(action).is_some_and(|binding| chord_matches(binding, key));
        self.error.clear();
        if self.editing {
            if matches("cancel") && !matches!(key.code, KeyCode::Backspace)
            ...
            if matches("feedback_back") { self.editing = false; return KeyOutcome::consumed(); }
            if matches("feedback_submit") { self.submit_plan(request, false); return KeyOutcome::consumed(); }
            match key.code {
                KeyCode::Backspace => { self.answers[self.page].custom.get_or_insert_default().pop(); }
                KeyCode::Char(c) if !key.modifiers.intersects(KeyModifiers::CONTROL | KeyModifiers::ALT) => self.answers[self.page].custom.get_or_insert_default().push(c),
                _ => {}
            }
            return KeyOutcome::consumed();
        }
        if matches("cancel") { return KeyOutcome::act(vec![Action::Cancel]); }
        if matches("dismiss") { self.questions.dismiss(&request.session, &request.call); }
        else if matches("approve") { self.submit_plan(request, true); }
        else if matches("feedback") { self.editing = true; }
        else if matches("edit") && config.edit_enabled {
            let text = self.answers[self.page].edited_markdown.as_deref().or(request.questions[self.page].markdown.as_deref()).unwrap_or("");
            return KeyOutcome::act(vec![Action::Custom("terminal:edit-plan".into(), serde_json::json!({
                "text":text, "editor":config.editor, "session":request.session, "call":request.call,
                "question":request.questions[self.page].id,
            }))]);
        }
        else if matches("down") { self.scroll = self.scroll.saturating_add(1).min(self.max_scroll + 1); }
        else if matches("up") { self.scroll = self.scroll.saturating_sub(1).max(1); }
        else if matches("page_down") { self.scroll = self.scroll.saturating_add(self.page_rows).min(self.max_scroll + 1); }
    ...
    pub(super) fn plan_edited(&mut self, ctx: &Ctx<'_>, payload: &serde_json::Value) {
        ...
        let Some(request) = self.questions.pending().into_iter().find(|r|
            payload["session"] == r.session && payload["call"] == r.call && r.session == ctx.model.session) else { return; };
        let Some(question) = request.questions.first().filter(|q| payload["question"] == q.id) else { return; };
        let Some(config) = question.plan_review.as_ref().filter(|c| c.edit_enabled) else { return; };
        self.sync(&request);
        if let Some(error) = payload["error"].as_str() { self.error = format!("Editor failed; plan unchanged: {error}"); return; }
        let Some(text) = payload["text"].as_str() else { return; };
        if let Err(error) = rness_engine::plan::validate_plan(text) { self.error = error; return; }
        self.error.clear();
        self.answers[0].edited_markdown = Some(text.into());
        self.scroll = 1;
        if config.approve_after_edit { self.submit_plan(&request, true); }
    }
}
