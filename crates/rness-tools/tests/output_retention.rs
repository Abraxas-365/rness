...
use rness_tools::{bash::BashTool, jobs::{JobRegistry, JobOutputTool, JobStatus}, Workspace};
use serde_json::json;
...
async fn full_background_output_is_paged_and_survives_recovery() {
    ...
    assert!(tool.execute_in(&"other".into(), json!({"job_id":id,"offset":0})).await.is_err());
    let text = tool.execute_in(&"owner".into(), json!({"job_id":id})).await.unwrap();
    ...
    let page = tool.execute_in(&"owner".into(), json!({"job_id":id,"offset":0})).await.unwrap();
    assert!(page.starts_with(&"a".repeat(65536)));
    assert!(page.contains("next offset: 65536"));
    drop(tool); drop(writer); drop(jobs);
    let jobs = JobRegistry::new();
    jobs.enable_persistence(dir.path()).unwrap();
    let page = JobOutputTool::new(jobs).execute_in(&"owner".into(), json!({"job_id":id,"offset":200000})).await.unwrap();
...
async fn foreground_spool_failure_stops_silent_command_promptly() {
            ...
            for directory in std::fs::read_dir(dir.path()).unwrap().flatten() {
                if let Ok(entries) = std::fs::read_dir(directory.path()) {
                    for entry in entries.flatten() {
                        if entry.path().extension().is_some_and(|ext| ext == "output") { return entry.path(); }
                    }
                }
            }
    ...
    let error = tokio::time::timeout(std::time::Duration::from_secs(5), task).await.unwrap().unwrap().unwrap_err();
    assert!(error.contains("capture") || error.contains("persistence"), "{error}");
}
...
async fn foreground_truncation_retains_retrievable_full_output() {
    ...
    let page = JobOutputTool::new(jobs).execute_in(&"owner".into(), json!({"job_id":id,"offset":0})).await.unwrap();
