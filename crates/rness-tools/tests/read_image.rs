...
async fn image_reader_is_opt_in_and_returns_durable_normalized_content() {
    ...
    let (parts, _, _) = reader
        .execute_rich(
            &"s".into(),
            "c",
            json!({"file_path":"extensionless"}),
            &Default::default(),
        )
        .await
        .unwrap();
    ...
    let (result, _) = rness_engine::tools::exposure::program(
        registry,
        "s".into(),
        rness_engine::tools::ToolCall {
            call: "ptc".into(),
            name: "run_code".into(),
            args: json!({"code":"return tools.call('read_image', {file_path='extensionless'})"}),
        },
        Default::default(),
        None,
    )
    ...
    assert!(result
        .content
        .iter()
        .any(|part| matches!(part, ToolResultContentPart::Image { .. })));
...
async fn image_reader_refuses_invalid_oversized_and_unavailable_inputs() {
    ...
    cancel.cancel();
    assert!(reader
        .execute_rich(&"s".into(), "c", json!({"file_path":"image"}), &cancel)
        .await
        .is_err());
