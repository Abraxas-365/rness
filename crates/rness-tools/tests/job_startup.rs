...
use rness_engine::{service::SessionService, session::branch::SessionStore,
    tools::ToolRegistry, turn::{TurnConfig, provider::{Provider, StepOutcome, StepRequest}}};
...
async fn recovered_job_waits_for_attachment_after_startup_config() {
    ...
    let session = store.create(Some(dir.path().display().to_string())).unwrap().session().clone();
    ...
    recovered.enable_persistence(&dir.path().join("jobs")).unwrap();
    assert_eq!(recovered.inspect(&session, &job).unwrap().job.status, "interrupted");
    let sessions = Arc::new(SessionService::new(store, Arc::new(Answer),
    ...
    let config = CallConfig { reasoning: Some(Reasoning::Effort { effort: "high".into() }), ..Default::default() };
    ...
    assert_eq!(recovered.inspect(&session, &job).unwrap().output, "retained output");
    assert!(!rness_engine::service::ServiceError::Busy.to_string().contains("compact"));
