...
use rness_protocol::events::{SessionId, TaskSnapshot, ToolResultContentPart};
...
impl Tool for ReadImage {
    ...
    async fn execute(&self, _: Value) -> Result<String, String> {
        Err("read_image requires session-aware rich tool dispatch".into())
    }
    async fn execute_rich(
        ...
        let store = self
            .images
            .get()
            .ok_or("image storage is not configured")?
            .clone();
        ...
        let path = if path.is_absolute() {
            ...
            self.workspace
                .as_ref()
                .ok_or("relative image path requires a session workspace")?
                .join(path)
        };
        ...
        let work = tokio::task::spawn_blocking(move || {
            ...
            if meta.len() > limit {
                return Err("image exceeds input byte limit".into());
            }
            ...
            Ok((
                vec![
                    ...
                    ToolResultContentPart::Image { attachment },
                ],
                None,
                false,
            ))
