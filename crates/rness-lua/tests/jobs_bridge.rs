...
async fn jobs_command_works_while_busy_without_history_or_model_activity() {
    ...
    let complete = || sessions.prepare_command(&session, "/jobs").unwrap().unwrap()
        .complete_items(&sessions).unwrap();
    let choices = complete();
    assert_eq!(choices.len(), 5);
    for expected in [
        ("".into(), "Open jobs monitor".into()),
        ("list".into(), "List active background jobs in this session".into()),
        ...
        assert!(choices.contains(&expected), "missing {expected:?} in {choices:?}");
    }
    assert!(!choices.iter().any(|(value, _)| value.contains(&exited) || value.contains(&killed)));
    for command in ["/jobs list"] {
        let result = sessions.send(&session, UserIntent::Followup,
            vec![ContentPart::Text { text: command.into() }]).unwrap();
        let rness_engine::inbox::Disposition::Command(result) = result else { panic!("expected command") };
    ...
    host.load("legacy-completions", "rness.commands.completion_descriptions = nil").await.unwrap();
    let legacy = complete();
    assert_eq!(legacy.iter().map(|(value, _)| value).collect::<Vec<_>>(),
    ...
    assert_eq!(complete().iter().map(|(value, _)| value.as_str()).collect::<Vec<_>>(), ["", "list", "stop"]);
    let result = sessions.send(&session, UserIntent::Followup,
        vec![ContentPart::Text { text: "/jobs list".into() }]).unwrap();
    assert!(matches!(result, rness_engine::inbox::Disposition::Command(result)
        if result.message == "No active background jobs in this session."));
    let result = sessions.send(&session, UserIntent::Followup,
        vec![ContentPart::Text { text: format!("/jobs {exited}") }]).unwrap();
    assert!(matches!(result, rness_engine::inbox::Disposition::Command(result)
...
async fn jobs_startup_setup_survives_mount_and_reload_with_callbacks() {
    ...
    for reload in [false, true] {
        ...
        assert_eq!(host.app_view("jobs", serde_json::json!({"session":"owner"})).await.unwrap()[0], "From init: owner");
    }
}
...
async fn jobs_failed_reload_preserves_selection_options_and_setup_api() {
    ...
    host.load("command-fixture", "rness.commands.register=function(command) jobs_command=command end").await.unwrap();
    let plugin = include_str!("../../../flavors/default/plugins/jobs.lua");
    host.load("jobs", plugin).await.unwrap();
    host.load("select", &format!(r#"
    ...
    assert_eq!(host.app_view("jobs", ctx).await.unwrap()[0], "Original list");
    writer.settle(JobStatus::Exited(Some(0)));
}
