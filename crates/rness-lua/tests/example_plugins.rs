...
async fn default_subagent_renderer_bounds_activity_without_nested_dumps() {
    ...
    assert!(lines
        .iter()
        .all(|line| line.structured && line.block.is_none()));
    ...
    assert_eq!(
        lines[3].text,
        "Recent tools: 4 · 1 running · 1 done · 1 failed · 1 cancelled"
    );
...
async fn default_subagent_renderer_handles_missing_metadata_and_errors() {
    ...
    let lines = host
        .tool_card_presented(
            ...
            "",
            false,
            Some(serde_json::json!({"kind":"subagent_activity"})),
        )
        .await
        .unwrap();
    ...
    for status in ["completed", "cancelled", "error", "failed"] {
        let lines = host
            .tool_card_presented(
                ...
                "",
                false,
                Some(serde_json::json!({"kind":"subagent_activity", "status":status, "tools":[]})),
            )
            .await
            .unwrap();
        assert_eq!(
    ...
    for (args, meta) in [
        (
            serde_json::json!({"background_mode":"continuable"}),
            serde_json::json!({"kind":"subagent_activity", "status":"completed"}),
        ),
        (
            serde_json::json!({}),
            serde_json::json!({"kind":"subagent_activity", "status":"completed", "mode":"continuable"}),
        ),
    ] {
        let lines = host
            .tool_card_presented("subagent", args, "", false, Some(meta))
            .await
            .unwrap();
...
async fn default_send_message_renderer_preserves_actual_long_multiline_message() {
    ...
    let (host, config) = LuaHost::spawn_from_init(root).unwrap();
    assert_eq!(
        config.messagebox["tools"]["send_message"]["display"],
        "preview"
    );
    assert_eq!(
        config.messagebox["tools"]["send_message"]["preview_lines"],
        4
    );
    for message in [
        "日本語 café ".repeat(1000),
        "First line\n\n  preserve indentation\n".repeat(30),
    ] {
        for meta in [
            None,
            Some(
                serde_json::json!({"kind":"send_message", "agent_id":"stale-recipient", "accepted":true, "message":"not the actual message"}),
            ),
        ] {
            let lines = host
                .tool_card_presented(
                    "send_message",
                    serde_json::json!({"agent_id":"child-123", "message":message}),
                    "message accepted by child-123",
                    false,
                    meta,
                )
                .await
...
async fn default_send_message_renderer_handles_errors_and_metadata_fallback() {
    ...
    for (is_error, meta) in [
        ...
        let lines = host
            .tool_card_presented(
                "send_message",
                serde_json::json!({"agent_id":"child-123", "message":"Please review\nnext file"}),
                "message not delivered: unknown agent",
                is_error,
                meta,
            )
            .await
    ...
    let lines = host
        .tool_card_presented(
            "send_message",
            serde_json::json!({"agent_id":42, "message":false}),
            "",
            true,
            Some(serde_json::json!("invalid")),
        )
...
async fn default_tool_renderers_respect_live_status() {
    ...
    for (name, args) in [
        ...
        (
            "Write",
            serde_json::json!({"path":"src/lib.rs", "content":"// test"}),
        ),
        (
            "send_message",
            serde_json::json!({"agent_id":"child", "message":"Review"}),
        ),
        ...
        let lines = host
            .tool_card_presented(
                ...
                false,
                Some(
                    serde_json::json!({"kind":"tool_live", "status":"running", "duration_ms":1200}),
                ),
            )
            .await
            .unwrap();
        ...
        assert!(
            header.text.contains("running")
                || header
                    .spans
                    .iter()
                    .chain(&header.right)
                    .any(|s| s.text == "running"),
            "{name}"
        );
        assert!(!header
            .spans
...
async fn default_task_renderer_shows_progress_and_states() {
    ...
    host.load(
        "tasks",
        include_str!("../../../flavors/default/plugins/tasks.lua"),
    )
    .await
    .unwrap();
    ...
    assert!(lines[0]
        .right
        .iter()
        .any(|span| span.text == "1 active · 1 pending · 1 done"));
    ...
    assert!(lines
        .iter()
        .any(|line| line.spans.iter().any(|span| span.text == "Active task")));
}
...
async fn default_bash_and_write_renderers_show_command_and_content() {
    ...
    let bash = host
        .tool_card_presented(
            "Bash",
            serde_json::json!({"command":"printf 'hello'", "workdir":"/tmp"}),
            "hello",
            false,
            Some(serde_json::json!({"duration_ms":27})),
        )
        .await
        .unwrap();
    ...
    assert!(bash.iter().any(|line| line
        .block
        .as_ref()
        .is_some_and(|block| block["text"] == "printf 'hello'")));
    assert!(bash.iter().any(|line| line.text == "hello"));

    let write = host
        .tool_card(
            "Write",
            serde_json::json!({"path":"/tmp/test.lua", "content":"return true"}),
            "Wrote 11 bytes",
            false,
        )
        .await
    ...
    assert!(write[0]
        .spans
        .iter()
        .any(|span| span.text == "  /tmp/test.lua"));
    assert!(write.iter().any(|line| line
        .block
        .as_ref()
        .is_some_and(|block| block["text"] == "return true")));
}
...
async fn default_flavor_loads_with_explicit_plugins_and_small_scout() {
    ...
    assert!(
        !config.agents.contains_key("worker"),
    ...
    for name in ["branches", "sessions", "tasks"] {
        assert_eq!(
            apps.iter().find(|a| a.name == name).unwrap().refresh_ms,
            Some(500)
        );
    }
    let reader = host
    ...
    for (name, key, operation) in [
        ...
        assert_eq!(
            host.call_action(name, "promptbox", serde_json::json!({}))
                .await
                .unwrap(),
            vec![operation]
        );
        assert!(host
            ...
            .await
            .iter()
            .any(|binding| binding.keys == vec![key]));
    }
}

#[tokio::test]
async fn explicit_file_lifecycle_remap_disable_reload_and_unload() {
    use rness_lua::loader::{discover_specs, PluginLocation, PluginSpec};
    ...
    let mut spec = PluginSpec {
        dependencies: vec![],
        name: "review".into(),
        source: PluginLocation::File(path.clone()),
        enabled: true,
        watch: false,
        opts: serde_json::json!({"text":"first"}),
        keys: serde_json::json!({"insert":["<F8>","<F9>"]}),
    };
    let host = LuaHost::spawn().unwrap();
    let sources = discover_specs(root.path(), &[spec.clone()]).unwrap();
    assert!(rness_lua::loader::load_all(&host, &sources)
        .await
        .is_empty());
    assert_eq!(host.binding_specs().await[0].keys, vec!["<F8>", "<F9>"]);
    assert_eq!(
        host.call_action("review.insert", "promptbox", serde_json::json!({}))
            .await
            .unwrap(),
        vec![rness_lua::runtime::UiActionOperation::InsertPrompt(
            "first".into()
        )]
    );
    let generation = host
    ...
    assert!(host
        .reload(discover_specs(root.path(), &[spec.clone()]).unwrap())
        .await
        .is_err());
    assert_eq!(
        host.action_generation()
            .load(std::sync::atomic::Ordering::SeqCst),
        generation
    );
    ...
    host.reload(discover_specs(root.path(), &[spec.clone()]).unwrap())
        .await
        .unwrap();
    ...
    assert!(discover_specs(root.path(), &[spec.clone()])
        .unwrap()
        .is_empty());
    spec.enabled = true;
...
async fn watcher_retries_busy_reload_without_another_save() {
    ...
    let registry = Arc::new(ToolRegistry::default());
    let sessions = Arc::new(SessionService::new(
        SessionStore::new(root.path().join("sessions")),
        Arc::new(Silent),
        registry.clone(),
        TurnConfig::default(),
        Arc::new(EventBus::default()),
    ...
    std::fs::write(
        &path,
        "rness.tool.register{name='old', run=function() return 'old' end}",
    )
    .unwrap();
    let specs = vec![rness_lua::loader::PluginSpec {
        dependencies: vec![],
        name: "live".into(),
        source: rness_lua::loader::PluginLocation::File(path.clone()),
        enabled: true,
        watch: true,
    ...
    let _watcher = rness_lua::reload::watch_specs(
        root.path().into(),
        specs,
        host.clone(),
        registry.clone(),
    ...
    std::fs::write(
        &replacement,
        "rness.tool.register{name='new', run=function() return 'new' end}",
    )
    .unwrap();
    ...
    assert_eq!(
        host.call_tool("old", serde_json::json!({})).await.unwrap(),
        "old"
    );
    drop(guard);
    let report = tokio::time::timeout(std::time::Duration::from_secs(5), rx.recv())
        ...
        .unwrap();
    assert!(
        matches!(report, rness_lua::reload::ReloadReport::Reloaded { .. }),
        "{report:?}"
    );
...
async fn references_enable_disable_failed_reload_and_ownership() {
    ...
    host.load(
        "references",
        "rness.file_references.enable { max_results = 3, respect_gitignore = false }",
    )
    .await
    .unwrap();
    assert!(sessions.reference_service().enabled());
    ...
    assert!(host
        .load("bad", "rness.file_references.disable(); error('rollback')")
        .await
        .is_err());
    assert_eq!(sessions.reference_service().generation(), generation);
    assert!(sessions.reference_service().enabled());
    assert!(host
        .reload(vec![rness_lua::loader::PluginSource {
            dependencies: vec![],
            name: "broken".into(),
            source: "rness.file_references.disable(); error('broken')".into()
        }])
        .await
        .is_err());
    assert!(sessions.reference_service().enabled());
    host.load("replacement", "rness.file_references.enable()")
        .await
        .unwrap();
    host.unload_coordinated("references", vec![], |_| {})
        .await
        .unwrap();
    assert!(sessions.reference_service().enabled());
    host.unload_coordinated("replacement", vec![], |_| {})
        .await
        .unwrap();
    assert!(!sessions.reference_service().enabled());
    host.load("on", "rness.file_references.enable()")
        .await
        .unwrap();
    host.load("off", "rness.file_references.disable()")
        .await
        .unwrap();
    assert!(!sessions.reference_service().enabled());
}
...
fn command_example_registers_help_and_arguments() {
    ...
    runtime
        .load(
            "commands",
            include_str!("../../../examples/plugins/commands.lua"),
        )
        .unwrap();
    let result = runtime
        .call_command(
            "project",
            serde_json::json!({"session":"s", "workspace":"/project", "raw_input":" path"}),
        )
        .unwrap();
...
async fn lua_permission_rules_survive_workspace_and_ceiling_snapshots() {
    ...
    for session in ["parent", "child"] {
        let scoped = registry
            .for_workspace(&session.into(), dir.path())
            .restricted(&["guarded".into()]);
        let result = scoped
            .dispatch(&session.into(), &calls, 1, &CancellationToken::new())
            .await;
    ...
    let result = registry
        .dispatch(&"parent".into(), &calls, 1, &CancellationToken::new())
        .await;
    assert!(!result[0].is_error);
...
async fn native_and_lua_text_providers_share_the_contract() {
    ...
    assert_eq!(
        providers[0].text().await.as_deref(),
        Some("shared contract")
    );
    assert_eq!(providers[1].text().await, None);
    host.load(
        "status",
        "rness.ui.statusline(function() return 'shared contract' end)",
    )
    .await
    .unwrap();
...
async fn runtime_reload_preserves_init_and_rolls_back_commands_and_hooks() {
    ...
    std::fs::write(
        &init,
        "boots = (boots or 0) + 1; rness.ui.statusline(function() return tostring(boots) end)",
    )
    .unwrap();
    ...
    let source = |version: &str| {
        rness_lua::loader::PluginSource { dependencies: vec![], name: "live".into(), source: format!("rness.commands.register{{name='live', description='{version}', arguments={{'{version}'}}, run=function() return '{version}' end}}; rness.hook.on('tick', function() ticks=(ticks or 0)+1 end)") }
    };
    ...
    assert_eq!(
        host.call_tool("widget", serde_json::json!({}))
            .await
            .unwrap(),
        "new"
    );
    assert_eq!(
        host.app_view("widget", serde_json::json!({}))
            .await
            .unwrap(),
        vec!["new"]
    );
    assert!(sessions
        ...
        .completions()
        .iter()
        .any(|(name, _)| name == "live v2"));
    assert!(!sessions
        .commands()
        .completions()
        .iter()
        .any(|(name, _)| name == "live v1"));
    let mut broken = source("broken");
    broken.source.push_str("; error('broken')");
    ...
    host.load("check", "assert(boots == 1); assert(ticks == 1)")
        .await
        .unwrap();
...
async fn coordinated_unload_holds_engine_reservation_through_ui_update() {
    ...
    let guard = sessions.try_extension_maintenance().unwrap();
    assert!(host
        .unload_coordinated("owned", installed.clone(), |_| panic!("busy teardown"))
        .await
        .is_err());
    assert!(registry.get("owned").is_some());
    assert_eq!(
        host.call_tool("owned", serde_json::json!({}))
            .await
            .unwrap(),
        "yes"
    );
    ...
    assert!(host
        .unload_coordinated("owned", installed, move |snapshot| {
    ...
    assert!(!host
        .unload_coordinated("owned", vec![], |_| panic!("already removed"))
        .await
        .unwrap());
}
...
async fn agents_command_lists_completes_and_stops_while_parent_runs() {
    ...
    let runtime = Arc::new(rness_engine::subagent::SubagentRuntime::new(
        sessions.clone(),
        3,
    ));
    let host = LuaHost::spawn().unwrap();
    ...
    host.load(
        "ordinary",
        "rness.commands.register{name='ordinary',run=function() return 'ok' end}",
    )
    .await
    .unwrap();
    ...
    let prepared = sessions
        .prepare_command(&parent, "/agents stop ")
        .unwrap()
        .unwrap();
    let service = sessions.clone();
    let choices = tokio::task::spawn_blocking(move || prepared.complete(&service))
        .await
        .unwrap()
        .unwrap();
    ...
    async fn command(
        ...
        parent: &str,
        text: String,
    ) -> Result<Disposition, rness_engine::service::ServiceError> {
        ...
        tokio::time::timeout(std::time::Duration::from_secs(2), async {
            loop {
                let result = sessions
                    ...
                    .await;
                if !matches!(result, Err(rness_engine::service::ServiceError::Busy)) {
                    break result;
                }
    ...
    let result = command(&sessions, &parent, "/agents stop".into())
        .await
        .unwrap();
    assert!(
        matches!(result, Disposition::Command(result) if result.message.contains("a1  depth=1") && result.message.contains("a2  depth=2"))
    );
    for id in [&parent, &unrelated, &"unknown".to_string()] {
        assert!(command(&sessions, &parent, format!("/agents stop {id}"))
            .await
            .is_err());
    }
    assert!(
        command(&sessions, &parent, "/agents stop extra args".into())
            .await
            .is_err()
    );
    assert!(command(&sessions, &parent, "/agents a2 steer".into())
        .await
        .is_err());
    assert!(
        command(&sessions, &parent, "/agents a2 steer redirect work".into())
            .await
            .is_err()
    );
    let result = command(&sessions, &parent, "/agents a1 steer redirect work".into())
        .await
        .unwrap();
    assert!(
        matches!(result, Disposition::Command(result) if result.message.contains("User steering sent"))
    );
    let result = command(&sessions, &parent, "/agents a2".into())
        .await
        .unwrap();
    assert!(matches!(result, Disposition::Command(result)
        if result.data == serde_json::json!({"action":"agents:open", "session":parent, "agent":grandchild})));
    command(&sessions, &parent, format!("/agents stop {child}"))
        .await
        .unwrap();
    assert_eq!(sessions.phase(&child), Phase::Running);
    command(&sessions, &parent, format!("/agents {child} stop confirm"))
        .await
        .unwrap();
    tokio::time::timeout(std::time::Duration::from_secs(2), sessions.join(&child))
        .await
        .unwrap();
    ...
    command(
        &sessions,
        &parent,
        format!("/agents {grandchild} stop confirm"),
    )
    .await
    .unwrap();
    tokio::time::timeout(
        std::time::Duration::from_secs(2),
        sessions.join(&grandchild),
    )
    .await
    .unwrap();
    let result = command(&sessions, &parent, "/agents stop".into())
        ...
        .unwrap();
    assert!(
        matches!(result, Disposition::Command(result) if result.message == "No running subagents.")
    );
    let prepared = sessions
        .prepare_command(&parent, "/agents stop ")
        .unwrap()
    ...
    assert!(!choices.contains(&"stop a1".into()));
    assert!(!choices.contains(&"stop a2".into()));
    for id in [&parent, &unrelated] {
        sessions.cancel(id);
        sessions.join(id).await;
    }
    assert!(sessions
        .prepare_command(&parent, "/ordinary")
        .unwrap()
        .is_some());
}
...
async fn booted_host(dir: &std::path::Path) -> LuaHost {
    ...
    .await
    .unwrap();
    host.install_questions(Arc::new(rness_engine::questions::Questions::default()))
        .await
        .unwrap();
    host
}
...
async fn region_confirmation_reuses_only_its_own_command_reservation() {
    ...
    "#).await.unwrap();
    let prepared = sessions
        .prepare_command(&session, "/region")
        .unwrap()
        .unwrap();
    ...
    assert!(sessions
        .compact_region(&session, 0, 1, sources, policy)
        .await
        .is_err());
    let service = sessions.clone();
    let outcome = tokio::task::spawn_blocking(move || prepared.execute(&service))
        .await
        .unwrap()
        .unwrap();
    assert!(
        matches!(outcome, rness_engine::inbox::Disposition::Command(result) if result.message == "unchanged")
    );
    ...
    assert!(sessions
        ...
        .unwrap()
        .iter()
        .any(|e| matches!(e.event, SessionEvent::CompactionStarted { .. })));
}
...
async fn usage_reports_latest_request_and_durable_turn_count() {
    ...
    for turn in 1..=3 {
        ...
        log.append(&SessionEvent::AssistantMessage(AssistantMessage {
            ...
            stop: StopReason::EndTurn,
            usage: Usage {
                input_tokens: u64::from(turn) * 100,
                output_tokens: 10,
                ..Default::default()
            },
            estimated_input: 0,
...
async fn plan_review_lua_configuration_is_validated_and_normalized() {
    ...
    for options in [
        ...
        assert!(
            host.load(
                "bad-plan",
                &format!("rness.plan.enable {{ review = {options} }}")
            )
            .await
            .is_err(),
            "{options}"
...
async fn plan_lifecycle_uses_live_questions_and_preserves_state() {
    ...
    assert!(host
        .load("broken", "rness.plan.disable(); error('fail')")
        .await
        .is_err());
    ...
    assert!(!host
        ...
        .iter()
        .any(|s| s.name == "exit_plan_mode"));
    host.load("replacement", "rness.plan.enable()")
        .await
        .unwrap();
    host.unload_coordinated("plan", vec![], |_| {})
        .await
        .unwrap();
    assert!(host
        .tool_specs()
...
fn examples() -> Vec<rness_lua::loader::PluginSource> {
    ...
    let specs: Vec<_> = names
        .into_iter()
        .map(|n| rness_lua::loader::PluginSpec {
            dependencies: if n == "plan.lua" {
                vec!["questions".into()]
            } else {
                vec![]
            },
            name: n.trim_end_matches(".lua").to_string(),
            source: rness_lua::loader::PluginLocation::File(
                std::path::Path::new(root).join("plugins").join(n),
            ),
            enabled: true,
            watch: false,
            opts: serde_json::json!({}),
...
fn specialist_roles_load_explicitly_with_bounded_tool_permissions() {
    ...
    std::fs::write(
        root.path().join("lua/roles.lua"),
        include_str!("../../../examples/lua/roles.lua"),
    )
    .unwrap();
    ...
    for (name, agent) in &config.agents {
        ...
        assert!(
            ...
            "examples inherit settings without invented models"
        );
...
async fn keymaps_example_supports_options_remapping_disable_and_unload() {
    let root = tempfile::tempdir().unwrap();
    let path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../examples/plugins/keymaps.lua");
    for (keys, expected) in [
        (serde_json::json!({}), vec!["<F6>"]),
        (
            serde_json::json!({"insert_review": ["<F8>", "<F9>"]}),
            vec!["<F8>", "<F9>"],
        ),
        ...
        assert_eq!(
            host.call_action("keymaps.insert_review", "promptbox", serde_json::json!({}))
                .await
                .unwrap(),
            vec![rness_lua::runtime::UiActionOperation::InsertPrompt(
                "Custom review".into()
...
async fn task_wrapping_preserves_long_tokens_and_unicode() {
    ...
    host.load("tasks", include_str!("../../../examples/plugins/tasks.lua"))
        .await
        .unwrap();
...
async fn task_overlay_scrolls_and_isolates_sessions() {
    ...
    assert_eq!(
        host.app_view("tasks", serde_json::json!({"session":"b","rows":4}))
            .await
            .unwrap()[1],
        "No saved tasks."
    );
...
async fn example_plugins_load_and_register_five_apps() {
    ...
    assert_eq!(
        names,
        vec!["branches", "compact-region", "sessions", "tasks", "tree"]
    );
    for name in ["branches", "sessions", "tasks"] {
        assert_eq!(
            apps.iter().find(|a| a.name == name).unwrap().refresh_ms,
    ...
    assert_eq!(
        apps.iter().find(|a| a.name == "tree").unwrap().refresh_ms,
        None
    );
...
async fn text_tool_and_bottomline_examples_work() {
    ...
    for name in ["text-tools", "bottomline", "session-log"] {
        host.load(
            name,
            &std::fs::read_to_string(root.join(format!("{name}.lua"))).unwrap(),
        )
        .await
        .unwrap();
    }
    assert_eq!(
        ...
        .await
        .unwrap(),
        "bytes=16 words=3 lines=2"
    );
    assert!(host
        .call_tool("text_stats", serde_json::json!({"text": 1}))
        .await
        .is_err());
    assert_eq!(
        host.statusline().await.as_deref(),
        Some("rness | 0 running")
    );
    host.fire_hook("turn_start", serde_json::json!({"session":"example"}));
    assert_eq!(
        host.statusline().await.as_deref(),
        Some("rness | 1 running | last session example")
    );
    host.fire_hook("turn_end", serde_json::json!({"session":"example"}));
    assert_eq!(
        host.statusline().await.as_deref(),
        Some("rness | 0 running | last session example")
    );
}
...
async fn tree_view_renders_and_navigates() {
    ...
    let host = booted_host(dir.path()).await;
    assert!(rness_lua::loader::load_all(&host, &examples())
        .await
        .is_empty());
