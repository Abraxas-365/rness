...
async fn opt_in_query_yields_while_sqlite_is_locked() {
    ...
    assert!(host
        .load(
            "tools-missing-provider",
            include_str!("../../../examples/plugins/session-search.lua")
        )
        .await
        .is_err());
    ...
    assert!(host
        .load(
            "failed-provider",
            &(include_str!("../../../examples/plugins/session-search-sqlite.lua").to_string()
                + "\nerror('failed')")
        )
        .await
        .is_err());
    host.load(
        "provider",
        include_str!("../../../examples/plugins/session-search-sqlite.lua"),
    )
    .await
    .unwrap();
    assert!(host.tool_specs().await.is_empty());
    ...
    let tools = tokio::time::timeout(Duration::from_secs(1), host.tool_specs())
        .await
        .expect("Lua VM remains responsive");
    assert_eq!(tools.len(), 5);
    lock.execute_batch("ROLLBACK;").unwrap();
    assert!(tokio::time::timeout(Duration::from_secs(10), query)
        .await
        .unwrap()
        .unwrap()
        .unwrap()
