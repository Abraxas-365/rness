...
use rness_kernel::{presentation::TextProvider, EventBus};
...
async fn exercise(fail: bool, cancel: bool, unchanged: bool) {
    ...
    log.append(&SessionEvent::UserMessage(UserMessage {
        ...
        content: vec![ContentPart::Text {
            text: "source context ".repeat(1000),
        }],
        source: None,
    }))
    .unwrap();
    drop(log);
    let provider = Arc::new(GatedSummary {
        started: Notify::new(),
        release: Notify::new(),
        fail,
        unchanged,
    ...
    let unsubscribe = bus.on::<FrameEv>(move |frame| {
        hook_host.fire_hook("frame", serde_json::to_value(frame).unwrap())
    });
    host.load(
        "policy",
        r#"
    ...
    host.load(
        "commands",
        include_str!("../../../flavors/default/plugins/commands.lua"),
    )
    .await
    .unwrap();
    host.load(
    ...
    assert!(
        sessions.prepare_command(&session, "/compact").is_err(),
        "reservation retained across yield"
    );
    assert!(
        sessions.try_extension_maintenance().is_err(),
        "reload cannot invalidate suspended coroutine"
    );
    assert!(
        tokio::time::timeout(Duration::from_secs(2), host.reload(vec![]))
            .await
            .unwrap()
            .is_err()
    );
    assert!(
        tokio::time::timeout(Duration::from_secs(2), host.load("replacement", ""))
            .await
            .unwrap()
            .is_err()
    );
    assert!(
        tokio::time::timeout(Duration::from_secs(2), host.unload("commands"))
            .await
            .unwrap()
            .is_err()
    );
    ...
    .unwrap()
    .unwrap();
    let Disposition::Command(result) = result else {
        panic!("expected independent command")
    };
    assert_eq!(result.message, other);
    let context = json!({"session":session,"model":"test"});
    for frame in ["⠋", "⠙", "⠹"] {
        let view = tokio::time::timeout(Duration::from_secs(2), host.status(context.clone()))
            .await
            .expect("Lua responds before summarizer completes")
            .unwrap()
            .to_string();
        assert!(
            view.contains(&format!("{frame} compacting context")),
            "{view}"
        );
    ...
    if cancel {
        ...
        let Disposition::Command(result) = result.unwrap() else {
            panic!("expected command result")
        };
        assert_eq!(
            result.message,
            if unchanged || fail {
                "No smaller summary produced; history unchanged."
            } else {
                "Context compacted."
            }
        );
    ...
    assert!(sessions
        .prepare_command(&session, "/compact")
        .unwrap()
        .is_some());
    unsubscribe();
}
...
impl Drop for CancelOnDrop {
    fn drop(&mut self) {
        self.sessions.cancel(&self.session);
    }
}

#[tokio::test(flavor = "multi_thread")]
async fn compaction_keeps_lua_responsive_until_success() {
    exercise(false, false, false).await;
}
#[tokio::test(flavor = "multi_thread")]
async fn compaction_keeps_lua_responsive_until_unchanged() {
    exercise(false, false, true).await;
}
#[tokio::test(flavor = "multi_thread")]
async fn compaction_keeps_lua_responsive_until_failure() {
    exercise(true, false, false).await;
}
#[tokio::test(flavor = "multi_thread")]
async fn compaction_keeps_lua_responsive_until_cancellation() {
