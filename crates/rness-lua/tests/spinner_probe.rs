...
async fn default_statusline_shows_session_compaction_and_restores_activity() {
    ...
    host.fire_hook(
        "frame",
        json!({"type":"delta","session":"s1","chunk":{"d":"thinking"}}),
    );
    ...
    for frame in ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠋"] {
        ...
        assert!(
            during.contains(&format!("{frame} compacting context")),
            "{during}"
        );
    }
    host.fire_hook(
        "frame",
        json!({"type":"compaction_finished","session":"s1","changed":false}),
    );
    ...
    host.fire_hook(
        "frame",
        json!({"type":"compaction_finished","session":"s1","changed":true}),
    );
    assert!(host
        .status(context.clone())
        .await
...
async fn statusline_pattern_works() {
    ...
    .unwrap();
    assert_eq!(
        host.statusline().await,
        None,
        "idle: nil lets builtin render"
    );
    host.fire_hook(
        "turn_start",
        serde_json::json!({"session": "s1", "turn": 1}),
    );
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    assert_eq!(host.statusline().await, Some("BUSY".into()), "busy: text");
    host.fire_hook(
        "turn_end",
        serde_json::json!({"session": "s1", "turn": 1, "outcome": "Completed"}),
    );
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;
