...
async fn runaway_renderers_fall_back_and_release_vm_limits() {
    ...
    for name in ["loop", "allocate"] {
        assert!(host.tool_card(name, serde_json::json!({}), "", false).await.is_none());
        assert_eq!(host.tool_card("healthy", serde_json::json!({}), "", false).await.unwrap()[0].text, "ok");
    }
}
...
async fn presentation_reaches_diff_cards_and_old_results_remain_supported() {
    ...
    let rows = rness_engine::presentation::ToolCards::tool_card_presented(
        &host, "Edit", serde_json::json!({}), "unchanged model output", false, Some(metadata),
    ).await.unwrap();
    ...
    let old = host.tool_card("Edit", serde_json::json!({}), "legacy", false).await.unwrap();
...
async fn malformed_structured_fields_fall_back() {
    ...
    for name in ["badtext","badspan","baddiff","badnumber"] {
        assert!(host.tool_card(name, serde_json::json!({}), "", false).await.is_none(), "{name}");
    }
}
...
async fn headerless_structured_cards_split_multispan_lines() {
    ...
    "#).await.unwrap();
    let rows = host.tool_card("test", serde_json::json!({}), "", false).await.unwrap();
    let texts: Vec<String> = rows.iter().map(|r| r.spans.iter().map(|s| s.text.as_str()).collect()).collect();
...
async fn oversized_and_recursive_cards_fall_back() {
    ...
    for name in ["large", "recursive", "deep"] {
        assert!(host.tool_card(name, serde_json::json!({}), "ok", false).await.is_none());
    }
}
...
async fn structured_cards_preserve_header_spans_and_direct_colors() {
    ...
    "##).await.unwrap();
    let rows = host.tool_card("Bash", serde_json::json!({}), "hello", false).await.unwrap();
...
async fn tool_card_renders_declines_and_survives_errors() {
    ...
    assert_eq!(
        lines,
        vec![
            StyledLine { text: "file a.rs".into(), style: "title".into(), ..Default::default() },
            StyledLine { text: "plain shorthand".into(), style: "".into(), ..Default::default() },
            StyledLine { text: "+x".into(), style: "added".into(), ..Default::default() },
        ]
    );
