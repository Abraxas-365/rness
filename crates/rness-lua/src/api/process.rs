...
pub fn install(lua: &Lua, rness: &Table) -> mlua::Result<()> {
    ...
    process.set("run", lua.create_function(|lua, spec: Table| {
        let spec: Spec = lua.from_value(mlua::Value::Table(spec))?;
        if spec.program.is_empty() || spec.timeout_ms == Some(0) {
            return Err(mlua::Error::runtime("program must be nonempty and timeout_ms must be positive"));
        }
        let cancel = lua.app_data_ref::<tokio_util::sync::CancellationToken>()
            .map(|token| token.clone()).unwrap_or_default();
        #[cfg(unix)]
        {
        ...
        {
            let _ = (spec, cancel);
            Err::<mlua::Value, _>(mlua::Error::runtime("rness.process.run requires Unix process-group support on this platform"))
        }
    })?)?;
...
#[cfg(unix)]
fn run(spec: Spec, cancel: tokio_util::sync::CancellationToken) -> std::io::Result<serde_json::Value> {
    ...
    if cancel.is_cancelled() { return Err(std::io::Error::other("command cancelled")); }
    let mut command = Command::new(&spec.program);
    command.args(&spec.args).process_group(0)
        .stdin(Stdio::null()).stdout(stdout.try_clone()?).stderr(stderr.try_clone()?);
    if let Some(cwd) = spec.cwd { command.current_dir(cwd); }
    let child = command.spawn()?;
    let mut process = Group { child, reaped: false };
    let deadline = Instant::now().checked_add(Duration::from_millis(spec.timeout_ms.unwrap_or(30_000)))
    ...
    let (status, reason) = runtime.block_on(async {
        loop {
            let reason = if cancel.is_cancelled() { Some("command cancelled") }
                else if Instant::now() >= deadline { Some("process timed out") }
                else if stdout.metadata()?.len() > LIMIT || stderr.metadata()?.len() > LIMIT { Some("process output limit exceeded") }
                else { None };
    ...
    if let Some(reason) = reason { return Err(std::io::Error::other(reason)); }
    let mut out = Vec::new();
    let mut err = Vec::new();
...
impl Group {
    fn exited(&self) -> std::io::Result<bool> {
        ...
        let result = unsafe { libc::waitid(libc::P_PID, self.child.id() as libc::id_t, &mut info, libc::WEXITED | libc::WNOHANG | libc::WNOWAIT) };
...
mod tests {
    ...
    fn spec(program: &str, args: &[&str]) -> Spec {
        Spec { program: program.into(), args: args.iter().map(|s| s.to_string()).collect(), cwd: None, timeout_ms: Some(2000) }
    }

    #[test]
    fn captures_exit_arguments_and_cwd_without_shell_interpolation() {
        let result = run(spec("/usr/bin/printf", &["%s", "$(touch should-not-exist); space"]), CancellationToken::new()).unwrap();
        ...
        assert_eq!(result["stdout"].as_str().unwrap().trim(), dir.path().canonicalize().unwrap().to_str().unwrap());
        let result = run(spec("/bin/sh", &["-c", "printf error >&2; exit 7"]), CancellationToken::new()).unwrap();
    ...
    fn timeout_and_early_cancellation() {
        ...
        assert!(run(input, CancellationToken::new()).unwrap_err().to_string().contains("timed out"));
        let cancel = CancellationToken::new();
        cancel.cancel();
        assert!(run(spec("missing-program", &[]), cancel).unwrap_err().to_string().contains("cancelled"));
    }
    ...
    fn cancellation_kills_group_before_returning() {
        ...
        let worker = std::thread::spawn(move || run(spec("/bin/sh", &["-c", "sleep 100 & echo $! > \"$1\"; wait", "sh", path.to_str().unwrap()]), token));
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(2);
        while !marker.exists() && std::time::Instant::now() < deadline { std::thread::yield_now(); }
        cancel.cancel();
        assert!(worker.join().unwrap().unwrap_err().to_string().contains("cancelled"));
