...
async fn handler(headers: HeaderMap, Json(message): Json<Value>) -> Response {
    if headers.get("authorization").and_then(|h| h.to_str().ok()) != Some("Bearer test-secret") {
...
async fn authenticated_json_and_sse_roundtrip() {
    ...
    let server = tokio::spawn(async move {
        axum::serve(
            ...
            axum::Router::new().route(
                "/mcp",
                axum::routing::post(handler).delete(|| async { StatusCode::NO_CONTENT }),
            ),
        )
        .await
        .unwrap();
    ...
    assert!(matches!(
        conn.request("rpc-error", json!({})).await,
        Err(rness_mcp::McpError::Rpc { .. })
    ));
...
async fn remote_plaintext_and_reserved_headers_are_rejected() {
    for (url, headers) in [
        ("http://example.com/mcp", vec![]),
        (
            "https://example.com/mcp",
            vec![("Host".into(), "evil".into())],
        ),
    ] {
        assert!(McpConnection::connect_http(HttpServer {
            name: "remote".into(),
            url: url.into(),
            headers,
