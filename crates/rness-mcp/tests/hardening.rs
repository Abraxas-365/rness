...
"#;
fn spec() -> StdioServer {
    StdioServer { name: "test".into(), command: "python3".into(), args: vec!["-u".into(), "-c".into(), SERVER.into()], env: vec![("EXPLICIT_KEY".into(), "chosen".into())], timeout: Duration::from_secs(2) }
}
#[tokio::test]
async fn environment_resync_eof_and_explicit_reconnect() {
    ...
    tokio::time::timeout(Duration::from_secs(3), async {
        while registry.get("mcp__test__echo1").is_none() { tokio::time::sleep(Duration::from_millis(10)).await; }
    }).await.unwrap();
    ...
    tokio::time::timeout(Duration::from_secs(3), async {
        while !registry.names().is_empty() { tokio::time::sleep(Duration::from_millis(10)).await; }
    }).await.unwrap();
...
async fn concurrent_catalog_refresh_and_disconnect_clear_deferred_state() {
    ...
    let (refresh, ()) = tokio::join!(biased; conn.bridge_tools(&registry), conn.disconnect(&registry));
    refresh.unwrap();
...
async fn incomplete_writes_close_transport_on_timeout_or_cancellation() {
    for cancel in [false, true] {
        ...
        let task = tokio::spawn(async move { writer.request("blocked", json!({"data":"x".repeat(4 * 1024 * 1024)})).await });
        if cancel {
