...
impl SsePolicy {
    pub fn validate(&self) -> Result<(), String> {
        if self.max_attempts > 100
            || !(1..=60_000).contains(&self.retry_delay_ms)
...
impl Decoder {
    fn byte(&mut self, byte: u8) -> Result<Option<Event>, &'static str> {
        ...
        self.size += 1;
        if self.size > MAX_FRAME_BYTES {
            return Err("MCP SSE event exceeds 16 MiB");
        }
        if byte != b'\r' && byte != b'\n' {
            self.line.push(byte);
            return Ok(None);
        ...
        if field == b"id" && !value.contains(&0) {
            ...
            if id.len() > 1024 || HeaderValue::from_str(id).is_err() {
                return Err("invalid MCP SSE event ID");
            }
...
impl HttpTransport {
    pub fn new(spec: HttpServer) -> Result<Arc<Self>, McpError> {
        let error = |message: &str| McpError::Protocol {
        ...
        for (key, value) in &spec.headers {
            ...
            headers.insert(
                name,
                HeaderValue::from_str(value).map_err(|_| error("invalid MCP header value"))?,
            );
        }
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|_| error("MCP HTTP client setup failed"))?;
        Ok(Arc::new(Self {
            spec,
            client,
    ...
    fn request(&self, method: reqwest::Method) -> reqwest::RequestBuilder {
        let accept = if method == reqwest::Method::GET {
            "text/event-stream"
        } else {
        ...
        if let Some(version) = &*self.version.lock().unwrap() {
    ...
    pub fn set_version(&self, version: &str) -> Result<(), McpError> {
        if !["2024-11-05", "2025-03-26", "2025-06-18"].contains(&version) {
            return Err(self.error("unsupported MCP protocol version"));
        }
    ...
    async fn server_message(
        ...
        if msg["jsonrpc"] != "2.0" {
            return Err(self.error("invalid JSON-RPC version"));
        }
        let Some(method) = msg.get("method") else {
            return Ok(false);
        };
        if !method.is_string() {
            return Err(self.error("invalid MCP method"));
        }
        if let Some(id) = msg.get("id") {
            ...
            let accepted = self
                ...
                .send()
                .await
                .map_err(|_| self.error("MCP server-request reply failed"))?;
            if accepted.status() != reqwest::StatusCode::ACCEPTED {
                return Err(self.error("MCP server-request reply expected HTTP 202"));
            }
    ...
    async fn consume(
        ...
        if content_type != "text/event-stream" {
            return Err(self.error("expected MCP event stream"));
        }
        ...
        loop {
            let chunk = match tokio::time::timeout(
                Duration::from_millis(self.spec.sse.idle_timeout_ms),
                stream.next(),
            )
            .await
            {
            ...
            for byte in chunk {
                let Some(event) = decoder.byte(byte).map_err(|e| self.error(e))? else {
                    continue;
                };
                if event.id.is_some() && event.id == previous {
                    continue;
                }
                if !event.data.is_empty() {
                    let msg: Value = serde_json::from_slice(&event.data)
                        .map_err(|_| self.error("invalid MCP SSE JSON"))?;
                    if !self.server_message(&msg, changed).await? {
                        if expected.is_none() || msg.get("id") != expected {
                            return Err(self.error("MCP stream response ID mismatch"));
                        }
                        return self.result(msg).map(Some);
                    }
                }
                if let Some(id) = event.id {
                    *cursor = if id.is_empty() { None } else { Some(id) };
                }
            }
        }
    }
    async fn get(&self, cursor: Option<&str>) -> Result<reqwest::Response, McpError> {
        ...
        tokio::time::timeout(self.spec.timeout, request.send())
            .await
        ...
        loop {
            match self.get(cursor.as_deref()).await {
                ...
                Ok(response) if response.status().is_success() => {
                    self.consume(response, None, changed, &mut cursor).await?;
                }
                Ok(response) if !response.status().is_server_error() => {
                    return Err(self.error(format!("MCP GET status {}", response.status().as_u16())))
                }
                _ => {}
            }
            if !self.spec.sse.resume || attempts >= self.spec.sse.max_attempts {
                return Err(
                    self.error("MCP notification stream ended; recovery disabled or exhausted")
                );
            }
            attempts += 1;
    ...
    pub async fn post(
        ...
        if body.len() > MAX_FRAME_BYTES {
            return Err(self.error("outgoing MCP frame exceeds 16 MiB"));
        }
        let response = self
            .request(reqwest::Method::POST)
            .timeout(self.spec.timeout)
            ...
            .await
            .map_err(|_| {
                self.error("MCP HTTP request failed (outcome may be unknown; not replayed)")
            })?;
        if !response.status().is_success() {
            return Err(self.error(format!("MCP HTTP status {}", response.status().as_u16())));
        }
        if payload["method"] == "initialize" {
            if let Some(session) = response.headers().get("mcp-session-id") {
                let session = session
                    .to_str()
                    .map_err(|_| self.error("invalid MCP session ID"))?;
                if session.is_empty()
                    || session.len() > 1024
                    || !session.bytes().all(|b| b.is_ascii_graphic())
                {
                    return Err(self.error("invalid MCP session ID"));
                }
                *self.session.lock().unwrap() = Some(session.into());
            }
        }
        if payload.get("id").is_none() {
            return if response.status() == reqwest::StatusCode::ACCEPTED {
                Ok(None)
            } else {
                Err(self.error("MCP notification expected HTTP 202"))
            };
        }
        let content_type = response
            .headers()
        ...
        if content_type == "text/event-stream" {
            ...
            if let Some(result) = self
                .consume(response, payload.get("id"), changed, &mut cursor)
                .await?
            {
            ...
            if self.spec.sse.resume {
                for _ in 0..self.spec.sse.max_attempts {
                    let Some(id) = cursor.as_deref() else {
                        break;
                    };
                    tokio::time::sleep(Duration::from_millis(self.spec.sse.retry_delay_ms)).await;
                    let response = match self.get(Some(id)).await {
                    ...
                    if !response.status().is_success() {
                        return Err(
                            self.error(format!("MCP resume status {}", response.status().as_u16()))
                        );
                    }
                    if let Some(result) = self
                        .consume(response, payload.get("id"), changed, &mut cursor)
        ...
        if content_type != "application/json" {
            return Err(self.error("unsupported MCP response content type"));
        }
        let mut stream = response.bytes_stream();
        ...
        while let Some(chunk) = stream.next().await {
            ...
            if buffer.len().saturating_add(chunk.len()) > MAX_FRAME_BYTES {
                return Err(self.error("MCP response exceeds 16 MiB"));
            }
            buffer.extend_from_slice(&chunk);
        }
        let msg: Value =
            serde_json::from_slice(&buffer).map_err(|_| self.error("invalid MCP response JSON"))?;
        if msg.get("id") != payload.get("id") {
            return Err(self.error("MCP response ID mismatch"));
        }
        ...
        if msg.get("error").is_some() {
            return Err(McpError::Rpc {
                ...
                message: msg["error"]["message"]
                    .as_str()
                    .unwrap_or("MCP remote error")
                    .into(),
            });
        }
        msg.get("result")
            .cloned()
            .ok_or_else(|| self.error("MCP response missing result"))
    }
