...
impl Auth {
    pub fn for_bind(address: std::net::SocketAddr, token: Option<String>) -> Result<Self, String> {
        if token.as_ref().is_some_and(|token| token.len() < 32 || !token.bytes().all(|b| b.is_ascii_graphic())) {
            return Err("RNESS_SERVER_TOKEN must contain at least 32 printable non-space ASCII characters".into());
        }
...
pub async fn authorize(State(auth): State<Auth>, request: Request, next: Next) -> Response {
    ...
    if request.headers().contains_key("origin") || request.headers().get("sec-fetch-site").is_some_and(|value| value != "none") {
        return StatusCode::FORBIDDEN.into_response();
    }
    if auth.0.is_none() {
        let local_authority = |authority: &axum::http::uri::Authority| {
            let host = authority.host();
            host.eq_ignore_ascii_case("localhost") || host.trim_start_matches('[').trim_end_matches(']')
                .parse::<std::net::IpAddr>().is_ok_and(|ip| ip.is_loopback())
        };
        let hosts: Vec<_> = request.headers().get_all("host").iter().collect();
        let valid_host = hosts.len() == 1 && hosts[0].to_str().ok()
            .and_then(|host| host.parse::<axum::http::uri::Authority>().ok())
            .is_some_and(|host| local_authority(&host));
        if !valid_host || request.uri().authority().is_some_and(|authority| !local_authority(authority)) {
            return StatusCode::FORBIDDEN.into_response();
        }
    }
    if let Some(expected) = &auth.0 {
        let supplied = request.headers().get("authorization").and_then(|v| v.to_str().ok()).and_then(|v| v.strip_prefix("Bearer "));
        ...
        if !valid { return (StatusCode::UNAUTHORIZED, [("www-authenticate", "Bearer")]).into_response(); }
    }
