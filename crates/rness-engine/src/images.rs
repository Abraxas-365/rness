...
mod tests {
    ...
    fn ownership_survives_reopen_and_cleanup_is_bounded() {
        ...
        for i in 0..20 {
            ...
            store
                .record_owned_upload(&scope, &id, u64::MAX - i)
                .unwrap();
            let file = std::fs::File::options()
                .write(true)
                .open(store.root.join("owned-uploads").join(&scope).join(id))
                .unwrap();
            file.set_times(
    ...
    fn upload_cache_survives_reopen_and_checks_expiration() {
        ...
        let store = ImageStore::new(dir.path().into(), Default::default()).unwrap();
        assert_eq!(
            store.cached_upload(&key, 100).unwrap(),
            Some(("file_test".into(), 500))
        );
    ...
    fn custom_processor_is_cached_versioned_and_validated() {
        ...
        store
            ...
            .unwrap();
        for _ in 0..2 {
            store.request_image(&reference, store.policy()).unwrap();
        }
        assert_eq!(calls.load(std::sync::atomic::Ordering::SeqCst), 1);
        store
            .set_processor(Some((
                "v2".into(),
                std::sync::Arc::new(|_, _| Ok(b"invalid".to_vec())),
            )))
            .unwrap();
    ...
    fn icc_conversion_preserves_alpha_and_removes_profile() {
        ...
        encoder
            ...
            .unwrap();
        encoder
            .write_image(&[200, 100, 50, 127], 1, 1, image::ExtendedColorType::Rgba8)
            .unwrap();
        let reference = store.admit(&bytes, "image/png").unwrap();
        let (_, converted) = store
            .request_image(&reference, &ImagePolicy::default())
            .unwrap();
        let pixel = image::load_from_memory(&converted)
            .unwrap()
            .to_rgba8()
            .get_pixel(0, 0)
            .0;
        ...
        assert!(image::codecs::png::PngDecoder::new(Cursor::new(&converted))
            .unwrap()
            .icc_profile()
            .unwrap()
            .is_none());
    ...
    fn runtime_policy_is_shared_validated_and_resettable() {
        ...
        assert_eq!(
            clone
                .effective_request_policy(store.policy())
                .max_request_images,
            2
        );
        assert!(store
            .set_request_policy(Some(ImagePolicy {
    ...
    fn request_budgets_remove_oldest_occurrences_without_mutating_history() {
        ...
        assert!(
            matches!(&projected.turns[0], ModelTurn::User { content } if matches!(&content[0], ContentPart::Text { text } if text.contains("omitted")))
        );
        assert!(
            matches!(&context.turns[0], ModelTurn::User { content } if matches!(&content[0], ContentPart::Image { .. }))
        );
        ...
        let projected = store
            ...
            .unwrap();
        let ModelTurn::ToolResults { results } = &projected.turns[1] else {
...
impl ImageStore {
    pub fn set_processor(
        &self,
        processor: Option<(String, std::sync::Arc<ImageProcessor>)>,
    ) -> Result<(), String> {
        if processor
            .as_ref()
            .is_some_and(|(version, _)| version.is_empty())
        {
            return Err("image processor version must not be empty".into());
        }
    ...
    pub fn set_request_policy(&self, policy: Option<ImagePolicy>) -> Result<(), String> {
        if let Some(policy) = &policy {
            policy.validate()?;
            if policy.max_input_bytes != self.policy.max_input_bytes
                || policy.max_input_pixels != self.policy.max_input_pixels
    ...
    fn decode(&self, data: &[u8]) -> Result<(image::DynamicImage, ImageFormat), String> {
        ...
        if width == 0
            || height == 0
            || u64::from(width) * u64::from(height) > self.policy.max_input_pixels
        {
        ...
        let orientation =
            image::ImageDecoder::orientation(&mut decoder).map_err(|e| e.to_string())?;
    ...
    pub fn cached_upload(&self, key: &str, now: u64) -> Result<Option<(String, u64)>, String> {
        ...
        {
            return Err("invalid cached upload ID".into());
        }
        Ok(Some((id, expires)))
    }
    ...
    pub fn cache_upload(&self, key: &str, id: &str, expires: u64) -> Result<(), String> {
        self.path(key)?;
        self.write(
            &self.root.join("uploads").join(key),
            &serde_json::to_vec(&(id, expires)).map_err(|e| e.to_string())?,
        )
    }
    ...
    pub fn record_owned_upload(&self, scope: &str, id: &str, expires: u64) -> Result<(), String> {
        ...
        self.write(
            &self.root.join("owned-uploads").join(scope).join(id),
    ...
    pub fn admit_tool_image(
        &self,
        session: &str,
        data: &[u8],
    ) -> Result<(ImageRef, (u32, u32)), String> {
        let format = image::guess_format(data).map_err(|e| e.to_string())?;
        ...
        let key = digest(
            &serde_json::to_vec(&(
                ...
                &attachment,
                &policy,
                processor.as_ref().map(|(version, _)| version),
            ))
            .map_err(|e| e.to_string())?,
        );
    ...
    pub fn read(&self, reference: &ImageRef) -> Result<Vec<u8>, String> {
        ...
        let mut data = Vec::new();
        file.take(self.policy.max_input_bytes as u64 + 1)
            .read_to_end(&mut data)
            .map_err(|e| e.to_string())?;
        if data.len() > self.policy.max_input_bytes
            || digest(&data) != reference.id
            || data.len() as u64 != reference.bytes
        {
            return Err("stored image is corrupt or exceeds the input limit".into());
        }
        let (image, format) = self.decode(&data)?;
        if image.dimensions() != (reference.width, reference.height)
            || media_type(format)? != reference.media_type
        {
    ...
    pub fn project_request(
        ...
        let mut record = |attachment: &ImageRef| -> Result<(), String> {
            ...
            let bytes = if let Some(bytes) = variants.get(&key) {
                *bytes
            } else {
                ...
                ModelTurn::ToolResults { results } => {
                    for result in results {
                        for part in &mut result.content {
                            if remove > 0 {
                                if let ToolResultContentPart::Image { attachment } = part {
                                    *part = ToolResultContentPart::Text {
                                        text: placeholder(attachment),
                                    };
                                    remove -= 1;
                                }
    ...
    pub fn request_image(
        ...
        if let Ok(file) = std::fs::File::open(&path) {
            use std::io::Read;
            let mut data = Vec::new();
            file.take(policy.max_bytes as u64 + 1)
                .read_to_end(&mut data)
                .map_err(|e| e.to_string())?;
        ...
        let source = if let Some((_, process)) = processor {
        ...
        if policy.animation == AnimationPolicy::Reject {
            ...
            let animated = match format {
                ImageFormat::Gif => {
                    let mut decoder = image::codecs::gif::GifDecoder::new(Cursor::new(&source))
                        .map_err(|e| e.to_string())?;
                    let mut limits = image::Limits::default();
                    limits.max_alloc = Some(self.policy.max_input_pixels.saturating_mul(16));
                ...
                ImageFormat::Png => image::codecs::png::PngDecoder::new(Cursor::new(&source))
                    .map_err(|e| e.to_string())?
                    .is_apng()
                    .map_err(|e| e.to_string())?,
                ImageFormat::WebP => image::codecs::webp::WebPDecoder::new(Cursor::new(&source))
                    .map_err(|e| e.to_string())?
                    .has_animation(),
                _ => false,
            };
            if animated {
                return Err("animated images are disabled by image policy".into());
            }
        }
        if policy.normalize_srgb {
            ...
            if let Some(icc) = decoder.icc_profile().map_err(|e| e.to_string())? {
                let profile = moxcms::ColorProfile::new_from_slice(&icc)
                    .map_err(|e| format!("invalid image ICC profile: {e}"))?;
                let transform = profile
                    ...
                    .map_err(|e| format!("unsupported image ICC transform: {e}"))?;
                let pixels = image.to_rgba8();
                let mut converted = vec![0; pixels.len()];
                transform
                    .transform(pixels.as_raw(), &mut converted)
                    .map_err(|e| format!("image ICC transform failed: {e}"))?;
                image = image::DynamicImage::ImageRgba8(
                    image::RgbaImage::from_raw(pixels.width(), pixels.height(), converted)
                        .ok_or("invalid converted image")?,
                );
            } else {
                image
                    .apply_color_space(image::metadata::Cicp::SRGB, Default::default())
                    .map_err(|e| format!("image color conversion failed: {e}"))?;
            }
        }
        let scale = ((policy.max_pixels as f64
            / (u64::from(image.width()) * u64::from(image.height())) as f64)
            .sqrt())
        .min(policy.max_dimension as f64 / image.width().max(image.height()) as f64)
        .min(1.0);
        if scale < 1.0 {
            image = image.resize(
                (image.width() as f64 * scale).max(1.0) as u32,
                (image.height() as f64 * scale).max(1.0) as u32,
        ...
        let mime = if policy.lossless {
            ...
            for quality in [
                policy.quality,
                policy.quality.min(75),
                policy.quality.min(60),
            ] {
                let mut config =
                    webp::WebPConfig::new().map_err(|e| format!("WebP configuration: {e:?}"))?;
                config.quality = f32::from(quality);
                config.method = 0;
                data = encoder
                    .encode_advanced(&config)
                    .map_err(|e| format!("WebP encoding: {e:?}"))?
                    .to_vec();
                if data.len() <= policy.max_bytes {
                    break;
                }
        ...
        if data.len() > policy.max_bytes {
            return Err("processed image exceeds byte limit; adjust image policy".into());
        }
