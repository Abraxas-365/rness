...
impl Default for PlanLabels {
    fn default() -> Self {
        Self {
            ...
            edit: "Open in editor".into(),
            edit_and_approve: "Edit & approve".into(),
...
impl PlanReviewConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.height < 10
            || self
                .height
                .saturating_sub(self.padding.top + self.padding.bottom + border)
                < 6
            || self
                .width
                .saturating_sub(self.padding.left + self.padding.right + border)
                < 24
        {
        ...
        for text in [
            ...
            &self.labels.edit,
            &self.labels.edit_and_approve,
            &self.labels.feedback_prompt,
        ] {
        ...
        if self.editor.as_ref().is_some_and(|argv| {
            argv.first().is_none_or(|s| s.trim().is_empty())
        ...
        for (action, key) in &self.keys {
            if !defaults.contains_key(action) {
                return Err(format!("unknown plan review action: {action}"));
            }
            ...
            if control && code == "c" && action != "cancel" {
                return Err("Ctrl+C is reserved for cancellation".into());
            }
            if action.starts_with("feedback_") {
                if (control && code == "c")
                    || (!control && !alt && (code.len() == 1 || code == "space"))
                    || code == "backspace"
                {
                    return Err(
                        "feedback controls must not conflict with text editing or cancellation"
                            .into(),
                    );
                }
                if self.keys.get("cancel").is_some_and(|key| {
            ...
            if !seen.insert(chord) {
                return Err("duplicate plan review key bindings".into());
            }
        }
        if self
            .keys
            .get("feedback_submit")
            .zip(self.keys.get("feedback_back"))
            .is_some_and(|(a, b)| {
                rness_kernel::presentation::canonical_chord(a).ok()
                    == rness_kernel::presentation::canonical_chord(b).ok()
            })
        {
