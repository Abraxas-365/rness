...
pub fn validate_plan(plan: &str) -> Result<(), String> {
    if !plan
        .trim()
        .strip_prefix("# ")
        .is_some_and(|s| !s.lines().next().unwrap_or("").trim().is_empty())
    {
...
impl PlanSelections {
    pub fn select(&self, session: &str, active: bool) {
        self.0.lock().unwrap().insert(session.into(), active);
    }
    pub fn pending(&self, session: &str) -> Option<bool> {
        self.0.lock().unwrap().get(session).copied()
    }
    pub fn accept(
        &self,
        session: &str,
        commit: impl FnOnce(bool) -> Result<(), crate::session::log::LogError>,
    ) -> Result<(), crate::session::log::LogError> {
        let mut pending = self.0.lock().unwrap();
        if let Some(active) = pending.get(session).copied() {
            commit(active)?;
...
impl crate::tools::Tool for ExitPlan {
    ...
    fn plan_config(&self) -> Option<PlanConfig> {
        (!self.alive.is_cancelled()).then(|| self.config.clone())
    }
    async fn execute(&self, _: serde_json::Value) -> Result<String, String> {
        Err("plan review requires agent dispatch".into())
    }
    ...
    ) -> Result<(String, PlanReview), String> {
        self.review(session, call, args, cancel).await
    }
}
...
impl ExitPlan {
    pub async fn review(
        ...
        .active
        {
            return Err("exit_plan_mode requires active plan mode".into());
        }
        if !self.questions.is_available() {
            return Err("no Questions frontend is available; plan mode remains active".into());
        }
        use crate::questions::{Answers, OptionItem, Question, Request};
        let request = Request {
            ...
            questions: vec![Question {
                ...
                markdown: Some(input.plan),
                multi_select: false,
                options: vec![
                    OptionItem {
                        label: "Approve".into(),
                        description: "Execute the plan from the next step".into(),
                    },
                    OptionItem {
                        label: "Keep planning".into(),
                        description: "Revise the plan using your feedback".into(),
                    },
                ],
            }],
        };
        ...
        if cancel.is_cancelled() || self.alive.is_cancelled() {
            return Err("plan review cancelled or plugin unloaded".into());
        }
        let response =
            match response {
                Ok(response) => response,
                Err(_) => return Ok((
                    "Plan review closed. Stay in plan mode and wait for the user's next message."
                        .into(),
                    PlanReview::Dismissed,
                )),
            };
        ...
        let answer = answers
            .answers
            .iter()
            .find(|answer| answer.id == "plan-review")
            .ok_or("missing plan review answer")?;
        if answer.selected == ["Approve"] && answer.custom.is_none() {
            let output = match &answer.edited_markdown {
                ...
                None => {
                    "Plan approved. Leave plan mode at the next step and carry out the plan.".into()
                }
            };
            ...
            Ok((
                format!(
                    "Keep planning. Revise and present the plan again. Feedback: {}",
                    answer.custom.as_deref().unwrap_or("")
                ),
                PlanReview::KeepPlanning,
            ))
        }
    }
}
