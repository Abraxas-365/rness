...
impl SubagentActivity {
    pub(super) fn register(&self, sessions: &SessionService, parent: &str, call: &str, child: &str, args: Value) {
        let after = sessions.store().history(&child.to_owned()).ok()
            .and_then(|history| history.last().map(|event| event.id.clone()));
        self.runs.lock().unwrap().insert(child.into(), Run {
            parent: parent.into(), call: call.into(), child: child.into(), args, after,
            started: Instant::now(), lines: Vec::new(), activity: "Starting".into(), status: "running".into(),
            live: String::new(), finished_ms: None, streams: Default::default(), recovered: false, start_ms: None,
        });
    }
    ...
    pub fn recover(&self, sessions: &SessionService, parent: &str) -> Result<(), crate::service::ServiceError> {
            ...
            if let SessionEvent::AssistantMessage(message) = event.event {
                for part in message.content {
                    if let ContentPart::ToolUse { call, name, args } = part {
                        if name == "subagent" { calls.insert(call, args); }
                    }
                }
            }
        }
        for child in sessions.list()? {
            let Some(link) = sessions.store().delegation(&child)? else { continue };
            if link.parent != parent { continue; }
            let Some(call) = link.call else { continue };
            let Some(args) = calls.get(&call) else { continue };
            let mut runs = self.runs.lock().unwrap();
            if runs.contains_key(&child) { continue; }
            let after = sessions.store().parent(&child)?.map(|fork| fork.at);
            runs.insert(child.clone(), Run {
                parent: parent.into(), call, child, args: args.clone(), after,
                started: Instant::now(), lines: Vec::new(), activity: "Interrupted".into(), status: "interrupted".into(),
                live: String::new(), finished_ms: Some(0), streams: Default::default(), recovered: true, start_ms: None,
            });
        }
        Ok(())
    }
    ...
    pub fn observe(&self, frame: &rness_protocol::frames::Frame) {
        ...
        match frame {
            ...
            Frame::StepCommitted { session, .. } => {
                if let Some(run) = runs.get_mut(session) { run.live.clear(); }
            }
            Frame::Delta { session, chunk: ChunkDelta::Text { t } } => {
                if let Some(run) = runs.get_mut(session) { run.live.push_str(t); }
            }
            _ => {}
        }
    }

    pub fn snapshots(&self, sessions: &SessionService, parent: &str) -> Vec<(String, Value, Value)> {
        let mut runs = self.runs.lock().unwrap();
        runs.values_mut().filter(|run| run.parent == parent).map(|run| {
            ...
            for event in history {
                ...
                match event.event {
                    ...
                    SessionEvent::ToolResult(result) => {
                        ...
                        run.lines.extend(result.output.lines().map(str::to_owned));
            ...
            (run.call.clone(), run.args.clone(), json!({
                ...
                "lines":run.lines, "live":run.live, "streams":run.streams,
            }))
        }).collect()
    }
}
