...
impl QuestionEvent {
    pub fn session(&self) -> &str {
        match self {
            Self::QuestionRequested { session, .. } | Self::QuestionResolved { session, .. } => {
                session
            }
        }
    }
...
pub struct Questions {
    ...
    registration: Mutex<
        Option<(
            std::sync::Weak<crate::tools::ToolRegistry>,
            std::sync::Weak<Questions>,
            Option<std::sync::Weak<dyn crate::tools::Tool>>,
        )>,
    >,
}
...
impl Questions {
    pub fn subscribe(&self) -> tokio::sync::broadcast::Receiver<QuestionEvent> {
        self.events.subscribe()
    }
    fn resolved(&self, session: &str, call: &str) {
        let _ = self.events.send(QuestionEvent::QuestionResolved {
    ...
    pub fn bind_registry(self: &Arc<Self>, registry: &Arc<crate::tools::ToolRegistry>) {
        *self.registration.lock().unwrap() =
            Some((Arc::downgrade(registry), Arc::downgrade(self), None));
    ...
    pub fn set_available(&self, available: bool) {
            ...
            if let Some(registry) = registry.upgrade() {
                if available && installed.as_ref().and_then(|tool| tool.upgrade()).is_none() {
                    let tool: Arc<dyn crate::tools::Tool> =
                        Arc::new(AskUser(broker.upgrade().expect("live broker")));
                    ...
                    *installed = Some(Arc::downgrade(&tool));
                    ...
                    if let Some(tool) = installed.take().and_then(|tool| tool.upgrade()) {
        ...
        if !available {
            ...
            for (session, call) in pending.keys() {
    ...
    pub fn resolve(&self, session: &str, call: &str, answers: Answers) -> Result<(), String> {
        ...
        if answers.answers.len() != item.request.questions.len() {
        ...
        for answer in &answers.answers {
            ...
            if !ids.insert(&answer.id) || (!question.multi_select && answer.selected.len() > 1) {
                return Err("duplicate answer or too many selections".into());
            }
            let mut selections = std::collections::HashSet::new();
            for selected in &answer.selected {
                if !selections.insert(selected)
                    || !question.options.iter().any(|o| &o.label == selected)
                {
            ...
            if let Some(plan) = &answer.edited_markdown {
                if !question
                    .plan_review
                    .as_ref()
                    .is_some_and(|config| config.edit_enabled)
                    || answer.selected != ["Approve"]
                    || answer.custom.is_some()
                {
                    return Err(
                        "edited Markdown requires plan approval with editing enabled".into(),
                    );
                }
        ...
        let result = pending
            ...
            .tx
            .send(answers)
            .map_err(|_| "question cancelled".into());
        self.resolved(session, call);
        result
    }
    ...
    pub(crate) async fn ask(
        ...
        self.ask_presented(request, cancel)
            .await
            .map(|(output, _)| output)
            .map_err(|(error, _)| error)
    }

    async fn ask_presented(
        &self,
        request: Request,
        cancel: &CancellationToken,
    ) -> Result<(String, serde_json::Value), (String, &'static str)> {
        let (tx, rx) = oneshot::channel();
        let key = (request.session.clone(), request.call.clone());
        {
            let mut pending = self.pending.lock().unwrap();
            if !self.available.load(std::sync::atomic::Ordering::SeqCst) {
                return Err(("no question frontend available".into(), "unavailable"));
            }
            if pending.contains_key(&key) {
                return Err(("duplicate pending question call".into(), "duplicate"));
            }
            let event = QuestionEvent::QuestionRequested {
        ...
        impl Drop for Guard<'_> {
            fn drop(&mut self) {
                self.0.dismiss(&self.1 .0, &self.1 .1);
            }
        }
...
mod tests {
    ...
    async fn dispatch_validates_answers_and_cleans_up_cancellation() {
        ...
        let call = ToolCall {
            call: "c".into(),
            name: "AskUser".into(),
            args: serde_json::json!({"questions":[{"id":"q","question":"Choose","options":[{"label":"A"}]}]}),
        };
        let result = registry
            .dispatch(&"s".into(), &[call.clone()], 1, &CancellationToken::new())
            .await;
        ...
        for cancel_it in [false, true] {
            ...
            let worker =
                tokio::spawn(
                    async move { registry.dispatch(&"s".into(), &[call], 1, &token).await },
                );
            ...
            .unwrap();
            assert!(questions
                .resolve("s", "c", Answers { answers: vec![] })
                .is_err());
            assert!(questions
                .resolve(
                    "s",
                    ...
                    Answers {
                        answers: vec![Answer {
                            ...
                            selected: vec!["A".into()],
                            custom: None,
                            edited_markdown: Some("# Not a plan review".into()),
                        }]
                    }
                )
                .is_err());
            ...
            assert_eq!(
                metadata["status"],
                if cancel_it { "cancelled" } else { "answered" }
            );
            if !cancel_it {
                assert_eq!(metadata["answers"]["answers"][0]["selected"][0], "A");
            }
            if !cancel_it {
                assert!(result[0].output.contains("A"));
            }
...
impl crate::tools::Tool for AskUser {
    ...
    fn description(&self) -> &str {
        "Ask the user for missing information or decisions. Provide stable question IDs and optional choices; custom text is always allowed."
    }
    ...
    async fn execute(&self, _: serde_json::Value) -> Result<String, String> {
        Err("AskUser requires agent dispatch context".into())
    }
    async fn execute_call(
        ...
        let (content, _, error, _) = self
            .execute_presented(session, &call.to_owned(), args, cancel)
            .await?;
    ...
    > {
        ...
        if input.questions.is_empty() || input.questions.len() > 16 {
            return Err("expected 1 to 16 questions".into());
        }
        let mut ids = std::collections::HashSet::new();
        for q in &input.questions {
            if q.id.trim().is_empty() || q.question.trim().is_empty() || !ids.insert(&q.id) {
                return Err("questions need unique nonempty IDs and text".into());
            }
            let mut labels = std::collections::HashSet::new();
            if q.options
                .iter()
                .any(|o| o.label.trim().is_empty() || !labels.insert(&o.label))
            {
                return Err("options need unique nonempty labels".into());
            }
        }
        let (output, error, metadata) = match self
            .0
            .ask_presented(
                Request {
            ...
            .await
        {
            ...
            Err((output, status)) => (
                output,
                true,
                serde_json::json!({"version":1,"kind":"questions","status":status}),
            ),
        };
        Ok((
