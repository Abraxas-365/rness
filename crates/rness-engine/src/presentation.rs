...
pub trait ToolCards: Send + Sync {
    ...
    async fn tool_card_result(
        &self,
        args: serde_json::Value,
        result: &rness_protocol::events::ToolResult,
    ) -> Option<Vec<rness_kernel::presentation::StyledLine>> {
        let mut presentation = result
            .presentation
            .clone()
            .filter(serde_json::Value::is_object)
            .unwrap_or_else(|| serde_json::json!({"version":1,"kind":"tool_result"}));
        if let Some(object) = presentation.as_object_mut() {
            ...
            if let Some(tasks) = &result.tasks {
                object.insert("tasks".into(), serde_json::to_value(tasks).ok()?);
            }
            if let Some(review) = &result.plan_review {
                object.insert("plan_review".into(), serde_json::to_value(review).ok()?);
            }
        }
        self.tool_card_presented(
            &result.name,
            args,
            &result.output,
            result.is_error,
