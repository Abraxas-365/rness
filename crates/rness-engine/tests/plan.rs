...
async fn approval_continues_next_step_but_dismissal_stops() {
    for dismiss in [false, true] {
        ...
        let answer = tokio::spawn(async move {
            ...
            if dismiss { q.dismiss(&s, "review"); } else { q.resolve(&s, "review", Answers { answers: vec![Answer { id: "plan-review".into(), selected: vec!["Approve".into()], custom: None }] }).unwrap(); }
        });
        ...
        let inherited = PlanState::from_history(&store.history(child.session()).unwrap());
...
async fn reviews_are_explicit_and_cancelable() {
    ...
    questions.set_available(true);
    for mode in ["approve", "feedback", "dismiss", "cancel", "unload"] {
        ...
        match mode {
            ...
            "cancel" => token.cancel(),
            "unload" => alive.cancel(),
            _ => questions.resolve(&session, "review", Answers { answers: vec![Answer { id: "plan-review".into(), selected: vec![if mode == "approve" { "Approve" } else { "Keep planning" }.into()], custom: if mode == "feedback" { Some("add tests".into()) } else { None } }] }).unwrap(),
        }
        let result = worker.await.unwrap().remove(0);
        assert_eq!(result.plan_review, match mode { "approve" => Some(PlanReview::Approved), "feedback" => Some(PlanReview::KeepPlanning), "dismiss" => Some(PlanReview::Dismissed), _ => None });
