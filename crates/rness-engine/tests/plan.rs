use rness_engine::{
    ...
    questions::{Answer, Answers, Questions},
...
impl rness_engine::turn::provider::Provider for ReviewProvider {
    ...
    async fn step(
        ...
        rness_engine::turn::provider::StepOutcome::Committed(AssistantMessage {
            model: "plan-test".into(),
            content: if first {
                vec![ContentPart::ToolUse {
                    call: "review".into(),
                    name: "exit_plan_mode".into(),
                    args: serde_json::json!({"plan":"# Implementation\nAdd tests"}),
                }]
            } else {
                vec![ContentPart::Text {
                    text: "execution begins".into(),
                }]
            },
            stop: if first {
                StopReason::ToolUse
...
async fn approval_continues_next_step_but_dismissal_stops() {
    for (dismiss, edited) in [(false, false), (false, true), (true, false)] {
        ...
        let at = history
            .iter()
            .find(|env| matches!(env.event, SessionEvent::ToolResult(_)))
            .unwrap()
            .id
            .clone();
        drop(log);
...
async fn reviews_are_explicit_and_cancelable() {
    ...
    let call = ToolCall {
        call: "review".into(),
        name: "exit_plan_mode".into(),
        args: serde_json::json!({"plan":"# Plan\nDo the work"}),
    };
    let unavailable = tools
        .dispatch(&session, &[call.clone()], 1, &CancellationToken::new())
        .await;
    assert!(unavailable[0].is_error);
    questions.set_available(true);
    for mode in [
        "approve", "edited", "feedback", "dismiss", "cancel", "unload",
    ] {
        ...
        match mode {
            "dismiss" => {
                questions.dismiss(&session, "review");
            }
            "cancel" => token.cancel(),
            "unload" => alive.cancel(),
            _ => questions
                .resolve(
                    ...
                    Answers {
                        answers: vec![Answer {
                            edited_markdown: (mode == "edited")
                                .then(|| "# Revised plan\nUser-authored implementation".into()),
                            id: "plan-review".into(),
                            selected: vec![if mode == "approve" || mode == "edited" {
                                "Approve"
                            } else {
                                "Keep planning"
                            }
                            .into()],
                            custom: if mode == "feedback" {
                                Some("add tests".into())
                            } else {
                                None
                            },
        ...
        assert_eq!(
            result.plan_review,
            match mode {
                "approve" | "edited" => Some(PlanReview::Approved),
                "feedback" => Some(PlanReview::KeepPlanning),
                "dismiss" => Some(PlanReview::Dismissed),
                _ => None,
            }
        );
        if mode == "edited" {
