...
fn session_search_filters_before_limit_refreshes_and_stays_lazy() {
    ...
    let hits = provider
        .search("/workspace", "search needle", 100, |revision| {
            store.search_snapshot("/workspace", revision)
        })
        .unwrap();
    ...
    assert!(store
        .search_snapshot("/workspace", Some(&snapshot.revision))
        .unwrap()
        .is_none());
}
...
fn session_event_read_authorizes_and_reads_original_local_text() {
    ...
    let document = store
        .search_event_read(caller.session(), target.session(), &event.id)
        .unwrap()
        .unwrap();
    assert_eq!(document.text, "full original text 🦀");
    assert_eq!(document.event_ref, event.id);
    assert!(store
        .search_event_read(caller.session(), target.session(), "missing")
        .unwrap()
        .is_none());
    assert!(store
        .search_event_read(caller.session(), other.session(), &event.id)
        .is_err());
    assert!(store
        .search_event_read(no_workspace.session(), target.session(), &event.id)
        .is_err());
    assert!(store
        .search_workspace(caller.session(), Some(&"../escape".into()))
        .is_err());
    assert!(store
        .search_workspace(caller.session(), Some(&"..\\escape".into()))
        .is_err());
