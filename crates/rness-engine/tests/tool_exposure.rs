...
fn search_ranks_partial_matches_reports_limits_and_respects_scope() {
    ...
    for i in 0..25 {
        tools.register(Arc::new(Named(format!("chrome_{i:02}"))));
    }
    tools.register(Arc::new(Named("chrome_browser".into())));
    let exposure = Exposure::default();
    let (output, names) = exposure
        .search(&tools, &json!({"query":"chrome MCP browser"}))
        .unwrap();
    ...
    let requested = (0..25)
        .map(|i| format!("CHROME_{i:02}"))
        .collect::<Vec<_>>()
        .join(",");
    let (output, names) = exposure
        .search(
            &tools,
            &json!({"query":format!(" select:{requested},missing ")}),
        )
        .unwrap();
    let output: Value = serde_json::from_str(&output).unwrap();
    ...
    let (output, names) = exposure
        .search(&tools.restricted(&[]), &json!({"query":"chrome"}))
        .unwrap();
    let output: Value = serde_json::from_str(&output).unwrap();
    ...
    assert!(output["guidance"]
        .as_str()
        .unwrap()
        .contains("No permitted"));
    assert!(exposure
        .search(&tools, &json!({"query":"select: , "}))
        .is_err());
}
...
async fn parallel_calls_overlap_and_keep_input_order() {
    ...
    impl Tool for BarrierTool {
        ...
        async fn execute(&self, args: Value) -> Result<String, String> {
            self.0.wait().await;
            Ok(args["index"].to_string())
        }
    }
    let registry = Arc::new(ToolRegistry::default());
    registry.register(Arc::new(BarrierTool(Arc::new(tokio::sync::Barrier::new(
        4,
    )))));
    let call = ToolCall {
        call: "p".into(),
        name: "run_code".into(),
        args: json!({"code":"local calls = {}; for i=1,8 do calls[i]={name='Barrier',args={index=i}} end; local results=tools.parallel(calls); for i=1,8 do assert(results[i].output == tostring(i)) end; return results"}),
    };
    let (result, nested) = tokio::time::timeout(
        std::time::Duration::from_secs(5),
        program(registry, "s".into(), call, CancellationToken::new(), None),
    )
    .await
    .unwrap();
    assert!(!result.is_error, "{}", result.output);
    assert_eq!(nested.len(), 8);
    for (index, (call, _)) in nested.iter().enumerate() {
        assert_eq!(call.call, format!("p/{index}"));
    }
}
...
async fn background_admission_uses_permissions_not_turn_exposure() {
    ...
    impl Tool for Named {
        ...
        fn starts_background_job(&self, _: &Value) -> bool {
            self.0 == "Background"
        }
        async fn execute(&self, _: Value) -> Result<String, String> {
            Ok("executed".into())
    ...
    impl rness_engine::turn::provider::Provider for BackgroundFlow {
        ...
        async fn step(
            &self,
            request: rness_engine::turn::provider::StepRequest<'_>,
            _: &CancellationToken,
            ...
            assert!(request
                .tools
                .iter()
                .all(|tool| !tool.name.starts_with("job_")));
            ...
            if first {
                if self.mixed {
                    content.push(ContentPart::ToolUse {
                        ...
                        args: json!({"query":"select:Background"}),
                    });
                }
                content.push(ContentPart::ToolUse {
                    call: "start".into(),
    ...
    for mixed in [false, true] {
        for permitted in [false, true] {
            ...
            let tools = ToolRegistry::default();
            for name in ["Background", "job_output", "job_list", "job_kill"] {
                tools.register(Arc::new(Named(name)));
            }
            if !permitted {
                log.append(&SessionEvent::RequestConfig(CallConfig {
                    tool_ceiling: Some(vec!["Background".into()]),
                    ..Default::default()
                }))
                .unwrap();
            }
            let config = rness_engine::turn::TurnConfig {
                tool_exposure: Exposure {
                    mode: Mode::Native,
                    deferred: ["job_output", "job_list", "job_kill"]
                        .map(str::to_owned)
                        .to_vec(),
                },
                ..Default::default()
            };
            let provider = BackgroundFlow {
                step: 0.into(),
                mixed,
            };
            ...
            assert!(
                results
                    .iter()
                    .find(|result| result.call == "hidden")
                    .unwrap()
                    .is_error
            );
        }
...
impl rness_engine::turn::provider::Provider for Flow {
    ...
    async fn step(
        ...
        let content = match step {
            0 => vec![ContentPart::ToolUse {
                call: "search".into(),
                name: "ToolSearch".into(),
                args: json!({"query":"select:Echo"}),
            }],
            1 => vec![ContentPart::ToolUse {
                call: "program".into(),
                name: "run_code".into(),
                args: json!({"code":"return tools.call('Echo', {value=42})"}),
            }],
...
async fn activation_and_nested_audit_survive_replay() {
    ...
    let started = replayed
        .history
        .iter()
        .position(|e| matches!(e.event, SessionEvent::ProgramToolStarted { .. }))
        .unwrap();
    let ended = replayed
        .history
        .iter()
        .position(|e| matches!(e.event, SessionEvent::ProgramToolResult { .. }))
        .unwrap();
...
fn discovery_respects_scope_and_modes() {
    ...
    assert!(!config
        ...
        .any(|s| s.name == "Echo"));
    let (_, names) = config
        .search(&tools, &json!({"query":"select:Echo"}))
        .unwrap();
    assert!(config
        .specs(&tools, &names.into_iter().collect())
        .iter()
        .any(|s| s.name == "Echo"));
    assert!(config
        .search(&tools.restricted(&[]), &json!({"query":"Echo"}))
        .unwrap()
        .1
        .is_empty());
    let ptc = Exposure {
    ...
    assert!(!ptc
        .specs(&tools, &BTreeSet::from(["Echo".into()]))
        .iter()
        .any(|s| s.name == "Echo"));
}
#[tokio::test]
async fn programs_are_isolated_and_budgeted() {
    ...
    tools
        .approvals()
        .set_rules(std::collections::BTreeMap::from([(
            "Echo".into(),
    ...
    let (_, nested) = program(
        ...
        ToolCall {
            call: "denied".into(),
            name: "run_code".into(),
            args: json!({"code":"return tools.call('Echo', {})"}),
        },
        CancellationToken::new(),
        None,
    )
    ...
    let (result, _) = program(
        ...
        ToolCall {
            call: "c".into(),
            name: "run_code".into(),
            args: json!({"code":"while true do end"}),
        },
        cancel,
        None,
    )
