...
use rness_providers::routes::{build, builtin_routes, env_var_for, RouteError, Selection};
...
fn select(slug: &str) -> Result<Selection, RouteError> {
    Selection::parse(&builtin_routes(), Some(slug))
}
...
fn no_selection_is_an_error_listing_routes() {
    let Err(err) = Selection::parse(&builtin_routes(), None) else {
        panic!("expected error");
    };
    let msg = err.to_string();
    assert!(msg.contains("-m <provider>/<model>"), "{msg}");
    for name in ["anthropic", "deepseek", "groq", "ollama", "openai-chatgpt", "openai"] {
...
fn missing_credential_names_env_and_command() {
    ...
    let err = match build(&builtin_routes(), &select("groq/llama-3.3-70b").unwrap(), store(&dir)) {
...
fn stored_key_builds_openai_compatible_route() {
    ...
    let provider = build(&builtin_routes(), &select("deepseek/deepseek-reasoner").unwrap(), s)
        .unwrap();
...
fn ollama_needs_no_credentials() {
    let dir = TempDir::new().unwrap();
    let provider =
        build(&builtin_routes(), &select("ollama/qwen3").unwrap(), store(&dir)).unwrap();
...
fn anthropic_route_builds_without_stored_credentials() {
    ...
    let dir = TempDir::new().unwrap();
    let provider =
        build(&builtin_routes(), &select("anthropic/claude-sonnet-4-5").unwrap(), store(&dir))
            .unwrap();
