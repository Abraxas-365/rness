...
use rness_engine::{
    session::projection::ModelContext,
    turn::provider::{Provider, StepRequest},
};
use rness_providers::{
    auth::{CredentialStore, Tokens},
    routes::{self, Kind, Route, Selection},
};
...
async fn body_inactivity_timeout_is_explicit_and_retryable() {
    ...
    for kind in [
        ...
        let route = Route {
            ...
            stream_idle_timeout: Some(std::time::Duration::from_millis(50)),
        ...
        let provider = if matches!(kind, Kind::ChatGptResponses) {
            ...
            routes::build_with_api_key(&route, "test", "fake".into()).unwrap()
        };
        ...
        assert!(
            error.message.contains("inactivity timeout"),
            "{}",
            error.message
        );
    }
...
async fn api_keys_use_protocol_specific_headers() {
    for (kind, expected) in [
        ...
        assert_eq!(
            headers.get(expected).unwrap().to_str().unwrap(),
            if expected == "x-api-key" {
                "fake-key"
            } else {
                "Bearer fake-key"
            }
        );
        assert!(!headers.contains_key(if expected == "x-api-key" {
            "authorization"
        } else {
            "x-api-key"
        }));
...
async fn no_auth_means_no_authorization_header() {
    ...
    send(provider.as_ref()).await;
    assert_eq!(
        server.received_requests().await.unwrap()[0].headers["x-tenant"],
        "tenant-secret"
    );
...
async fn oauth_uses_named_tokens_not_stored_api_keys() {
    for kind in [Kind::Anthropic, Kind::ChatGptResponses] {
        ...
        let mut tokens = Tokens {
            access_token: "named-token".into(),
            ..Default::default()
        };
        tokens
            .extra
            .insert("accountId".into(), serde_json::json!("named-account"));
        ...
        let provider =
            routes::build_with_oauth(&route, "test", store, "custom-login".into()).unwrap();
        send(provider.as_ref()).await;
        let requests = server.received_requests().await.unwrap();
        ...
        if matches!(route.kind, Kind::ChatGptResponses) {
            assert_eq!(
                requests[0].headers.get("chatgpt-account-id").unwrap(),
                "named-account"
            );
        }
    }
...
async fn custom_headers_follow_same_origin_redirects_only() {
    for same_origin in [false, true] {
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(400))
            .expect(if same_origin { 1 } else { 0 })
            .mount(target)
            .await;
