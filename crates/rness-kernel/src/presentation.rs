...
pub fn core_action_scope(name: &str) -> Option<&'static str> {
    match name {
        ...
        "core.messagebox.previous_thinking" | "core.messagebox.next_thinking" | "core.messagebox.toggle_thinking"
        | "core.messagebox.previous_tool" | "core.messagebox.next_tool" | "core.messagebox.toggle_tool"
        | "core.messagebox.noop" => Some("messagebox"),
        "core.promptbox.completion_previous" | "core.promptbox.completion_next" | "core.promptbox.completion_accept"
        | "core.promptbox.completion_dismiss" | "core.promptbox.close_preview" | "core.promptbox.preview_up"
        | "core.promptbox.preview_down" | "core.promptbox.preview_page_up" | "core.promptbox.preview_page_down"
        | "core.promptbox.preview_start" | "core.promptbox.preview_end" => Some("promptbox"),
...
pub fn core_action_matches_scope(name: &str, scope: &str) -> bool {
    match core_action_scope(name) {
        Some("app") => scope.strip_prefix("app:").is_some_and(|name| !name.is_empty()),
...
pub fn canonical_chord(input: &str) -> Result<String, String> {
    let input = match (input.strip_prefix('<'), input.strip_suffix('>')) {
        (Some(_), Some(_)) => input[1..input.len()-1].replace("C-", "ctrl+").replace("A-", "alt+").replace("S-", "shift+"),
    ...
    for part in input.split('+') {
        let modifier = match part { "ctrl" => Some(0), "alt" => Some(1), "shift" => Some(2), _ => None };
        if let Some(index) = modifier {
            if modifiers[index] { return Err("duplicate key modifier".into()); }
            modifiers[index] = true;
        } else {
            if code.is_some() { return Err("multi-key sequences are not supported".into()); }
            let valid = matches!(part, "enter" | "esc" | "tab" | "space" | "up" | "down" | "left" | "right" | "pageup" | "pagedown" | "home" | "end" | "backspace" | "delete")
                || (part.len() == 1 && part.as_bytes()[0].is_ascii_graphic() && !matches!(part, "<" | ">"))
                || part.strip_prefix('f').and_then(|n| n.parse::<u8>().ok()).is_some_and(|n| (1..=24).contains(&n));
            if !valid { return Err(format!("invalid key chord: {input}")); }
            code = Some(part);
        }
    }
