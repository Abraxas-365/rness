...
pub trait Backend: Send + Sync {
    ...
    fn prepare_input(
        ...
        _session: &SessionId,
        text: &str,
    ) -> Result<Option<Vec<ContentPart>>, String> {
...
impl Model {
    ...
    pub fn load_history(&mut self, history: &History) {
        let append = self.projected_session.as_ref() == Some(&history.session)
            ...
            && !history.envelopes[self.projected_events..]
                .iter()
        ...
        if self.history_revision == 0
            ...
            || self.entry_ids != old_ids
            || self.tool_durations != old_durations
            || self.error_entries != old_errors
            || self.assistant_ids != old_assistant_ids
        {
    ...
    fn append_history(&mut self, events: &[rness_protocol::events::Envelope]) {
        ...
        if let Some(last) = events.last() {
            self.projected_tail = Some(last.id.clone());
        }
    }
    ...
    fn project_events(
        ...
        for env in events {
            ...
            if let Some((shadowed, summary)) = checkpoints.get(env.id.as_str()) {
            ...
            match &env.event {
                ...
                SessionEvent::TurnEnded {
                    ...
                    let detail = if *failed_attempts > 0 {
                        format!(
                            " after {failed_attempts} failed provider attempt(s); no automatic retries remain"
                        )
                    } else {
                        String::new()
                    };
                    ...
                    self.entries.push(Entry::Notice(format!(
                        "Turn failed{detail}. Use /retry to continue from saved context."
                    )));
                }
                ...
                SessionEvent::AssistantAttempt(attempt) => {
                    if let rness_protocol::events::AttemptOutcome::Error {
                        message,
                        code,
                        retry_in_ms,
                        ..
                    ...
                    {
                        *failed_attempts += 1;
                        let recovery = retry_in_ms
                            .map(|ms| {
                                format!("; retry {} scheduled after {ms} ms", *failed_attempts + 1)
                            })
                            .unwrap_or_default();
                        self.error_entries.insert(self.entries.len());
                        self.entries.push(Entry::Notice(format!(
                            ...
                            attempt.model,
                            failed_attempts,
                            code.as_deref().unwrap_or("PROVIDER"),
                            message,
                            recovery
                        )));
                    }
            ...
            if self.entries.len() > previous_len {
                self.entry_ids.push(env.id.clone());
            }
        }
    }
...
impl App {
    ...
    pub fn reconcile(&mut self) {
        let delta = self
            ...
            .as_deref()
            .filter(|_| {
                self.model.projected_session.as_ref() == Some(&self.model.session)
        ...
        if let Some(results) = self.command_results.get(&self.model.session) {
            self.model
                .entries
                .extend(results.iter().cloned().map(Entry::Notice));
        }
    }
    ...
    pub fn apply_frame(&mut self, frame: &Frame) {
        ...
        if session != &self.model.session {
            if !self.background_models.contains_key(session)
                && !matches!(
                    frame,
        ...
        if matches!(
            frame,
            Frame::ApprovalRequested { .. } | Frame::ApprovalResolved { .. }
        ) {
            self.input_epoch
        ...
        match event {
            TermEvent::Paste(text) => {
                ...
                let actions = self
                    .slots
                    .focused_mut(&ctx)
                    .map(|c| c.on_paste(&ctx, &text).actions)
                    .unwrap_or_default();
    ...
    fn route_key(&mut self, key: KeyEvent) -> Vec<Action> {
                ...
                if prompt_focused {
                        ...
                        && map.layer(&scope, &key) == Some(crate::keymaps::BindingLayer::User)
                    {
                        if let Some(action) = map
                            .lookup_exact(&scope, &key)
                            .and_then(|name| name.strip_prefix("core.promptbox."))
                        {
                            if action == "noop"
        ...
        if self.slots.prompt_focused(&ctx) {
            if let Some(sender) = &self.plugin_actions {
                ...
                if let Some(name) = map
                    .lookup_exact(&crate::keymaps::Scope::Promptbox, &key)
                    .filter(|_| {
                        map.generation
                            == self
                                .plugin_generation
                                .load(std::sync::atomic::Ordering::SeqCst)
                            && map.layer(&crate::keymaps::Scope::Promptbox, &key)
                                == Some(crate::keymaps::BindingLayer::User)
                    })
                {
                    ...
                    if let Some(action) = name.strip_prefix("core.promptbox.") {
                        return self
                            .slots
                            .focused_mut(&ctx)
                            .map(|component| component.on_binding(&ctx, action).actions)
                            .unwrap_or_default();
                    }
                    if let Some(action) = name
                        .strip_prefix("core.")
                        .and_then(crate::keymaps::HostAction::from_name)
                    {
                        ...
                        return vec![match action {
                            ...
                            HostAction::CancelOrQuit
                                if self.model.busy
                                    || self.backend.command_running(&self.model.session) =>
                            {
                                Action::Cancel
                            }
                    ...
                    if sender
                        .send(PluginActionRequest {
                            input_epoch: self.input_epoch.load(std::sync::atomic::Ordering::SeqCst),
                            app: None,
                            generation: map.generation,
                            name: name.into(),
                            session: self.model.session.clone(),
                            epoch: self.model.history_epoch,
                        })
                        .is_ok()
                    {
                        return vec![];
        ...
        if let Some(focused) = self.slots.focused_mut(&ctx) {
            if focused.name() == "ext_apps" {
                if let (Some((Some(name), activation)), Some(sender)) = (
                    self.apps.as_ref().map(|apps| apps.activation()),
                    &self.plugin_actions,
                ) {
                    let map = self.plugin_keymap.read().unwrap();
                    if map.generation
                            ...
                            .load(std::sync::atomic::Ordering::SeqCst)
                    {
                        ...
                        if let Some(action) = map.lookup_exact(&scope, &key).filter(|_| {
                            !core_close
                                || map.layer(&scope, &key)
                                    == Some(crate::keymaps::BindingLayer::User)
                        }) {
                            if action == "core.app.noop" {
                                return vec![];
                            }
                            if action == "core.app.close" {
                                if let Some(apps) = &self.apps {
                                    apps.close_activation(&name, activation);
                                }
                                return vec![];
                            }
                            if sender
        ...
        let explicit_message_binding = {
            ...
            map.generation
                == self
                    ...
                    .load(std::sync::atomic::Ordering::SeqCst)
                && map
                    .lookup_exact(&crate::keymaps::Scope::Messagebox, &key)
                    .is_some()
                && map.layer(&crate::keymaps::Scope::Messagebox, &key)
                    == Some(crate::keymaps::BindingLayer::User)
        };
        let prompt_binding = {
            let map = self.plugin_keymap.read().unwrap();
        ...
        if self.slots.prompt_focused(&ctx) {
                    ...
                    .load(std::sync::atomic::Ordering::SeqCst)
            {
                let scope = if map
                    ...
                    .is_some()
                {
                    crate::keymaps::Scope::Promptbox
                } else {
                    crate::keymaps::Scope::Messagebox
                };
                if let Some(name) = map.lookup(&scope, &key).filter(|_| {
                    map.lookup_exact(&scope, &key).is_some()
                        || map.layer(&scope, &key) == Some(crate::keymaps::BindingLayer::User)
                        || self.keymap.lookup(&key).is_none()
                }) {
                    if let Some(action) = name.strip_prefix("core.messagebox.") {
                        return self.slots.message_binding(&ctx, action).actions;
                    }
    ...
    pub fn apply(&mut self, action: Action) {
        ...
        match action {
            Action::PluginBatch {
                ...
                if request.input_epoch != self.input_epoch.load(std::sync::atomic::Ordering::SeqCst)
                    || request.generation
                        != self
                            .plugin_generation
                            .load(std::sync::atomic::Ordering::SeqCst)
                    || request.session != self.model.session
                    || request.epoch != self.model.history_epoch
                {
                    return;
                }
                let ctx = Ctx {
                ...
                if let Some((name, activation)) = &request.app {
                    if !self
                        .slots
                        .focused_mut(&ctx)
                        .is_some_and(|c| c.name() == "ext_apps")
                        || self.apps.as_ref().is_none_or(|apps| {
                            apps.activation() != (Some(name.clone()), *activation)
                        })
                    {
                        return;
                    }
                } else if !self.slots.prompt_focused(&ctx) {
                    return;
                }
                ...
                let apply_operations = || {
                    ...
                    for operation in operations {
                        match operation {
                            PluginOperation::InsertPrompt(text) => self.slots.broadcast(
                                &ctx,
            ...
            Action::PluginAppClose {
                ...
                if input_epoch != self.input_epoch.load(std::sync::atomic::Ordering::SeqCst) {
                    return;
                }
                let ctx = Ctx {
                    model: &self.model,
                ...
                let focused = self
                    .slots
                    .focused_mut(&ctx)
                    .is_some_and(|component| component.name() == "ext_apps");
                if focused
                    && generation
                        == self
                            .plugin_generation
                    ...
                    && self.model.history_epoch == epoch
                {
                    if let Some(apps) = &self.apps {
            ...
            Action::PluginPromptInsert {
                ...
                if generation
                    ...
                    && self.model.session == session
                    && self.model.history_epoch == epoch
                    && self.slots.prompt_focused(&ctx)
                {
                    self.slots
                        .broadcast(&ctx, "input:clipboard-text", &serde_json::json!(text));
                }
            }
            Action::PreviewHistoryImage => {
                let history = self.backend.history(&self.model.session);
                    ...
                    match &envelope.event {
                        SessionEvent::UserMessage(message) => {
                            for part in &message.content {
                                if let ContentPart::Image { attachment } = part {
                                    references.push(attachment.clone());
                                }
                            }
                        }
                        ...
                        SessionEvent::ToolResult(result) => {
                            for part in &result.content {
                                if let rness_protocol::events::ToolResultContentPart::Image {
                                    attachment,
                                } = part
                                {
                                    references.push(attachment.clone());
                ...
                if references.is_empty() {
                    self.apply(Action::Notice("No images in session history".into()));
                    return;
                }
                self.apply(Action::Custom(
                    "input:history-images".into(),
                    serde_json::to_value(&references).unwrap(),
                ));
                for reference in references {
                    let thumbnail = self
                        .backend
                        .read_image(&self.model.session, &reference.id)
                        .and_then(|bytes| {
                            image::load_from_memory(&bytes).map_err(|e| e.to_string())
                        })
                        .map(|image| {
                            image
                                .thumbnail(160.min(image.width()), 80.min(image.height()))
                                .to_rgba8()
                        });
                    match thumbnail {
            ...
            Action::PasteClipboard => {
                let result: Result<_, String> = (|| {
                    ...
                    if u64::from(width) * u64::from(height) > 40_000_000 {
                        return Err("clipboard image exceeds pixel limit".into());
                    }
                    let image =
                        image::RgbaImage::from_raw(width, height, pixels.bytes.into_owned())
                            .ok_or("invalid clipboard pixels")?;
                    let thumbnail = image::DynamicImage::ImageRgba8(image.clone())
                        .thumbnail(160.min(width), 80.min(height))
                        .to_rgba8();
                    let mut bytes = std::io::Cursor::new(Vec::new());
                    image::DynamicImage::ImageRgba8(image)
                        .write_to(&mut bytes, image::ImageFormat::Png)
                        .map_err(|e| e.to_string())?;
                    let reference = self.backend.admit_image(
                        &self.model.session,
                        bytes.get_ref(),
                        "image/png",
                    )?;
                ...
                match result {
                    ...
                    Err(error) => self.apply(Action::Notice(format!(
                        "Cannot paste from clipboard: {error}"
                    ))),
                }
            }
            Action::SubmitImages(text, images) | Action::SteerImages(text, images) => {
                ...
                content.extend(
                    images
                        .into_iter()
                        .map(|attachment| ContentPart::Image { attachment }),
                );
            ...
            Action::Submit(text) | Action::Steer(text) => {
                ...
                if text.trim() == "/help bindings" {
                    ...
                    if self.slots.modal_active(&ctx) {
                        lines.push(
                            "Host/global mappings below are inactive while this modal owns focus."
                                .into(),
                        );
                    }
                ...
                if text.split_whitespace().next() == Some("/retry") && text.trim() != "/retry" {
                    self.model
                        .entries
                        .push(Entry::Notice("Usage: /retry".into()));
                    return;
                }
                if text.trim() == "/retry" {
                    match self.backend.submit(ClientRequest::Retry {
                        session: self.model.session.clone(),
                    }) {
                        Ok(_) => {
                ...
                if text.split_whitespace().next() == Some("/colorscheme") {
                    ...
                    if words.len() != 2 {
                        self.model
                            .entries
                            .push(Entry::Notice("Usage: /colorscheme <name>".into()));
                    } else if let Some(theme) = self.colorschemes.get(words[1]) {
                        self.theme = theme.clone();
                        self.model
                            .entries
                            .push(Entry::Notice(format!("Colorscheme: {}", words[1])));
                    } else {
                        self.model
                            .entries
                            .push(Entry::Notice(format!("Unknown colorscheme: {}", words[1])));
                    }
                    return;
                }
                ...
                if words.next() == Some("/unload") {
                    if let (Some(name), None, Some(apps)) = (words.next(), words.next(), &self.apps)
                    {
                        ...
                        self.model.entries.push(Entry::Notice(
                            "Usage: /unload <plugin> (requires the local plugin host)".into(),
                        ));
                    }
                    return;
                }
                let content = match self.backend.prepare_input(&self.model.session, &text) {
                    ...
                    Ok(None) => {
                        self.model
                            .entries
                            .push(Entry::Notice("Command completed".into()));
                        return;
                    }
                    Err(error) => {
                        self.model.entries.push(Entry::Notice(error));
                        return;
                    }
                };
                match self.backend.submit(ClientRequest::Send {
            ...
            Action::Custom(name, payload) => {
                ...
                if name == "terminal:copy-text" {
                    let result = payload["text"]
                        .as_str()
                        .ok_or_else(|| "Missing message text".to_owned())
                        .and_then(|text| {
                    ...
                    self.apply(Action::Notice(match result {
                        ...
                        Err(error) => format!("Cannot copy message: {error}"),
                    }));
                    return;
                }
                if name == "terminal:view-message" {
                    let mut payload = payload;
                    payload["snapshot"] = serde_json::json!(true);
...
fn edit_prompt(payload: &serde_json::Value) -> Result<String, String> {
    use std::io::Write;
    let argv = serde_json::from_value::<Vec<String>>(payload["editor"].clone())
        .ok()
        .or_else(|| {
            std::env::var("VISUAL")
    ...
    let result = loop {
        ...
        if let Some(payload) = app.edit_prompt.take() {
            ...
            match edited {
                ...
                Err(error) => app.apply(Action::Notice(format!(
                    "Editor failed; prompt unchanged: {error}"
                ))),
            }
        }
        if app.model.should_quit {
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn message_editor_snapshot_discards_changes() {
        let payload = serde_json::json!({"text":"original", "snapshot":true,
            "editor":["/bin/sh", "-c", "printf edited > \"$1\"", "snapshot"]});
        assert_eq!(edit_prompt(&payload).unwrap(), "");
        let mut prompt = payload;
        prompt["snapshot"] = serde_json::json!(false);
        assert_eq!(edit_prompt(&prompt).unwrap(), "edited");
    }

    #[test]
    fn message_selection_captures_keys_and_preserves_history() {
        ...
        let mut app = App::new(Model::new("s".into(), "m".into()), slots, backend);
        app.model.entries = vec![Entry::User {
        ...
        app.route_key(KeyEvent::new(KeyCode::Char('m'), KeyModifiers::ALT));
        ...
        assert!(rendered.contains("e editor"), "{rendered}");
        let original = app.model.entries.clone();
        app.model.entries.push(Entry::Notice("new arrival".into()));
        app.model.entry_ids.push("message-2".into());
        let actions = app.route_key(KeyEvent::new(KeyCode::Char('e'), KeyModifiers::NONE));
        assert!(
        ...
        assert!(
            matches!(&actions[..], [Action::Custom(name, payload)] if name == "terminal:copy-text" && payload["text"] == "new arrival")
        );
        app.route_key(KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
        assert_eq!(app.model.entries[0], original[0]);
        let ctx = Ctx {
            model: &app.model,
            theme: &app.theme,
        };
        assert!(app.slots.focused_mut(&ctx).is_none());
    }
    ...
    async fn editor_handoff_precedes_reader_reacquisition() {
        ...
        let editor_guard = tokio::time::timeout(Duration::from_secs(2), editor)
            .await
            .unwrap();
        assert!(matches!(
        ...
        drop(editor_guard);
        tokio::time::timeout(Duration::from_secs(2), acquisition)
            .await
            .unwrap()
            .unwrap();
        thread.join().unwrap();
    ...
    #[test]
    fn compaction_frames_show_and_clear_status() {
        let session = SessionId::new();
        let mut model = Model::new(session.clone(), "test".into());
        assert_eq!(
        ...
        let progress = model.compaction.as_ref().unwrap();
        assert_eq!((progress.events, progress.estimated_tokens), (42, 183_000));
        assert_eq!(
            model.apply_frame(&Frame::CompactionFinished {
                session,
                changed: true
            }),
            FrameEffect::None
        );
        assert!(model.compaction.is_none());
    }
    ...
    fn ctrl_c_cancels_admitted_command_before_any_turn_frame() {
        ...
        impl Backend for Pending {
            fn request(&self, _: ClientRequest) {}
            fn command_running(&self, _: &SessionId) -> bool {
                true
            }
            fn history(&self, _: &SessionId) -> History {
                prior_history("s")
        ...
        assert!(!app.model.busy);
        let actions = app.route_key(KeyEvent::new(
    ...
    fn hidden_command_results_survive_switch_and_reconcile_without_duplicates() {
        ...
        assert_eq!(
            app.model
                .entries
                .iter()
                .filter(|entry| **entry == Entry::Notice("saved result".into()))
                .count(),
            1
        );
    ...
    fn command_result_is_notice_without_user_echo() {
        ...
        impl Backend for CommandBackend {
            ...
            fn submit(&self, _: ClientRequest) -> Result<Option<String>, String> {
    ...
    fn rejected_submission_is_notice_not_optimistic_message() {
        ...
        impl Backend for Reject {
            fn request(&self, _: ClientRequest) {
                panic!("must use acknowledged submit");
            }
            fn submit(&self, _: ClientRequest) -> Result<Option<String>, String> {
                Err("engine busy".into())
            }
            fn history(&self, _: &SessionId) -> History {
                prior_history("s")
            }
    ...
    fn unload_command_routes_locally_without_a_user_message() {
        ...
        assert_eq!(
            rx.try_recv().unwrap(),
            crate::modules::ext_apps::AppEvent::Unload("spinner".into())
        );
        assert!(app.model.entries.is_empty());
    ...
    fn provider_errors_are_visible_after_history_reload() {
        ...
        let history = History {
            session: "s".into(),
            envelopes: vec![env(SessionEvent::AssistantAttempt(AssistantAttempt {
                model: "fake".into(),
                chunks: vec![],
                outcome: AttemptOutcome::Error {
                    code: None,
                    retry_in_ms: None,
                    message: "connection closed before message_stop".into(),
                    retryable: true,
                },
            }))],
        };
        model.load_history(&history);
        assert!(
            matches!(&model.entries[0], Entry::Notice(text) if text.contains("Provider error (fake)") && text.contains("connection closed"))
        );
        model.load_history(&history);
    ...
    fn retries_interruptions_and_delayed_reconcile_keep_distinct_identities() {
        ...
        model.apply_frame(&Frame::TurnIdle {
            session: "s".into(),
        });
        ...
        model.apply_frame(&Frame::StepCommitted {
            session: "s".into(),
            event: "target".into(),
        });
        model.apply_frame(&Frame::TurnIdle {
    ...
    fn assistant_identity_survives_removed_history_and_new_commits() {
        ...
        history
            .envelopes
            .push(env(SessionEvent::AssistantMessage(AssistantMessage {
                model: "fake".into(),
    ...
    #[test]
    fn compaction_projection_keeps_messages_and_places_summary_at_checkpoint() {
        ...
        history.envelopes.push(Envelope {
            ...
            event: SessionEvent::Compaction(rness_protocol::events::Compaction {
                replaces: vec!["event-1".into(), "event-2".into()],
        ...
        let checkpoint_index = model
            .entry_ids
            .iter()
            .position(|id| id == "compaction:checkpoint")
            .expect("compaction card");
        assert_eq!(model.entry_ids.len(), model.entries.len());
        assert_eq!(checkpoint_index, model.entries.len() - 1);
    ...
    fn append_projection_visits_only_new_events_and_rebuilds_compaction() {
        ...
        let mut checkpoint = env(SessionEvent::Compaction(
            rness_protocol::events::Compaction {
                replaces: vec!["event-1".into()],
        ...
        model.load_history(&history);
        assert_eq!(
            model.projection_visits,
            visits + 1 + history.envelopes.len()
        );
    ...
    fn switching_back_restores_stream_including_background_deltas() {
        ...
        let mut app = App::new(
            Model::new("s1".into(), "fake".into()),
            Slots::default(),
            backend,
        );
        app.model.busy = true;
        app.model.live_assistant_id = Some(42);
        app.model.live = Some(LiveStep {
            text: "before ".into(),
            ..Default::default()
        });
        app.apply(Action::SwitchSession("s2".into()));
        app.apply_frame(&Frame::Delta {
            session: "s1".into(),
            chunk: ChunkDelta::Text { t: "during".into() },
        });
        ...
        app.apply(Action::SwitchSession("s2".into()));
        app.apply_frame(&Frame::TurnIdle {
            session: "s1".into(),
        });
        app.apply(Action::SwitchSession("s1".into()));
        assert!(!app.model.busy);
    ...
    fn app_default_cannot_replace_escape_but_user_override_can() {
        ...
        let mut binding = ScopedBinding {
            owner: "review".into(),
            scope: Scope::App("review".into()),
            chord: crate::keys::Chord::parse("esc").unwrap(),
            action: "review.inspect".into(),
            layer: BindingLayer::PluginDefault,
        };
        *app.plugin_keymap.write().unwrap() = ScopedKeymap::resolve(&[binding.clone()]).unwrap();
        crate::modules::ext_apps::handle_global_key(
            &apps,
            &KeyEvent::from(crossterm::event::KeyCode::F(6)),
        );
    ...
    fn messagebox_core_precedes_plugin_default_but_not_user_override() {
        ...
        impl Component for MessageControl {
            ...
            fn on_key(&mut self, _: &Ctx<'_>, _: KeyEvent) -> KeyOutcome {
                KeyOutcome::act(vec![Action::ScrollUp(1)])
            }
        }
        let backend = Arc::new(FakeBackend {
            history: prior_history("s1"),
        ...
        let mut binding = ScopedBinding {
            ...
            chord: crate::keys::Chord::parse("f6").unwrap(),
            action: "review.inspect".into(),
            layer: BindingLayer::PluginDefault,
        };
        *app.plugin_keymap.write().unwrap() = ScopedKeymap::resolve(&[binding.clone()]).unwrap();
    ...
    fn scoped_completion_accept_and_app_close_support_remap_and_disable() {
        ...
        *app.plugin_keymap.write().unwrap() = ScopedKeymap::resolve(
            ...
            .into_iter()
            .map(|(scope, key, action)| ScopedBinding {
                owner: "core".into(),
                scope,
    ...
    fn effective_help_does_not_advertise_disabled_core_submit() {
        ...
        *app.plugin_keymap.write().unwrap() = ScopedKeymap::resolve(&[ScopedBinding {
            owner: "core".into(),
            scope: Scope::Promptbox,
            chord: crate::keys::Chord::parse("enter").unwrap(),
            action: "core.promptbox.noop".into(),
            layer: BindingLayer::User,
        }])
        .unwrap();
        app.apply(Action::Submit("/help bindings".into()));
        let Some(Entry::Notice(help)) = app.model.entries.last() else {
            panic!("missing help")
        };
    ...
    fn scoped_core_submit_and_noop_override_editor_defaults() {
        ...
        *app.plugin_keymap.write().unwrap() = ScopedKeymap::resolve(
            &[
                ("enter", "core.promptbox.noop"),
                ("f8", "core.promptbox.submit"),
            ]
            .into_iter()
            .map(|(key, action)| ScopedBinding {
                owner: "core".into(),
        ...
        assert!(app.route_key(KeyEvent::from(KeyCode::Enter)).is_empty());
        assert!(
            matches!(app.route_key(KeyEvent::from(KeyCode::F(8))).as_slice(), [Action::Submit(text)] if text == "x")
        );
        assert!(rx.try_recv().is_err());
    }
    ...
    fn binding_help_reports_resolved_actions_without_submitting() {
        ...
        *app.plugin_keymap.write().unwrap() = ScopedKeymap::resolve(&[ScopedBinding {
            owner: "review".into(),
            scope: Scope::Promptbox,
            chord: crate::keys::Chord::parse("f8").unwrap(),
            action: "review.inspect".into(),
            layer: BindingLayer::User,
        }])
        .unwrap();
        app.apply(Action::Submit("/help bindings".into()));
        assert!(
            matches!(app.model.entries.last(), Some(Entry::Notice(text)) if text.contains("review.inspect") && text.contains("Promptbox"))
        );
        assert!(!app.model.busy);
    }
    ...
    fn plugin_defaults_preserve_editor_input_but_user_mapping_overrides() {
        ...
        let mut binding = ScopedBinding {
            owner: "review".into(),
            scope: Scope::Promptbox,
            chord: crate::keys::Chord::parse("j").unwrap(),
            action: "review.inspect".into(),
            layer: BindingLayer::PluginDefault,
        };
        *app.plugin_keymap.write().unwrap() = ScopedKeymap::resolve(&[binding.clone()]).unwrap();
    ...
    fn plugin_prompt_action_dispatch_and_stale_result_guard() {
        ...
        *app.plugin_keymap.write().unwrap() = ScopedKeymap::resolve(&[ScopedBinding {
        ...
        app.apply(Action::PluginPromptInsert {
        ...
        let actions = app.route_key(KeyEvent::from(KeyCode::Enter));
        assert!(
            matches!(actions.as_slice(), [Action::Submit(text)] if text == "review"),
            "{actions:?}"
        );
    }
    ...
    fn non_input_transitions_expire_pending_prompt_operations() {
        use crossterm::event::KeyCode;
        for transition in [
            Action::SwitchSession("s1".into()),
            Action::Custom("input:prompt-edited".into(), serde_json::json!({"text":""})),
            Action::ResolveApproval(rness_protocol::api::ApprovalDecision::Allowed),
        ] {
            let mut slots = Slots::default();
            crate::modules::input::install(&mut slots);
            let mut app = App::new(
    ...
    fn global_user_mapping_cannot_capture_prompt_typing() {
        ...
        *app.plugin_keymap.write().unwrap() = ScopedKeymap::resolve(&[ScopedBinding {
        ...
        assert!(rx.try_recv().is_err());
        assert!(
            matches!(app.route_key(KeyEvent::from(KeyCode::Enter)).as_slice(), [Action::Submit(text)] if text == "j")
        );
    }
    ...
    fn pending_approval_routes_keys_to_overlay_and_resolves() {
        ...
        assert!(
            matches!(
                actions[..],
                [Action::ResolveApproval(ApprovalDecision::Allowed)]
            ),
        ...
        assert!(
            app.model.pending_approval.is_none(),
            "resolved clears the question"
        );
