...
pub fn install(slots: &mut Slots) -> CardCache {
    ...
    slots.mount(
        ...
        Box::new(Chat {
            ...
            session: None,
            thinking_expanded: Default::default(),
            card_rows: Default::default(),
            total_rows: 0,
            viewport_height: 0,
...
fn row_bytes(rows: &Vec<Line<'_>>) -> usize {
    rows.capacity() * std::mem::size_of::<Line<'_>>()
        + rows
            .iter()
            .map(|row| {
...
fn card_rows_limited(
    ...
    let source_bytes = ["before", "after", "text"]
        .iter()
        .map(|key| block[*key].as_str().map_or(0, str::len))
        .fold(0usize, usize::saturating_add);
    if source_bytes > 256 * 1024 {
        return vec![Line::styled(
            "Block omitted: snapshot exceeds rendering limit",
            theme.dim,
        )];
    }
    let language = if block["syntax_highlight"] == false {
        ""
    } else {
    ...
    let mut push = |text: &str,
        ...
        for code in highlighted {
            let gutter = if block["line_numbers"] == false {
                ...
                format!("{number:>4} {sign}")
            };
            let gutter_style = theme
                .resolve_style(&block["styles"]["gutter"], theme.dim)
                .unwrap_or(theme.dim);
    ...
    if block["kind"] == "diff" {
        ...
        if before.len().saturating_add(after.len()) > 256 * 1024 {
            return vec![Line::styled(
                "Diff omitted: snapshot exceeds rendering limit",
                theme.dim,
            )];
        }
        ...
        for (group_index, group) in diff.grouped_ops(context).iter().enumerate() {
            ...
            for op in group {
                for change in diff.iter_changes(op) {
                    ...
                    let style = theme
                        .resolve_style(&block["styles"][name], fallback)
                        .unwrap_or(fallback);
                    let colored = if change.tag() == similar::ChangeTag::Delete {
                        before_colors
                            .as_ref()
                            .and_then(|lines| change.old_index().and_then(|i| lines.get(i)))
                    } else {
                        after_colors
                            .as_ref()
                            .and_then(|lines| change.new_index().and_then(|i| lines.get(i)))
                    };
                    push(
            ...
            .enumerate()
        {
            push(
                ...
                "",
                theme.code_block,
                after_colors.as_ref().and_then(|lines| lines.get(index)),
            );
        }
    }
    result
...
fn card_line(
    ...
    let decode = |span: rness_kernel::presentation::StyledSpan| {
        Span::styled(
            sanitize(&span.text),
            theme.resolve_style(&span.style, base).unwrap_or(base),
        )
    };
    let mut left = if row.spans.is_empty() {
        vec![Span::styled(sanitize(&row.text), base)]
    } else {
        row.spans.into_iter().map(decode).collect()
    };
    ...
    if !right.is_empty() {
        ...
        clipped.push(Span::styled(
            " ".repeat(usize::from(width).saturating_sub(used + reserve)),
            base,
        ));
...
fn panel_inner_width(options: &serde_json::Value, width: u16) -> u16 {
    let width = if options["border"]["kind"].as_str().unwrap_or("none") != "none" && width >= 2 {
        width - 2
    } else {
        width
    };
    ...
    let gutter = if marker.is_empty()
        || marker_width + 1 >= usize::from(width).saturating_sub(left + right)
    {
        0
    } else {
        marker_width + 1
    ...
    usize::from(width)
        .saturating_sub(left + right + gutter)
        .max(1) as u16
}
...
fn panel(
    ...
    width: u16,
    theme: &crate::theme::Theme,
    inherited: ratatui::style::Style,
) -> Vec<Line<'static>> {
    let kind = options["border"]["kind"].as_str().unwrap_or("none");
    if kind == "none" || width < 2 {
        return panel_body(content, options, width, theme, inherited);
    }
    let style = theme
        .resolve_style(&options["style"], inherited)
        .unwrap_or(inherited);
    let border = theme
        .resolve_style(&options["border"]["style"], style)
        .unwrap_or(style);
    ...
    let mut out = vec![Line::styled(
        format!("{tl}{}{tr}", h.repeat(usize::from(width - 2))),
        border,
    )];
    ...
    out.push(Line::styled(
        format!("{bl}{}{br}", h.repeat(usize::from(width - 2))),
        border,
    ));
    out
}
...
fn panel_body(
    ...
    let style = if options["style"].is_null() {
        inherited
    } else {
        theme
            .resolve_style(&options["style"], inherited)
    ...
    let marker_style = theme
        .resolve_style(&options["marker"]["style"], style)
        .unwrap_or(style);
    ...
    for _ in 0..padding("top") {
        output.push(Line::styled(" ".repeat(width.into()), style));
    }
    let mut row_index = 0;
    for line in content {
        for row in visual_rows(line, inner, true) {
            let row = row.spans.into_iter().map(|span| {
...
fn message_text(entry: &Entry) -> String {
    match entry {
        Entry::User { content, .. } | Entry::Assistant { content, .. } => content
            ...
            .collect::<Vec<_>>()
            .join("\n\n"),
        Entry::ToolResult { output, .. } => output.clone(),
        Entry::Compaction { summary, .. } => summary.clone(),
        Entry::Notice(text) => text.clone(),
    }
}

const SELECTION_KEYS: &[(&str, &[&str], &str)] = &[
    ("selection_previous", &["k", "up"], "previous"),
    ("selection_next", &["j", "down"], "next"),
    ("selection_scroll_up", &["ctrl+u", "pageup"], "scroll up"),
    (
...
impl Chat {
    ...
    fn selection_keys(&self, action: &str, defaults: &[&str]) -> Vec<String> {
        match &self.config["keys"][action] {
            ...
            _ => defaults.iter().map(|key| (*key).to_owned()).collect(),
        }
    }
}
...
impl Component for Chat {
    ...
    fn on_key(
        ...
        if let Some(id) = self.selected_message.clone() {
            ...
            match selected_action {
                Some("selection_close") => self.selected_message = None,
                Some("selection_previous") => {
                    self.selected_message = Some(targets[index.saturating_sub(1)].1.clone())
                }
                Some("selection_next") => {
                    self.selected_message =
                        Some(targets[(index + 1).min(targets.len() - 1)].1.clone())
                }
                Some("selection_scroll_down") => {
                ...
                Some("selection_toggle") => {
                    if let Entry::ToolResult { call, .. } = &ctx.model.entries[targets[index].0] {
                        let expanded = self.expanded.get(call).copied().unwrap_or(false);
                        self.expanded.insert(call.clone(), !expanded);
                    } else if matches!(
                        &ctx.model.entries[targets[index].0],
                        Entry::Compaction { .. }
                    ) {
                        let expanded = self
                            .expanded
                            .get(&id)
                            .copied()
                            .unwrap_or(self.config["compaction"]["display"] == "expanded");
                        self.expanded.insert(id.clone(), !expanded);
                    } else {
                ...
                Some("selection_copy" | "selection_editor") => {
                    let text = message_text(&ctx.model.entries[targets[index].0]);
                    let action = if selected_action == Some("selection_copy") {
                        "terminal:copy-text"
                    } else {
                    ...
                    return KeyOutcome::act(vec![Action::Custom(
                        action.into(),
                        serde_json::json!({"text":text,"editor":self.config["editor"]}),
                    )]);
                }
                _ => {}
            }
            if self.selected_message.as_ref() != Some(&id) {
                self.message_scroll = 0;
            }
            return KeyOutcome::consumed();
        }
        let action = self
            .bindings()
            .into_iter()
            .find(|binding| binding.matches_key(&key))
            .map(|binding| binding.action);
        action.map_or_else(crate::component::KeyOutcome::pass, |action| {
            self.on_binding(ctx, action)
        })
    }
    ...
    fn on_binding(&mut self, ctx: &Ctx<'_>, action: &str) -> crate::component::KeyOutcome {
        ...
        if matched("toggle_thinking") || previous_thinking || next_thinking {
            let mut targets: Vec<_> = ctx
                ...
                .filter_map(|(entry_index, entry)| match entry {
                    Entry::Assistant { content, .. } => Some((entry_index, content)),
                    _ => None,
                })
                .enumerate()
                .flat_map(|(ordinal, (entry_index, content))| {
                    ...
                    content
                        ...
                        .enumerate()
                        .map(move |(thinking_index, _)| (assistant_index, thinking_index))
                })
                .collect();
            if ctx
                .model
                .live
                .as_ref()
                .is_some_and(|live| !live.thinking.is_empty())
            {
                targets.push((
                    ctx.model.live_assistant_id.unwrap_or_else(|| {
                        ctx.model.next_assistant_id.max(
                            ctx.model
                                .entries
                                .iter()
            ...
            if targets.is_empty() || self.config["thinking"]["visible"] == false {
                return KeyOutcome::pass();
            }
            let current = targets
                .iter()
                .position(|target| Some(*target) == self.focused_thinking);
            ...
            if !previous_thinking && !next_thinking {
                let expanded = self
                    .thinking_expanded
                    .entry(targets[index])
                    .or_insert(self.config["thinking"]["display"] == "expanded");
                *expanded = !*expanded;
            }
            return KeyOutcome::consumed();
        }
        ...
        let current = calls
            .iter()
            .position(|(call, _, _)| self.focused_call.as_ref() == Some(call));
        let index = match current {
            None => calls.len() - 1,
        ...
        if toggle {
            ...
            let options = merge_options(
                &options,
                &options["states"][if ctx.model.cancelled_tools.contains(call) {
                    "cancelled"
                } else if is_error {
                    "error"
                } else {
        ...
        if let Some(row) = self.card_rows.get(call) {
            let target = if toggle {
                let mut probe = Chat {
                    ...
                    focused_call: self.focused_call.clone(),
                    expanded: self.expanded.clone(),
                    session: self.session.clone(),
                    thinking_expanded: self.thinking_expanded.clone(),
                    card_rows: Default::default(),
                    total_rows: 0,
                    viewport_height: 0,
                ...
                probe.total_rows.saturating_sub(
                    probe
                        .card_rows
                        .get(call)
                        .copied()
            ...
            let action = if target >= current {
                ...
                crate::app::Action::ScrollDown(current - target)
            };
    ...
    fn on_action(&mut self, _ctx: &Ctx<'_>, name: &str, payload: &serde_json::Value) {
        if name == "chat:messagebox-config" {
    ...
    fn render(&mut self, ctx: &Ctx<'_>, area: Rect, buf: &mut Buffer) {
        ...
        if self
            .selected_message
            .as_ref()
            .is_some_and(|id| !ctx.model.entry_ids.contains(id))
        {
            self.selected_message = None;
        }
        if self.session.as_ref() != Some(&ctx.model.session) {
        ...
        if self.config.is_object() {
            ...
            let style = ctx
                .theme
                .resolve_style(&self.config["style"], ratatui::style::Style::default())
                .unwrap_or_default();
            ...
            if kind != "none" {
                let border_style = ctx
                    .theme
                    .resolve_style(&self.config["border"]["style"], style)
                    .unwrap_or(style);
            ...
            let pad =
                |side: &str| self.config["padding"][side].as_u64().unwrap_or(0).min(64) as u16;
            let left = pad("left").min(render_area.width);
            let top = pad("top").min(render_area.height);
            render_area.x += left;
            render_area.y += top;
            render_area.width = render_area
                .width
                .saturating_sub(left)
            ...
            render_area.height = render_area
                .height
                .saturating_sub(top)
                .saturating_sub(pad("bottom"));
        }
        let area = if self.selected_message.is_some() && render_area.height > 1 {
            use ratatui::widgets::{Paragraph, Widget};
            let hint = SELECTION_KEYS
                .iter()
                .filter_map(|(action, defaults, label)| {
                    self.selection_keys(action, defaults)
                        .first()
                ...
                .collect::<Vec<_>>()
                .join(" · ");
            Paragraph::new(hint).style(ctx.theme.dim).render(
                Rect::new(render_area.x, render_area.y, render_area.width, 1),
                buf,
            );
            Rect::new(
                render_area.x,
        ...
        let marker = self.config["selection"]["marker"]["text"]
            .as_str()
            .unwrap_or("▎");
        let gutter =
            if self.selected_message.is_some() && self.config["selection"]["marker"] != false {
                (unicode_width::UnicodeWidthStr::width(marker) as u16)
                    .saturating_add(1)
        ...
        let interaction = format!(
            "{:?}:{:?}:{:?}:{:?}",
            self.focused_call, self.expanded, self.focused_thinking, self.thinking_expanded
        );
        ...
        let dirty = if selective {
            self.cards
                .changes_since(self.cache_card_revision)
                .and_then(|calls| {
                    calls
                        .iter()
                        .all(|call| self.cached_calls.contains_key(call))
                        .then_some(calls)
                })
        } else {
            None
        ...
        if !unchanged {
                ...
                if let Entry::Assistant { content, .. } = entry {
                    for part in content {
                        if let ContentPart::ToolUse { call, args, .. } = part {
                            self.cache_args.insert(call.clone(), args.clone());
                        }
                    }
                }
            }
            ...
            if append_only {
                ...
                let mut by_id: std::collections::HashMap<_, _> =
                    self.cached_ids.iter().cloned().zip(old).collect();
                self.entry_cache = ctx
                    .model
                    .entry_ids
                    .iter()
                    .enumerate()
            ...
            let mut indices: Vec<usize> = if let Some(calls) = dirty {
                let mut indices: Vec<_> = calls
                    .iter()
                    .filter_map(|call| self.cached_calls.get(call).copied())
                    .collect();
                for call in self.expanded.keys().chain(self.cache_expanded.keys()) {
                    if self.expanded.get(call) != self.cache_expanded.get(call) {
                        indices.extend(
                            self.cached_calls
                                .get(call)
                                .or_else(|| self.cached_positions.get(call))
                                .copied(),
                        );
                    }
                }
                if self.focused_call != self.cache_focused_call {
                    for call in self
                        .focused_call
                        .iter()
                        .chain(self.cache_focused_call.iter())
                    {
                ...
                for key in self
                    .thinking_expanded
                    .keys()
                    .chain(self.cache_thinking_expanded.keys())
                {
                    if self.thinking_expanded.get(key) != self.cache_thinking_expanded.get(key) {
                        indices.extend(self.cached_assistants.get(&key.0).copied());
                    }
                }
                if self.focused_thinking != self.cache_focused_thinking {
                    for key in self
                        .focused_thinking
                        .iter()
                        .chain(self.cache_focused_thinking.iter())
                    {
                        indices.extend(self.cached_assistants.get(&key.0).copied());
                    }
                }
                        ...
                        if configured {
                            if let Entry::Compaction { summary, shadowed } = entry {
                                ...
                                let header_style = theme
                                    .resolve_style(&options["header"]["style"], theme.tool_name)
                                    .unwrap_or(theme.tool_name);
                                let mut body = vec![Line::styled(
                                    format!("Conversation summary · {shadowed} earlier events"),
                                    header_style,
                                )];
                                body.extend(crate::core::render::render_markdown_configured(
                                    summary,
                                    inner_width,
                                    theme,
                                ...
                                let limit = match options["display"].as_str().unwrap_or("preview") {
                                    ...
                                    "expanded" => usize::MAX,
                                    _ => {
                                        options["preview_lines"].as_u64().unwrap_or(6).min(10000)
                                            as usize
                                            + 1
                                    }
                                };
                                ...
                                for line in body {
                                    ...
                                    rows.extend(visual_rows_limited(
                                        line,
                                ...
                                lines.extend(panel(
                                    ...
                                    width,
                                    theme,
                                    background.patch(theme.tool_output),
                                ));
                            ...
                            let role = match entry {
                                ...
                                Entry::Notice(_) => {
                                    Some(if ctx.model.error_entries.contains(&entry_index) {
                                        "error"
                                    } else {
                                        "notice"
                                    })
                            ...
                            if let Some(role) = role {
                                let defaults = if role == "user" {
                                    serde_json::json!({"marker":{"text":">","mode":"first_line","style":"user_prefix"}})
                                } else {
                                    serde_json::json!({})
                                };
                                let options = merge_options(
                                ...
                                _ => {},
                                    ...
                                    Entry::Compaction { summary, shadowed } => {
                                        body.push(Line::styled(format!("── {shadowed} earlier events compacted into a summary ──"), theme.dim));
                                        body.push(Line::default());
                                        body.extend(
                                            crate::core::render::render_markdown_configured(
                                                summary,
                                                inner_width,
                                                theme,
                                                &options["markdown"],
                                    ...
                                    Entry::Notice(text) => {
                                        body.extend(text.lines().map(|s| Line::raw(sanitize(s))))
                                    }
                                    _ => {}
                                }
                                let inherited = background.patch(match role {
                                    "user" => theme.user_message,
                                    "error" => theme.error,
                                    "notice" => theme.dim,
                                ...
                                if !body.is_empty() {
                                    lines.extend(panel(
                                        message_rows(body, &options, width),
                                        &options,
                                        width,
                                        theme,
                                        inherited,
                                    ));
                                    for _ in 0..self.config["spacing"].as_u64().unwrap_or(1).min(64)
                                    {
                        ...
                        match entry {
                            ...
                            Entry::ToolResult {
                                ...
                                if configured {
                                    ...
                                    if self.expanded.get(call) == Some(&true) {
                                        options["display"] = serde_json::json!("expanded");
                                    }
                                    if self.focused_call.as_ref() == Some(call) {
                                        options["marker"] = serde_json::json!({"text":">","mode":"first_line","style":"title"});
                                    }
                                    let inherited = background.patch(if *is_error {
                                        theme.error
                                    } else {
                                        theme.tool_output
                                    });
                                    let header_rows =
                                        if let Some(card) = self.cards.get(call) {
                                            usize::from(card.first().is_some_and(|row| {
                                                row.is_header || !row.structured
                                            }))
                                        } else {
                                            usize::from(options["header"]["visible"] != false)
                                        };
                                    let limit =
                                        match options["display"].as_str().unwrap_or("preview") {
                                    ...
                                    let mut body = if let Some(card) = self.cards.get(call) {
                                        ...
                                        let row_limit = if options["display"] == "collapsed" {
                                            1
                                        } else {
                                            usize::MAX
                                        };
                                        ...
                                        if options["header"]["visible"] != false {
                                            ...
                                            if options["header"]["show_status"] == true {
                                                title.push_str(
                                                    if ctx.model.cancelled_tools.contains(call) {
                                                        " · cancelled"
                                                    } else if *is_error {
                                                        " · error"
                                                    } else {
                                            ...
                                            if options["header"]["show_duration"] == true {
                                                if let Some(ms) = ctx.model.tool_durations.get(call)
                                                {
                                        ...
                                        if options["arguments"]["visible"] == true {
                                            if let Some(args) = tool_args.get(call) {
                                                let style = theme
                                                    .resolve_style(
                                                        &options["arguments"]["style"],
                                                        theme.dim,
                                                    )
                                                    .unwrap_or(theme.dim);
                                                rows.extend(visual_rows(
                                                    Line::styled(
                                                        sanitize(&args.to_string()),
                                                        style,
                                                    ),
                                                    usize::from(panel_inner_width(&options, width)),
                                                    options["arguments"]["wrap"] != false,
                                                ));
                                            }
                                        }
                                        if options["output"]["visible"] != false {
                                            ...
                                            rows.extend(
                                                output
                                                    ...
                                                    .map(|s| Line::styled(sanitize(s), style)),
                                            );
                                        }
                                        rows
                                    };
                                    ...
                                    for (index, row) in body.into_iter().enumerate() {
                                        let remaining =
                                            limit.saturating_add(1).saturating_sub(laid_out.len());
                                        if remaining == 0 {
                                            break;
                                        }
                                        laid_out.extend(visual_rows_limited(
                                            row,
                                            inner,
                                            index < header_rows
                                                || options["output"]["wrap"] != false,
                                            remaining,
                                        ));
                                    }
                                    body = laid_out;
                                    if body.len() > limit {
                                        body.truncate(limit);
                                        if limit > 1 {
                                            body.push(Line::styled("… more lines", theme.dim));
                                        }
                                    }
                            ...
                            Entry::Compaction { summary, shadowed } => {
                                ...
                                lines.push(Line::styled(
                                    format!(
                                        "── {shadowed} earlier events compacted into a summary ──"
                                    ),
                                    theme.dim,
                                ));
                                lines.push(Line::default());
                                lines.extend(render_markdown(summary, width, theme));
                            }
                            Entry::Notice(text) => {
                                for line in text.lines() {
                                    for wrapped in
                                        textwrap::wrap(&sanitize(line), usize::from(width.max(1)))
                                    {
        ...
        if let Some(live) = &ctx.model.live {
            ...
            if !live.thinking.is_empty() && self.config["thinking"]["visible"] != false {
                if configured {
                    let mut options = merge_options(
                        &serde_json::json!({"label":{"text":"Thinking"},"style":"thinking"}),
                        &self.config["thinking"],
                    );
                    let key = (
                        ctx.model
                            .live_assistant_id
                            .unwrap_or(ctx.model.next_assistant_id.max(assistant_count)),
                        0,
                    );
                    if self.focused_thinking == Some(key) {
                        options["marker"] =
                            serde_json::json!({"text":">","mode":"first_line","style":"title"});
                    }
                    let expanded = self
                        .thinking_expanded
                        .get(&key)
                        .copied()
                        .unwrap_or(options["display"] == "expanded");
                    let limit = if expanded {
                        usize::MAX
                    } else if options["display"] == "collapsed" {
                        0
                    } else {
                        options["preview_lines"].as_u64().unwrap_or(3) as usize
                    };
                    let inner = panel_inner_width(&options, width);
                    let rows = live
                        .thinking
            ...
            if !live.text.is_empty() {
                ...
                let body = crate::core::render::render_markdown_configured(
                    &live.text,
                    if configured {
                        panel_inner_width(&options, width)
                    } else {
                        width
                    },
                    theme,
                ...
                if configured {
                    if options["visible"] != false {
                        lines.extend(panel(
                            ...
                            width,
                            theme,
                            background.patch(theme.assistant_text),
                        ));
                    }
                } else {
                    lines.extend(body);
            ...
            for (call, name) in &live.running_tools {
                self.card_rows
                    .insert(call.clone(), durable_rows + lines.len());
                if configured {
                    let options = merge_options(&self.config["tool"], &self.config["tools"][name]);
                    ...
                    if let Some(expanded) = self.expanded.get(call) {
                        options["display"] =
                            serde_json::json!(if *expanded { "expanded" } else { "collapsed" });
                    }
                    if let Some(card) = self.cards.get(call) {
                        let inner = panel_inner_width(&options, width);
                        let limit = match options["display"].as_str() {
                        ...
                        let body = card
                            .iter()
                            .take(limit)
                            .flat_map(|row| card_rows(row.clone(), inner, theme))
                            .collect();
        ...
        let selected_index = self
            .selected_message
            .as_ref()
            .and_then(|id| ctx.model.entry_ids.iter().position(|target| target == id));
        ...
        self.live_scroll_anchor = if ctx.model.scroll_from_bottom != 0
            && start >= durable_rows
            && ctx.model.live.is_some()
        {
        ...
        self.scroll_anchor = if ctx.model.scroll_from_bottom != 0 {
            self.cached_ids.get(first).map(|id| {
                (
                    id.clone(),
                    start.saturating_sub(if first == 0 {
                        0
                    } else {
                        self.row_ends[first - 1]
                    }),
                )
            })
        } else {
        ...
        for index in first..self.entry_cache.len() {
            ...
            for (row_index, row) in rows
                .iter()
                .enumerate()
                .take(end.saturating_sub(base).min(rows.len()))
                .skip(start.saturating_sub(base).min(rows.len()))
            {
                let mut row = row.clone();
                if selected_index == Some(index) {
                    let fallback = ratatui::style::Style::default()
                        .bg(ratatui::style::Color::Rgb(0x45, 0x40, 0x3a));
                    row = row.patch_style(
                        theme
                            .resolve_style(&self.config["selection"]["style"], fallback)
                            .unwrap_or(fallback),
                    );
                    if !self.message_expanded
                        && row_index >= 3
                        && !matches!(ctx.model.entries[index], Entry::ToolResult { .. })
                    {
                        continue;
                    }
                    selected_rows.push(visible.len());
                }
                visible.push(row);
            }
        }
        if end > durable_rows {
            visible.extend(
                lines[start.saturating_sub(durable_rows)..end - durable_rows]
                    .iter()
                    .cloned(),
            );
        }
        ...
        if gutter > 0 {
            let style = theme
                .resolve_style(
                    &self.config["selection"]["marker"]["style"],
                    theme.tool_name,
                )
                .unwrap_or(theme.tool_name);
            let top = area.y + area.height.saturating_sub(visible.len() as u16);
        ...
        let budget = self.config["cache_bytes"]
            .as_u64()
        ...
        if self.retained_bytes > budget {
            ...
            candidates.sort_unstable_by_key(|i| {
                std::cmp::Reverse(if *i < first {
                    first - *i
                } else {
                    i.saturating_sub(last.saturating_sub(1))
                })
            });
...
mod tests {
    ...
    fn selected_compaction_toggle_refreshes_without_leaving_selection() {
        ...
        use crate::{app::Model, theme::Theme};
        use crossterm::event::{KeyCode, KeyEvent, KeyModifiers};
        let mut model = Model::new("compaction".into(), "fake".into());
        for entry in [
            Entry::Compaction {
                summary: "Preview text\n\nExpanded detail".into(),
                shadowed: 20,
            },
            Entry::ToolResult {
                call: "tool-call".into(),
    ...
    #[test]
    fn subagent_selection_has_no_stop_key_even_with_legacy_config() {
        use super::*;
        use crate::{app::Model, theme::Theme};
    ...
    #[test]
    fn inline_selection_preserves_cards_and_surrounding_messages() {
        use super::*;
        use crate::{app::Model, theme::Theme};
        let mut model = Model::new("inline".into(), "fake".into());
        model
            .entries
            .push(Entry::Notice("Earlier conversation".into()));
        model.entries.push(Entry::ToolResult {
            call: "tasks".into(),
            name: "TaskWrite".into(),
            output: "{\"tasks\":[]}".into(),
        ...
        model.entry_ids = vec!["earlier".into(), "task-result".into()];
        let theme = Theme::default();
        let mut chat = Chat {
            config: serde_json::json!({}),
            ..Default::default()
        ...
        chat.cards.insert(
            ...
            vec![rness_kernel::presentation::StyledLine {
                text: "Tasks: readable checklist".into(),
                ..Default::default()
            }],
        );
        ...
        assert_eq!(chat.message_scroll, 10);
        assert_eq!(chat.selected_message.as_deref(), Some("task-result"));
        chat.on_key(
            &ctx,
        ...
        assert_eq!(chat.selected_message.as_deref(), Some("earlier"));
        chat.config["keys"] =
            serde_json::json!({"selection_scroll_down":"ctrl+f", "selection_copy":false});
        chat.on_key(
            &ctx,
            KeyEvent::new(KeyCode::Char('d'), KeyModifiers::CONTROL),
        );
        ...
        assert_eq!(chat.message_scroll, 10);
        assert!(chat.selection_keys("selection_copy", &["y"]).is_empty());
        let mut remapped = Buffer::empty(area);
        chat.render(&ctx, area, &mut remapped);
        let hint: String = remapped.content.iter().map(|cell| cell.symbol()).collect();
        assert!(hint.contains("ctrl+f scroll down"));
        ...
        assert!(!disabled.content.iter().any(|cell| cell.symbol() == ">"));
    }
    ...
    fn bounded_rows_match_unbounded_after_eviction_scroll_and_resize() {
        ...
        for i in 0..150 {
            model.entries.push(Entry::Notice(format!(
                "entry {i}: {}",
                "é 漢字 ".repeat(if i == 70 { 3000 } else { 8 })
            )));
        ...
        for budget in [0, 1024, 8192] {
            ...
            for width in [40, 60, 20] {
                for offset in [0, 40, 1000, 50, 0, 400] {
                    ...
                    assert_eq!(
                        actual, expected,
                        "budget={budget}, width={width}, offset={offset}"
                    );
                    assert!(
                        bounded
                            .entry_cache
                            .iter()
                            .map(|entry| row_bytes(&entry.2))
                            .sum::<usize>()
                            <= budget
                    );
    ...
    fn configured_terminal_states_visibility_and_modes_match_fresh_render() {
        ...
        for state in ["success", "error", "cancelled"] {
            if let Entry::ToolResult { is_error, .. } = &mut model.entries[0] {
                *is_error = state != "success";
            }
            model.cancelled_tools.clear();
            if state == "cancelled" {
                model.cancelled_tools.insert("call".into());
            }
    ...
    fn durable_cache_survives_removed_preceding_entry() {
        ...
        model.entries = vec![
            Entry::Notice("first".into()),
            Entry::Notice("retained".into()),
        ];
    ...
    fn virtual_viewport_reuses_entries_and_matches_flat_render() {
        ...
        for offset in [0, 1, 500, 990, 1000] {
            ...
            let flat: Vec<_> = chat
                .entry_cache
                .iter()
                .flat_map(|entry| entry.2.iter().cloned())
                .collect();
            let mut expected = Buffer::empty(area);
        ...
        assert_eq!(pointer, chat.entry_cache[0].2.as_ptr());
        assert_eq!(
            chat.entry_cache[999].0,
            entry_fingerprint(&model.entries[999])
        );
    ...
    fn code_cards_preserve_multiline_syntax_state_and_allow_disabling_it() {
        ...
        let row = rness_kernel::presentation::StyledLine {
            block: Some(
                serde_json::json!({"kind":"code","text":source,"language":"rust","line_numbers":false}),
            ),
            ..Default::default()
        };
    ...
    fn error_visibility_is_independent_of_notice_visibility() {
        ...
        let mut chat = Chat {
            config: serde_json::json!({"spacing":0,"error":{"visible":false}}),
            ..Default::default()
        };
        let area = Rect::new(0, 0, 40, 5);
        let theme = Theme::default();
    ...
    fn configured_thinking_renders_markdown_before_previewing() {
        ...
        let theme = Theme::default();
        let mut chat = Chat {
            config: serde_json::json!({"thinking":{"visible":true,"display":"preview","preview_lines":3}}),
            ..Default::default()
        };
        ...
        let rendered = chat.entry_cache[0]
            .2
            .iter()
    ...
    fn message_display_limits_visual_rows_without_mutating_content() {
        ...
        assert_eq!(
            message_rows(
                original.clone(),
                &serde_json::json!({"display":"expanded"}),
                5
            ),
            original
        );
    ...
    fn oversized_right_header_stays_inside_card() {
        ...
        let row = rness_kernel::presentation::StyledLine {
            text: "left".into(),
            right: vec![rness_kernel::presentation::StyledSpan {
                text: "long status".into(),
    ...
    fn builtin_card_displays_requested_arguments_and_duration() {
        ...
        let mut chat = Chat {
            config: serde_json::json!({"tool":{"display":"expanded","header":{"show_duration":true},"arguments":{"visible":true}}}),
            ..Default::default()
        };
        let theme = Theme::default();
    ...
    fn terminal_sanitizer_handles_osc_and_nonletter_csi() {
        assert_eq!(
            sanitize("\u{1b}]8;;https://example.com\u{1b}\\link\u{1b}]8;;\u{1b}\\"),
            "link"
        );
    ...
    fn terminal_tool_visibility_and_cancelled_state_are_applied() {
        ...
        let mut buf = Buffer::empty(area);
        let mut chat = Chat {
            config: serde_json::json!({"tool":{"header":{"show_status":true},"states":{"cancelled":{"display":"collapsed"}}}}),
            ..Default::default()
        };
        chat.render(
            &Ctx {
    ...
    fn hidden_headers_do_not_consume_visual_preview_budget() {
        ...
        model.entries.push(Entry::ToolResult {
            call: "c".into(),
            name: "Read".into(),
            output: "abcdefghijklmnop".into(),
            is_error: false,
        });
        let theme = Theme::default();
        let mut chat = Chat {
            config: serde_json::json!({"tool":{"header":{"visible":false},"preview_lines":1}}),
            ..Default::default()
        };
        ...
        assert_eq!(chat.total_rows, 1);
        assert_eq!(
            (0..4).map(|x| buf[(x, 9)].symbol()).collect::<String>(),
            "abcd"
        );
    ...
    fn message_background_fills_padding_and_wrapped_rows() {
        ...
        for y in 0..3 {
            for x in 0..8 {
                assert_eq!(buf[(x, y)].bg, Color::Rgb(60, 56, 54));
            }
        }
        assert_eq!(
            (0..8).map(|x| buf[(x, 1)].symbol()).collect::<String>(),
            " abcdef "
        );
        assert_eq!(
            (0..8).map(|x| buf[(x, 2)].symbol()).collect::<String>(),
            " ghijk  "
        );
    }
    ...
    fn running_subagent_cards_expand_and_update_without_switching_sessions() {
        ...
        model.live = Some(LiveStep {
            running_tools: vec![
                ("a".into(), "subagent".into()),
                ("b".into(), "subagent".into()),
            ],
            ..Default::default()
        });
        let theme = Theme::default();
        ...
        let mut chat = Chat {
            config: serde_json::json!({"tool":{"display":"collapsed"}}),
    ...
    fn navigation_returns_scroll_action_and_thinking_toggle_is_reversible() {
        ...
        let mut chat = Chat {
            cards: CardCache::default(),
            config: serde_json::json!({"tool":{}}),
            focused_call: None,
            expanded: Default::default(),
            session: Some("qa".into()),
            thinking_expanded: Default::default(),
            card_rows: std::collections::HashMap::from([("a".into(), 10)]),
            total_rows: 100,
            viewport_height: 20,
            focused_thinking: None,
            last_area: Rect::default(),
    ...
    fn diff_blocks_keep_offsets_and_full_width_backgrounds() {
        ...
        assert_eq!(
            ...
            Some(ratatui::style::Color::Rgb(41, 75, 45))
        );
    }
        ...
        for width in [24, 60, 100] {
            ...
            for id in ["first", "last"] {
                model.entries.push(Entry::ToolResult {
                    call: id.into(),
                    name: id.into(),
                    output: (0..40).map(|i| format!("output line {i}\n")).collect(),
                    is_error: false,
                });
            }
            let theme = Theme::default();
            ...
            {
                match action {
                    Action::ScrollUp(n) => model.scroll_from_bottom += n,
                    Action::ScrollDown(n) => {
                        model.scroll_from_bottom = model.scroll_from_bottom.saturating_sub(n)
                    }
                    _ => {}
                }
            }
    ...
    fn live_thinking_expansion_survives_notices_and_multiple_blocks() {
        ...
        let theme = Theme::default();
        let mut chat = Chat {
            config: serde_json::json!({"thinking":{"display":"collapsed"}}),
            ..Default::default()
        };
        let toggle = KeyEvent::new(KeyCode::Char('t'), KeyModifiers::ALT);
        assert!(
        ...
        model
            .entries
            .push(Entry::Notice("intervening event".into()));
        ...
        model.entries.push(Entry::Assistant {
            ...
            content: vec![
                ...
                ContentPart::Text {
                    text: "between blocks".into(),
                },
                ContentPart::Thinking {
                    text: "separate block".into(),
                    signature: None,
                },
            ],
        });
    ...
    fn thinking_bindings_are_overridable_and_disableable() {
        ...
        assert!(
            !chat
                .on_key(&ctx, KeyEvent::new(KeyCode::Char('p'), KeyModifiers::ALT))
                .handled
        );
        assert!(
            !chat
                .on_key(&ctx, KeyEvent::new(KeyCode::Char('n'), KeyModifiers::ALT))
                .handled
        );
        assert!(
            !chat
                .on_key(&ctx, KeyEvent::new(KeyCode::Char('t'), KeyModifiers::ALT))
                .handled
        );
        assert!(
            chat.on_key(&ctx, KeyEvent::new(KeyCode::F(7), KeyModifiers::NONE))
                .handled
        );
        assert!(
            chat.on_key(&ctx, KeyEvent::new(KeyCode::F(6), KeyModifiers::NONE))
                .handled
        );
        assert_eq!(chat.focused_thinking, Some((0, 0)));
    }
