...
impl Input {
    pub fn new() -> Self {
        Self {
            ...
            history_image_index: 0,
            history_preview: false,
            history_image_key: crate::keys::Chord::parse("alt+h"),
            image_preview: false,
            image_preview_key: crate::keys::Chord::parse("alt+p"),
            image_close_key: crate::keys::Chord::parse("esc"),
            thumbnails: Default::default(),
            selected_image: 0,
            next_image_key: crate::keys::Chord::parse("alt+right"),
            image_session: None,
            images: Vec::new(),
            clipboard_key: crate::keys::Chord::parse("ctrl+v"),
            remove_image_key: crate::keys::Chord::parse("alt+backspace"),
            edit_key: crate::keys::Chord::parse("ctrl+e"),
            preview_key: crate::keys::Chord::parse("ctrl+g"),
            preview: None,
            preview_scroll: 0,
            preview_max: 0,
            paste_lines: 5,
            ...
            editor: Editor::new(),
            scroll_top: 0,
            candidates: vec![("unload".into(), "Unload a plugin".into())],
            selected: 0,
            dismissed: false,
            history: Vec::new(),
            history_index: None,
    ...
    fn at_token(&self) -> Option<String> {
        if self.editor.selected_paste().is_some() {
            return None;
        }
    ...
    fn matches(&self) -> Vec<&(String, String)> {
        if !self.dismissed && self.at_token().as_deref() == Some(self.reference_query.as_str()) {
            return self.references.iter().collect();
        }
        if self.editor.has_pastes() {
            return Vec::new();
        }
        ...
        self.commands
            ...
            .chain(
                self.candidates
                    .iter()
                    .filter(|(name, _)| !self.commands.iter().any(|(command, _)| command == name)),
            )
            .filter(|(name, _)| {
                if query.contains(' ') {
                    name.contains(' ') && name.to_lowercase().starts_with(&query)
                } else {
                    !name.contains(' ') && name.to_lowercase().starts_with(&query)
                }
            })
...
mod tests {
    ...
    fn named_submit_is_not_redirected_by_configured_clipboard_key() {
        ...
        assert!(
            matches!(outcome.actions.as_slice(), [Action::Submit(text)] if text == "draft"),
            "named submit was redirected: {:?}",
            outcome.actions
        );
    }
    ...
    fn editor_modifier_compatibility_is_preserved() {
        ...
        for mods in [
            ...
            assert!(
                input
                    .on_key(&ctx, KeyEvent::new(KeyCode::Left, mods))
                    .handled
            );
            assert!(
                input
                    .on_key(&ctx, KeyEvent::new(KeyCode::Backspace, mods))
                    .handled
            );
    ...
    fn configured_binding_declarations_drive_help_and_dispatch() {
        ...
        assert!(input
            .binding_help()
            .iter()
            .any(|line| line.contains("F(8)") && line.contains("paste_clipboard")));
        assert!(matches!(
            input
                .on_key(&ctx, KeyEvent::from(KeyCode::F(8)))
                .actions
                .as_slice(),
        ...
        assert!(!input
            .binding_help()
            .iter()
            .any(|line| line.contains("paste_clipboard")));
    ...
    fn history_preview_is_read_only_and_preserves_draft() {
        ...
        assert!(input
            .on_key(&ctx, KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE))
            .actions
            .is_empty());
    ...
    fn unified_clipboard_paste_uses_promptbox_key_and_text_paste_behavior() {
        ...
        input.on_action(
            &ctx,
            "input:promptbox-config",
            &serde_json::json!({"keys":{"paste":"ctrl+y"}}),
        );
        assert!(matches!(
            input
                .on_key(
        ...
        assert!(!input
            ...
            .actions
            .iter()
            .any(|a| matches!(a, Action::PasteClipboard)));
        ...
        input.on_action(
            &ctx,
            "input:promptbox-config",
            &serde_json::json!({"keys":{"paste":false}}),
        );
    ...
    fn raster_preview_uses_configured_keys_and_renders_pixels() {
        ...
        input.on_action(
            &ctx,
            "input:promptbox-config",
            &serde_json::json!({"images":{"keys":{"preview":"ctrl+y","close":"ctrl+x"}}}),
        );
        input.on_action(
            &ctx,
            "input:image-added",
            &serde_json::json!({"id":"a","media_type":"image/png","width":1,"height":2,"bytes":8}),
        );
        input.on_action(
            &ctx,
            "input:image-thumbnail",
            &serde_json::json!({"id":"a","width":1,"height":2,"pixels":[255,0,0,255,0,0,255,255]}),
        );
    ...
    fn image_draft_survives_editor_and_clears_only_on_success() {
        ...
        input.on_action(
            &ctx,
            "input:prompt-edited",
            &serde_json::json!({"text":"describe"}),
        );
        assert_eq!(input.images.len(), 2);
        let outcome = input.on_key(&ctx, KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
        assert!(
            matches!(&outcome.actions[..], [Action::SubmitImages(text, images)] if text == "describe" && images.len() == 2)
        );
        let outcome = input.on_binding(&ctx, "steer");
        assert!(
            matches!(&outcome.actions[..], [Action::SteerImages(text, images)] if text == "describe" && images.len() == 2)
        );
        let outcome = input.on_binding(&ctx, "queue");
        assert!(
            matches!(&outcome.actions[..], [Action::SubmitImages(_, images)] if images.len() == 2)
        );
    ...
    fn paste_preview_keys_are_configurable_and_modifier_sensitive() {
        ...
        assert!(
            matches!(&outcome.actions[..], [Action::Custom(name, payload)] if name == "terminal:edit-prompt" && payload["text"] == format!("{first}between{second}"))
        );
        input.on_action(
            &ctx,
            "input:prompt-edited",
            &serde_json::json!({"text": "edited draft"}),
        );
        ...
        input.on_action(
            &ctx,
            "input:promptbox-config",
            &serde_json::json!({"keys": {"edit": false}, "paste": {"keys": {"preview": false}}}),
        );
    ...
    fn reference_picker_quotes_paths_preserves_suffix_and_drills() {
        ...
        for _ in 0..6 {
            input.editor.move_left();
        }
        input.on_action(
            &ctx,
            "input:completion",
            &serde_json::json!({"text":"Read @old", "values":["wrong"]}),
        );
        assert!(input.matches().is_empty());
        input.on_action(
            &ctx,
            "input:completion",
            &serde_json::json!({"text":"Read @日", "values":["日本語/a b.txt"]}),
        );
        ...
        input.on_action(
            &ctx,
            "input:completion",
            &serde_json::json!({"text":"@", "values":["docs/"]}),
        );
        ...
        input.editor.take();
        input.editor.insert_str("mail@host");
    ...
    fn typing_and_accepting_commands_requests_dynamic_completion() {
        ...
        assert!(
            matches!(outcome.actions.as_slice(), [Action::Complete(text)] if text == "/model ")
        );
        input.on_action(
            &ctx,
            "input:completion",
            &serde_json::json!({"text":"/model ", "values":["chatgpt/test"]}),
        );
        ...
        input.on_action(
            &ctx,
            "input:commands",
            &serde_json::json!([["model", "Select model"]]),
        );
    ...
    fn exact_agent_completions_submit_on_enter_without_dismissing_picker() {
        ...
        for text in ["/agents", "/agents ", "/agents a1", "/agents a1 "] {
            for named in [false, true] {
                ...
                input.on_action(
                    &ctx,
                    "input:commands",
                    &serde_json::json!([["agents", "Monitor"]]),
                );
                ...
                let outcome = if named {
                    let action = input
                        ...
                        .into_iter()
                        .find(|b| {
                            b.chord.code == KeyCode::Enter && b.chord.mods == KeyModifiers::NONE
                        })
                        .unwrap()
                        .action;
                ...
                assert!(
                    matches!(outcome.actions.as_slice(), [Action::Submit(sent)] if sent == text),
                    "{text}: {:?}",
                    outcome.actions
                );
        ...
        let outcome = input.on_key(&ctx, KeyEvent::new(KeyCode::Tab, KeyModifiers::NONE));
        assert!(
            matches!(outcome.actions.as_slice(), [Action::Complete(text)] if text == "/agents ")
        );
        input.editor.take();
    ...
    fn agents_stop_id_completes_as_a_multiword_argument_while_busy() {
        ...
        input.on_action(
            &ctx,
            "input:commands",
            &serde_json::json!([["agents", "Stop children"]]),
        );
    ...
    fn completion_hints_are_display_only_and_ignore_stale_queries() {
        ...
        assert_eq!(
            input.matches()[0],
            &("agents a1".into(), "scout · Trace compaction".into())
        );
        input.on_key(&ctx, KeyEvent::new(KeyCode::Tab, KeyModifiers::NONE));
        assert_eq!(input.editor.text().trim(), "/agents a1");
        let outcome = input.on_key(&ctx, KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
        assert!(
            matches!(outcome.actions.as_slice(), [Action::Submit(text)] if text.trim() == "/agents a1")
        );
    }
    ...
    fn dynamic_completion_ignores_stale_queries() {
        ...
        assert!(
            matches!(outcome.actions.as_slice(), [Action::Complete(text)] if text == "/project pa")
        );
        input.on_action(
            &ctx,
            "input:completion",
            &serde_json::json!({"text":"/project old", "values":["wrong"]}),
        );
        assert!(input.matches().is_empty());
        input.on_action(
            &ctx,
            "input:completion",
            &serde_json::json!({"text":"/project pa", "values":["path"]}),
        );
    ...
    fn history_walk_restores_draft_and_does_not_submit() {
        ...
        for (key, expected) in [
            ...
            (KeyCode::Up, "primero"),
            (KeyCode::Up, "primero"),
            (KeyCode::Down, "segundo"),
            (KeyCode::Down, "borrador ñ"),
            (KeyCode::Down, "borrador ñ"),
        ] {
            assert!(input
                .on_key(&ctx, KeyEvent::new(key, KeyModifiers::NONE))
                .actions
                .is_empty());
    ...
    fn picker_accepts_without_submitting_and_escape_preserves_draft() {
        ...
        let outcome = input.on_key(&ctx, KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
        assert!(
            matches!(outcome.actions.as_slice(), [Action::Complete(text)] if text == "/unload ")
        );
        ...
        assert!(input
            .binding_help()
            .iter()
            .any(|line| line.contains("→ submit")));
        assert!(!input
            .binding_help()
            .iter()
            .any(|line| line.contains("completion_accept")));
    }
}
...
impl Component for Input {
    ...
    fn on_action(&mut self, ctx: &Ctx<'_>, name: &str, payload: &serde_json::Value) {
        if name == "input:history-images" {
            if let Ok(images) =
                serde_json::from_value::<Vec<rness_protocol::events::ImageRef>>(payload.clone())
            {
        ...
        if name == "input:clipboard-text" {
            if let Some(text) = payload.as_str() {
                self.on_paste(ctx, text);
            }
            return;
        }
        if name == "input:image-thumbnail" {
            if let (Some(id), Some(width), Some(height), Ok(pixels)) = (
                payload["id"].as_str(),
                payload["width"].as_u64(),
                payload["height"].as_u64(),
                serde_json::from_value::<Vec<u8>>(payload["pixels"].clone()),
            ) {
                if width <= 160
                    && height <= 80
                    && self
                        .images
                        .iter()
                        .chain(self.history_images.iter())
                        .any(|image| image.id == id)
                {
                    if let Some(image) =
                        image::RgbaImage::from_raw(width as u32, height as u32, pixels)
                    {
                        self.thumbnails.insert(id.into(), image);
                    }
                }
            }
            return;
        ...
        if name == "input:images-submitted" {
            self.images.clear();
        ...
        if name == "input:promptbox-config" {
            for (value, target) in [
                (
                    &payload["images"]["keys"]["history"],
                    &mut self.history_image_key,
                ),
                (
                    &payload["images"]["keys"]["preview"],
                    &mut self.image_preview_key,
                ),
                (
                    &payload["images"]["keys"]["close"],
                    &mut self.image_close_key,
                ),
                (&payload["images"]["keys"]["next"], &mut self.next_image_key),
                (&payload["keys"]["paste"], &mut self.clipboard_key),
                (
                    &payload["images"]["keys"]["remove"],
                    &mut self.remove_image_key,
                ),
                (&payload["keys"]["edit"], &mut self.edit_key),
                (&payload["paste"]["keys"]["preview"], &mut self.preview_key),
            ] {
                if value == &serde_json::Value::Bool(false) {
                    *target = None;
                } else if let Some(key) = value.as_str() {
                    if let Some(chord) = crate::keys::Chord::parse(key) {
                        *target = Some(chord);
                    } else {
                        tracing::warn!(key, "invalid promptbox shortcut; retaining default");
                    }
                }
            }
            if let Some(n) = payload["paste"]["lines"].as_u64() {
                self.paste_lines = n as usize;
            }
            if let Some(n) = payload["paste"]["chars"].as_u64() {
                self.paste_chars = n as usize;
            }
            if let Ok(argv) = serde_json::from_value::<Vec<String>>(payload["editor"].clone()) {
                self.external_editor = Some(argv);
            }
            return;
        }
        ...
        if name == "input:completion" {
            ...
            if payload["text"].as_str() != Some(self.editor.text().as_str()) {
                return;
            }
            let items = if payload.get("items").is_some() {
                serde_json::from_value::<Vec<(String, String)>>(payload["items"].clone())
            } else {
                serde_json::from_value::<Vec<String>>(payload["values"].clone()).map(|values| {
                    values
                        .into_iter()
            ...
            if let (Some(text), Ok(values)) = (payload["text"].as_str(), items) {
                ...
                self.commands.extend(
                    values
                        .into_iter()
                        .map(|(value, description)| (format!("{prefix}{value}"), description)),
                );
        ...
        if name == "input:skills" {
            if let Ok(skills) = serde_json::from_value::<Vec<(String, String)>>(payload.clone()) {
                ...
                for (name, description) in skills {
                    self.candidates
                        .push((format!("skill {name}"), description.clone()));
                    if !["agent", "skill", "unload", "colorscheme"].contains(&name.as_str()) {
                        self.candidates
                            .push((name, format!("Skill: {description}")));
                    }
                }
                self.selected = 0;
            }
        ...
        if name != "input:plugins" {
            return;
        }
        if let Ok(names) = serde_json::from_value::<Vec<String>>(payload.clone()) {
            self.candidates
                .retain(|(name, _)| !name.starts_with("unload "));
            self.candidates.extend(
                names
                    .into_iter()
                    .map(|name| (format!("unload {name}"), "Loaded plugin".into())),
            );
            self.selected = 0;
        }
    }
    ...
    fn height(&self, _ctx: &Ctx<'_>, width: u16) -> Option<u16> {
        if self.image_preview || self.preview.is_some() {
            return Some(18);
        }
    ...
    fn render(&mut self, ctx: &Ctx<'_>, area: Rect, buf: &mut Buffer) {
        if self.image_preview && self.image_session.as_ref() == Some(&ctx.model.session) {
            ...
            let reference = if self.history_preview {
                self.history_images.get(self.history_image_index)
            } else {
            ...
            if let Some(source) = reference.and_then(|r| self.thumbnails.get(&r.id)) {
                if inner.width > 0 && inner.height > 0 {
                    let image = image::DynamicImage::ImageRgba8(source.clone())
                        .thumbnail(
                            u32::from(inner.width).min(source.width()),
                            (u32::from(inner.height) * 2).min(source.height()),
                        )
                        .to_rgba8();
                    for y in 0..image.height().div_ceil(2) {
                        for x in 0..image.width() {
                            let color = |p: &image::Rgba<u8>| {
                            ...
                            let bottom = if y * 2 + 1 < image.height() {
                                color(image.get_pixel(x, y * 2 + 1))
                            } else {
                ...
                Line::raw("Preview unavailable for this attachment").render(inner, buf);
            }
            return;
        }
        if let Some((id, content)) = &self.preview {
            ...
            let block = Block::default()
                .borders(Borders::ALL)
                .title(format!(" Paste #{id} "))
                .title_bottom(" Esc: close · arrows/PageUp/PageDown: scroll ");
            let inner = block.inner(area);
            block.render(area, buf);
            ...
            let lines: Vec<_> = rows
                ...
                .map(|r| Line::raw(display.display_lines()[r.line][r.start..r.end].to_owned()))
                .collect();
            Paragraph::new(lines).render(inner, buf);
            return;
        }
        if !self.images.is_empty()
            && self.image_session.as_ref() == Some(&ctx.model.session)
            && area.height > 0
        {
            ...
            Line::styled(
                ...
                ctx.theme.editor_prompt,
            )
            .render(Rect::new(area.x, area.y, area.width, 1), buf);
        } else if self.editor.selected_paste().is_some() && area.height > 0 {
            Line::styled("Paste selected", ctx.theme.editor_prompt)
                .render(Rect::new(area.x, area.y, area.width, 1), buf);
        }
    ...
    fn bindings(&self) -> Vec<crate::keymaps::ComponentBinding> {
        if self.preview.is_some() {
            return self
                .edit_key
                .into_iter()
                .map(|chord| crate::keymaps::ComponentBinding {
                ...
                .chain(
                    [
                        ...
                        ("down", "preview_down"),
                        ("j", "preview_down"),
                        ("pageup", "preview_page_up"),
                        ("pagedown", "preview_page_down"),
                        ("home", "preview_start"),
                        ("end", "preview_end"),
                    ]
                    .into_iter()
                    .map(|(key, action)| crate::keymaps::ComponentBinding {
                        action,
                        chord: crate::keys::Chord::parse(key).expect("preview chord"),
                    }),
                )
                .collect();
        }
        let mut bindings: Vec<_> = [
            (self.clipboard_key, "paste_clipboard"),
            (self.edit_key, "external_editor"),
            (self.preview_key, "paste_preview"),
            (self.history_image_key, "history_images"),
            (self.image_preview_key, "image_preview"),
            (self.image_close_key, "close_image_preview"),
            (self.next_image_key, "next_image"),
            (self.remove_image_key, "remove_image"),
        ]
        .into_iter()
        .filter_map(|(chord, action)| {
            chord.map(|chord| crate::keymaps::ComponentBinding { action, chord })
        })
        .collect();
        let matches = self.matches();
        let keys = if !matches.is_empty() {
            ...
            vec![
                ...
                ("tab", "completion_accept"),
                ("enter", if exact { "submit" } else { "completion_accept" }),
                ("esc", "completion_dismiss"),
            ]
        } else {
            Vec::new()
        };
        bindings.extend(
            keys.into_iter()
                ...
                .map(|(key, action)| crate::keymaps::ComponentBinding {
                    action,
                    chord: crate::keys::Chord::parse(key).expect("editor chord"),
                }),
        );
        bindings
    }
    ...
    fn binding_help(&self) -> Vec<String> {
        if self.preview.is_some() {
            let mut lines: Vec<_> = self
                .bindings()
                .iter()
                .map(|binding| binding.help("Paste preview"))
                .collect();
            lines.push(
                "Paste preview captures all other keys; prompt and global mappings are inactive."
                    .into(),
            );
            return lines;
        }
        if self.image_preview && self.history_preview {
            let mut lines: Vec<_> = self
                ...
                .filter(|binding| {
                    matches!(
                        binding.action,
                        "history_images" | "close_image_preview" | "next_image"
                    )
                })
                .map(|binding| binding.help("History image preview"))
                .collect();
        ...
        let mut lines = vec![
            ...
            "Queue (submit): later turn; Steer: next model step. Both start a turn when idle."
                .into(),
        ];
        let completion = !self.matches().is_empty();
        let bindings = self.bindings();
        if completion {
            lines.push("Completion menu: navigation, acceptance and dismissal take precedence over prompt mappings.".into());
        }
        lines.extend(
            bindings
                .iter()
                .filter(|binding| {
                    !completion
                        || binding.action.starts_with("completion_")
                        || !bindings.iter().any(|other| {
                            other.action.starts_with("completion_") && other.chord == binding.chord
                        })
                })
    ...
    fn on_binding(&mut self, ctx: &Ctx<'_>, action: &str) -> KeyOutcome {
        ...
        self.handle_key(
            ctx,
            KeyEvent::new(binding.chord.code, binding.chord.mods),
            Some(action),
        )
    }
}
...
impl Input {
    fn handle_key(&mut self, ctx: &Ctx<'_>, key: KeyEvent, action: Option<&str>) -> KeyOutcome {
        ...
        let matched = |name: &str| {
            action.map_or_else(
                || bindings.iter().any(|binding| binding.matches(name, &key)),
                |action| action == name,
            )
        };
        if self
            .image_session
            .as_ref()
            .is_some_and(|session| session != &ctx.model.session)
        {
            self.images.clear();
            self.thumbnails.clear();
        ...
        if self.image_preview && matched("close_image_preview") {
            self.image_preview = false;
            self.history_preview = false;
            return KeyOutcome::consumed();
        }
        if self.image_preview && self.history_preview {
            if matched("next_image") && !self.history_images.is_empty() {
                self.history_image_index =
                    (self.history_image_index + 1) % self.history_images.len();
            }
            return KeyOutcome::consumed();
        }
        if matched("image_preview") && !self.images.is_empty() {
            self.image_preview = !self.image_preview;
            return KeyOutcome::consumed();
        }
        if matched("paste_clipboard") {
            return KeyOutcome::act(vec![Action::PasteClipboard]);
        }
        ...
        if !self.images.is_empty()
            ...
            && (matched("submit") || matched("steer"))
        {
            return KeyOutcome::act(vec![if matched("steer") {
                Action::SteerImages(self.editor.text(), self.images.clone())
            } else {
                Action::SubmitImages(self.editor.text(), self.images.clone())
            }]);
        }
        if matched("external_editor") {
            return KeyOutcome::act(vec![Action::Custom(
                "terminal:edit-prompt".into(),
                serde_json::json!({"text": self.editor.text(), "editor": self.external_editor}),
            )]);
        }
        ...
        if self.preview.is_some() {
            ...
            match action {
                ...
                Some("preview_down") => {
                    self.preview_scroll = (self.preview_scroll + 1).min(self.preview_max)
                }
                Some("preview_page_up") => {
                    self.preview_scroll = self.preview_scroll.saturating_sub(10)
                }
                Some("preview_page_down") => {
                    self.preview_scroll =
                        self.preview_scroll.saturating_add(10).min(self.preview_max)
                }
        ...
        if !matches.is_empty() {
            ...
            match action.or_else(|| {
                bindings
                    .iter()
                    .find(|binding| {
                        binding.action.starts_with("completion_") && binding.matches_key(&key)
                    })
                    .map(|binding| binding.action)
            }) {
                ...
                Some("completion_accept") => {
                    if let Some(token) = self.at_token() {
                        ...
                        self.selected = 0;
                        return if drill {
                            KeyOutcome::act(vec![Action::Complete(self.editor.before_cursor())])
                        } else {
                            KeyOutcome::consumed()
                        };
                    }
        ...
        if key.modifiers == KeyModifiers::NONE
            && (matched("cursor_up") || matched("cursor_down"))
        ...
        let outcome = match (key.code, key.modifiers) {
            ...
            _ if matched("submit") || matched("steer") => {
                ...
                if !text.trim().is_empty()
                    && self.history.last().map(Editor::text).as_ref() != Some(&text)
                {
                ...
                self.dismissed = false;
                KeyOutcome::act(vec![if matched("steer") {
                    Action::Steer(text)
                } else {
                    Action::Submit(text)
                }])
