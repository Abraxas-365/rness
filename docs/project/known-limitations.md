...
- **Persistent terminals are plain text, Unix-only in practice, and not restored.** [Terminals](../guides/terminals.md#limitations) clean output rather than emulate a screen, so full-screen programs are reported instead of shown. Only Linux detects that a command is waiting for input. Login shells report no exit codes. Terminals close when rness exits, but not when you switch or delete sessions, and a signal-killed rness leaves cleanup to the PTY hangup. They run with the rness user's full authority: the filesystem sandbox does not apply to them, and a non-`danger-full-access` mode disables them instead.
